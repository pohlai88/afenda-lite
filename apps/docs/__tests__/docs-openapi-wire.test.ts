...
const generatePackageDocsScriptPath = join(
	appDir,
	"scripts/generate-package-docs.mts",
);
...
describe("docs OpenAPI wire", () => {
	...
	it("locks AsyncAPI Outside baseline (OpenAPI-only API docs)", () => {
		...
		expect(pkg.scripts?.["generate:package-docs"]).toContain(
			"generate-package-docs.mts",
		);
		expect(pkg.scripts?.prebuild).toContain("generate:package-docs");
	});

	it("locks package docs generator (packages/* → content/docs/packages)", () => {
		expect(existsSync(generatePackageDocsScriptPath)).toBe(true);
		const generatePackageDocs = readFileSync(
			generatePackageDocsScriptPath,
			"utf8",
		);
		expect(generatePackageDocs).toContain("content/docs/packages");
		expect(generatePackageDocs).toContain("generate:package-docs");
		expect(generatePackageDocs).toContain(
			"https://github.com/pohlai88/afenda-lite/blob/main",
		);
		expect(generatePackageDocs).toContain("index.mdx");

		const packagesDir = join(appDir, "content/docs/packages");
		expect(existsSync(packagesDir)).toBe(true);
		expect(existsSync(join(packagesDir, "index.mdx"))).toBe(true);
		expect(existsSync(join(packagesDir, "meta.json"))).toBe(true);

		const packagesIndex = readFileSync(join(packagesDir, "index.mdx"), "utf8");
		expect(packagesIndex).not.toContain(
			"This file is auto-generated by `pnpm --filter @afenda/docs generate:package-docs`",
		);

		const packagesMeta = JSON.parse(
			readFileSync(join(packagesDir, "meta.json"), "utf8"),
		) as {
			readonly title?: string;
			readonly icon?: string;
			readonly pages?: readonly string[];
		};
		expect(packagesMeta.title).toBe("Packages");
		expect(packagesMeta.icon).toBe("Package");
		expect(packagesMeta.pages?.[0]).toBe("index");
		expect(packagesMeta.pages).toContain("errors");
		expect(packagesMeta.pages).toContain("env");
		...
		for (const entry of readdirSync(packagesDir)) {
			...
			const body = readFileSync(join(packagesDir, entry), "utf8");
			expect(body).toContain(generatedBanner);
		}
		...
		expect(rootPkg.scripts?.["check:docs-app"]).toContain(
	...
	it("locks page conventions (slugs · meta · no root tabs folders)", () => {
		...
		expect(rootMeta.pages).toEqual(["index", "guide", "api", "packages"]);
		...
		const packagesMetaPath = join(appDir, "content/docs/packages/meta.json");
		expect(existsSync(packagesMetaPath)).toBe(true);
		const packagesMeta = JSON.parse(readFileSync(packagesMetaPath, "utf8")) as {
			readonly pages?: readonly string[];
			readonly title?: string;
			readonly root?: boolean;
			readonly icon?: string;
		...
		expect(packagesMeta.title).toBe("Packages");
		expect(packagesMeta.pages?.[0]).toBe("index");
		expect(packagesMeta.root).toBeUndefined();
		expect(packagesMeta.icon).toBe("Package");
