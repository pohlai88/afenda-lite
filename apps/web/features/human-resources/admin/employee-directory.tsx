...
export async function EmployeeDirectoryWorkspace({
	...
	const totalPages = employees.ok
		? Math.max(1, Math.ceil(employees.data.totalCount / employees.data.pageSize))
		: 1;

	return (
		<main className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-8 sm:px-6">
			<header className="space-y-2">
				<p className="text-sm text-muted-foreground">Operator · Human resources</p>
			...
			<nav aria-label="Human resources administration" className="flex flex-wrap gap-2">
			...
			>
				...
				<div className="space-y-2">
					...
					<Input
						...
						maxLength={200}
						placeholder={
							params.field === "name" ? "Start of legal name" : "Start of employee number"
						}
					/>
				</div>
			...
			{!employees.ok ? (
				...
				<section aria-labelledby="employee-directory-heading" className="space-y-4">
					<div className="flex flex-wrap items-end justify-between gap-3">
						<div>
							<h2 id="employee-directory-heading" className="text-lg font-medium">
								Employee directory
							</h2>
							<p className="text-sm text-muted-foreground">
								{employees.data.totalCount} records · page {employees.data.page} of {totalPages}
							</p>
						</div>
					</div>
							...
							<TableBody>
								{employees.data.employees.map((employee) => (
									<TableRow key={employee.id}>
										...
										<TableCell>{formatHrInstant(employee.updatedAt, preferences)}</TableCell>
										<TableCell className="text-right">
											<Button asChild variant="ghost" size="sm">
												<Link href={`/admin/human-resources/employees/${employee.id}`}>
													Open
												</Link>
											</Button>
										</TableCell>
					...
					<nav aria-label="Employee directory pages" className="flex items-center justify-between">
						<Button asChild variant="outline" size="sm" disabled={params.page <= 1}>
							<Link
								href={pageHref({ page: Math.max(1, params.page - 1), params, preferences })}
						...
						<Button asChild variant="outline" size="sm" disabled={params.page >= totalPages}>
							<Link
