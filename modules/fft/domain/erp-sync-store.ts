...
function mapJobRow(row: Record<string, unknown>): FftSyncJob {
  return {
    id: String(row.id),
    jobType: String(row.job_type) as FftSyncJobType,
    entityId: String(row.entity_id),
    idempotencyKey: String(row.idempotency_key),
    status: String(row.status) as FftSyncJob["status"],
...
export async function enqueueErpSyncJob(input: {
  ...
  actorId?: string;
  version?: string;
}): Promise<FftSyncJob | null> {
  if (!isFftErpSyncEnabled()) return null;
  ...
  if (result.rows.length === 0) {
    const existing = await pool.query(
      `SELECT * FROM fft_sync_job WHERE idempotency_key = $1`,
      [idempotencyKey],
    );
...
export async function listSyncJobs(limit = 50): Promise<FftSyncJob[]> {
  const result = await pool.query(
    `SELECT * FROM fft_sync_job ORDER BY created_at DESC LIMIT $1`,
    [limit],
  );
  return result.rows.map(mapJobRow);
}

export async function listPendingSyncJobs(limit = 20): Promise<FftSyncJob[]> {
  const result = await pool.query(
    `SELECT * FROM fft_sync_job
...
export async function getSyncJobById(jobId: string): Promise<FftSyncJob | null> {
  const result = await pool.query(`SELECT * FROM fft_sync_job WHERE id = $1`, [jobId]);
  return result.rows[0] ? mapJobRow(result.rows[0]) : null;
}

export async function getSyncJobDetail(jobId: string): Promise<FftSyncJobDetail | null> {
  const job = await getSyncJobById(jobId);
  if (!job) return null;

  const attemptsResult = await pool.query(
    `SELECT * FROM fft_sync_attempt WHERE job_id = $1 ORDER BY attempt_no ASC`,
    [jobId],
  );

  const attempts: FftSyncJobDetail["attempts"] = [];
  for (const row of attemptsResult.rows) {
    const attempt: FftSyncAttempt = {
      id: String(row.id),
      jobId: String(row.job_id),
      attemptNo: Number(row.attempt_no),
      status: String(row.status) as FftSyncAttempt["status"],
      startedAt: new Date(String(row.started_at)),
      finishedAt: row.finished_at ? new Date(String(row.finished_at)) : null,
    };
    const errorsResult = await pool.query(
      `SELECT * FROM fft_sync_error WHERE attempt_id = $1 ORDER BY created_at ASC`,
      [attempt.id],
    );
    const errors: FftSyncError[] = errorsResult.rows.map((err) => ({
      id: String(err.id),
...
export async function retrySyncJob(jobId: string): Promise<FftSyncJob | null> {
  if (!isFftErpSyncEnabled()) return null;
...
async function recordAttempt(input: {
  ...
  const result = await pool.query(
    `INSERT INTO fft_sync_attempt (job_id, attempt_no, status, finished_at, provider_response)
...
async function recordSyncError(input: {
  ...
  await pool.query(
    `INSERT INTO fft_sync_error (attempt_id, code, message, retryable)
     VALUES ($1, $2, $3, $4)`,
...
export async function processSyncJob(
  ...
  await pool.query(
    `UPDATE fft_sync_job SET status = 'running', attempt_count = $2, updated_at = NOW() WHERE id = $1`,
    [job.id, attemptNo],
  );
  ...
  if (result.ok) {
    ...
    if (result.externalId) {
      await pool.query(
        `INSERT INTO fft_external_mapping
    ...
    await pool.query(
      `UPDATE fft_sync_job SET status = 'succeeded', last_error = NULL, updated_at = NOW() WHERE id = $1`,
      [job.id],
    );
    return { ok: true };
  }
