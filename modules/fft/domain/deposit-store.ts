...
} from "@/modules/fft/domain/deposit";
import type {
  FftDepositAdjustment,
  FftDepositListItem,
  FftDepositReceipt,
  FftDepositRecord,
  FftDepositStatus,
  FftFinanceAuditEntry,
} from "@/modules/fft/domain/types";

function mapDepositRow(row: Record<string, unknown>): FftDepositRecord {
  return {
    id: String(row.id),
...
function mapReceiptRow(row: Record<string, unknown>): FftDepositReceipt {
  return {
    id: String(row.id),
...
function mapAdjustmentRow(row: Record<string, unknown>): FftDepositAdjustment {
  return {
    id: String(row.id),
    depositId: String(row.deposit_id),
...
export async function getDepositByOrderId(
  orderId: string,
): Promise<FftDepositRecord | null> {
  const result = await pool.query(
    `SELECT * FROM fft_deposit WHERE order_id = $1`,
    [orderId],
  );
...
async function loadDepositProjectionContext(orderId: string) {
  const orderResult = await pool.query(
    `SELECT o.deposit_status, e.deposit_required
     FROM fft_order o
     JOIN fft_event e ON e.id = o.event_id
     WHERE o.id = $1`,
    [orderId],
  );
  ...
  if (deposit) {
    const receipts = await pool.query(
      `SELECT amount FROM fft_deposit_receipt WHERE deposit_id = $1`,
      [deposit.id],
    );
    ...
    const adjustments = await pool.query(
      `SELECT adjustment_type FROM fft_deposit_adjustment
       WHERE deposit_id = $1
  ...
  const projected = projectDepositStatus({
    ...
    totalReceiptAmount,
    terminalAdjustment,
    legacyStatus: String(orderRow.deposit_status) as FftDepositStatus,
  });
...
export async function syncOrderDepositProjection(orderId: string) {
  ...
  await pool.query(
    `UPDATE fft_order SET deposit_status = $2, updated_at = NOW() WHERE id = $1`,
    [orderId, projected],
  );
  if (deposit && DEPOSIT_ROW_STATUSES.has(projected)) {
    await pool.query(
      `UPDATE fft_deposit SET status = $2, updated_at = NOW() WHERE id = $1`,
...
export async function recordDepositReceipt(input: {
  ...
  const result = await pool.query(
    `INSERT INTO fft_deposit_receipt
...
export async function recordDepositAdjustment(input: {
  ...
  const depositResult = await pool.query(
    `SELECT non_refundable FROM fft_deposit WHERE id = $1`,
    [input.depositId],
  );
  ...
  const result = await pool.query(
    `INSERT INTO fft_deposit_adjustment
...
export async function listDepositsForEvent(
  ...
  return result.rows.map((row) => ({
    ...
    orderDepositStatus: String(row.order_deposit_status) as FftDepositStatus,
  }));
}

export async function listFinanceAuditForEvent(
  eventId: string,
): Promise<FftFinanceAuditEntry[]> {
  const result = await pool.query(
    `SELECT a.*
     FROM fft_finance_audit a
     LEFT JOIN fft_order o ON o.id = a.order_id
     LEFT JOIN fft_deposit d ON d.id = a.deposit_id
     LEFT JOIN fft_order o2 ON o2.id = d.order_id
...
export async function listReceiptsForDeposit(
  depositId: string,
): Promise<FftDepositReceipt[]> {
  const result = await pool.query(
    `SELECT * FROM fft_deposit_receipt WHERE deposit_id = $1 ORDER BY paid_at DESC`,
    [depositId],
  );
