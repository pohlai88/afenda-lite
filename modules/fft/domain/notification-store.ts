...
import type {
  FftNotificationDelivery,
  FftNotificationLocale,
  FftNotificationTemplate,
} from "@/modules/fft/domain/notification-types";

function mapTemplateRow(row: Record<string, unknown>): FftNotificationTemplate {
  return {
    id: String(row.id),
    eventKey: String(row.event_key),
    locale: String(row.locale) as FftNotificationLocale,
    subject: String(row.subject),
    bodyMarkdown: String(row.body_markdown),
  };
}

function mapDeliveryRow(row: Record<string, unknown>): FftNotificationDelivery {
  return {
    id: String(row.id),
    eventKey: String(row.event_key),
    ...
    status: String(row.status) as FftNotificationDelivery["status"],
...
export async function isNotificationEventEnabled(
  ...
  const result = await pool.query(
    `SELECT enabled FROM fft_notification_event
...
): Promise<FftNotificationTemplate | null> {
  const result = await pool.query(
    `SELECT * FROM fft_notification_template WHERE event_key = $1 AND locale = $2`,
    [eventKey, locale],
  );
...
): Promise<FftNotificationDelivery | null> {
  const result = await pool.query(
    `SELECT * FROM fft_notification_delivery WHERE idempotency_key = $1`,
    [idempotencyKey],
  );
...
export async function insertNotificationDelivery(input: {
  ...
  locale: FftNotificationLocale;
  idempotencyKey: string;
  status: FftNotificationDelivery["status"];
  providerId?: string;
  error?: string;
}): Promise<FftNotificationDelivery> {
  const result = await pool.query(
    `INSERT INTO fft_notification_delivery
...
export async function updateDeliveryStatus(input: {
  ...
  await pool.query(
    `UPDATE fft_notification_delivery
