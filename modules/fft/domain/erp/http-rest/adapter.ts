import type { ErpAdapter, ErpPushResult } from "@/modules/fft/domain/erp/types";
...
/** Reference ERP pack — wire customer endpoints per docs/fft/integrations/. */
    ...
    async push(input): Promise<ErpPushResult> {
      if (!baseUrl) {
        return {
          ...
          message: "FFT_ERP_BASE_URL is not configured",
          retryable: false,
        };
      }

      try {
        const response = await fetch(`${baseUrl}/fft/sync/${input.jobType}`, {
          method: "POST",
          headers: {
