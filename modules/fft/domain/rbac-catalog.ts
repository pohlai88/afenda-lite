/**
 * Feed Farm Trade RBAC permission catalog + default role templates (data only).
 * Authorization must use permission codes — never role display names / template_key in business ifs.
 */

export const FFT_SCOPE_TYPES = [
  "own",
  "team",
  "event",
  "bu",
...
export type FftScopeType = (typeof FFT_SCOPE_TYPES)[number];

export type FftPermissionDef = {
  code: string;
  description: string;
...
export const FFT_PERMISSION_CATALOG: readonly FftPermissionDef[] = [
  { code: "event.view", description: "View Feed Farm Trade events", sensitive: false },
  { code: "event.create", description: "Create Feed Farm Trade events", sensitive: false },
...
export type FftPermissionCode =
  (typeof FFT_PERMISSION_CATALOG)[number]["code"];

export const SENSITIVE_PERMISSION_CODES: ReadonlySet<string> = new Set(
  FFT_PERMISSION_CATALOG.filter((p) => p.sensitive).map((p) => p.code),
);
...
export type FftRoleTemplateDef = {
...
const ALL_CODES = FFT_PERMISSION_CATALOG.map((p) => p.code);
...
export const FFT_ROLE_TEMPLATES: readonly FftRoleTemplateDef[] = [
  {
    templateKey: "super_admin",
    name: "Super Admin",
    description: "Platform / full Feed Farm Trade control",
