...
import { validateOrderAttrs, sanitizeFieldKey } from "@/modules/fft/domain/fields";
import type {
  AllocationInputOrder,
  FftFieldDef,
  FftProduct,
} from "@/modules/fft/domain/types";
import { formatCountdown, getCountdownParts } from "@/modules/fft/domain/countdown";
import { buildEventSummary, eventSummaryToCsv } from "@/modules/fft/domain/export";
import { calculateEstimatedSupport, canCompleteOrder } from "@/modules/fft/domain/support";
import { canTransferOrder } from "@/modules/fft/domain/transfer";
...
describe("transfer (G3)", () => {
  it("allows transfer when event permits and order is allocated", () => {
    expect(
      canTransferOrder(
        { status: "full", transferStatus: "none" },
  ...
  it("denies transfer when event disallows or order is not allocated", () => {
    ...
    expect(
      canTransferOrder(
        { status: "registered", transferStatus: "none" },
        { transferAllowed: true },
      ).reason,
