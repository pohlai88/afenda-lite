...
import { recordFftAudit } from "@/modules/fft/domain/store";
import {
...
} from "@/modules/fft/domain/pickup";
import type {
  FftFulfillmentRecord,
  FftPickupAssignment,
  FftPickupListItem,
  FftPickupWindow,
} from "@/modules/fft/domain/types";

function mapWindowRow(row: Record<string, unknown>): FftPickupWindow {
  return {
    id: String(row.id),
...
function mapAssignmentRow(row: Record<string, unknown>): FftPickupAssignment {
  return {
    id: String(row.id),
...
export async function createPickupWindow(input: {
  ...
  const result = await pool.query(
    `INSERT INTO fft_pickup_window (event_id, starts_at, ends_at, location, capacity)
...
export async function ensurePickupAssignment(orderId: string) {
  const existing = await pool.query(
    `SELECT * FROM fft_pickup_assignment WHERE order_id = $1`,
    [orderId],
  );
  ...
  const result = await pool.query(
    `INSERT INTO fft_pickup_assignment (order_id, status)
...
async function loadPickupContext(orderId: string) {
  const orderResult = await pool.query(
    `SELECT confirmed_quantity FROM fft_order WHERE id = $1`,
    [orderId],
  );
  ...
  if (assignment) {
    const fulfillments = await pool.query(
      `SELECT quantity FROM fft_fulfillment_record WHERE order_id = $1`,
      [orderId],
    );
    ...
    const exceptions = await pool.query(
      `SELECT exception_type FROM fft_pickup_exception WHERE order_id = $1`,
      [orderId],
    );
...
export async function syncOrderPickupProjection(orderId: string) {
  ...
  if (assignment) {
    await pool.query(
      `UPDATE fft_pickup_assignment SET status = $2, updated_at = NOW() WHERE id = $1`,
      [assignment.id, assignmentStatus],
    );

    if (assignmentStatus === "picked_up") {
      await pool.query(
        `UPDATE fft_order SET status = 'completed', updated_at = NOW()
...
export async function recordFulfillment(input: {
  ...
  await pool.query(
    `INSERT INTO fft_fulfillment_record
  ...
  if (input.finalSupport != null) {
    await pool.query(
      `UPDATE fft_order SET final_support = $2, updated_at = NOW() WHERE id = $1`,
...
export async function recordPickupException(input: {
  ...
  const assignment = await ensurePickupAssignment(input.orderId);
  await pool.query(
    `INSERT INTO fft_pickup_exception
...
export async function listPickupWindowsForEvent(
  eventId: string,
): Promise<FftPickupWindow[]> {
  const result = await pool.query(
    `SELECT * FROM fft_pickup_window WHERE event_id = $1 ORDER BY starts_at`,
    [eventId],
  );
...
export async function listPickupQueueForEvent(
  ...
  const result = await pool.query(
    `SELECT o.id AS order_id, o.order_number, o.customer_name, o.confirmed_quantity,
     ...
     FROM fft_order o
     LEFT JOIN fft_pickup_assignment a ON a.order_id = o.id
     WHERE o.event_id = $1
...
export async function listFulfillmentRecordsForOrder(
  orderId: string,
): Promise<FftFulfillmentRecord[]> {
  const result = await pool.query(
    `SELECT * FROM fft_fulfillment_record WHERE order_id = $1 ORDER BY recorded_at DESC`,
    [orderId],
  );
  return result.rows.map((row) => ({
    id: String(row.id),
