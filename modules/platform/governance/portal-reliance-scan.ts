...
} from "@/modules/platform/routing/surface-entry-points";
import { API_ROUTE_ACTION_IDS } from "@/modules/platform/api/routes";
...
export function extractLibImports(content: string): string[] {
  const specs: string[] = [];
  const patterns = [
    /import\s+(?:type\s+)?\{[^}]+\}\s*from\s*["'](@\/(?:lib|modules)\/[^"']+)["']/g,
    /import\s+(?!type)(\w+)\s*from\s*["'](@\/(?:lib|modules)\/[^"']+)["']/g,
  ];

  for (const re of patterns) {
    for (const match of content.matchAll(re)) {
      const spec = match[2] ?? match[1];
      if (spec?.startsWith("@/lib/") || spec?.startsWith("@/modules/")) {
        specs.push(spec);
      }
    }
  }
  ...
  for (const [helperName, actionId] of Object.entries(SESSION_HELPER_ACTIONS)) {
    const libPath =
      helperName === "requireAccountSession"
        ? "modules/identity/account-session.ts"
        : "modules/identity/auth/session.ts";
