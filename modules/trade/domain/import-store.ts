...
} from "@/modules/trade/domain/import-types";
import { canSubmitOrder } from "@/modules/trade/domain/events";
import { recordFulfillment } from "@/modules/trade/domain/pickup-store";
import { resolveDepositStatusForEvent } from "@/modules/trade/domain/transfer";
import {
  createOrder,
  getEventById,
  importPriorityCsv,
...
import { isHotSalesDepositEnabled } from "@/modules/platform/env/accessors";

function mapBatchRow(row: Record<string, unknown>): HotSalesImportBatch {
  return {
    id: String(row.id),
    eventId: String(row.event_id),
    importType: String(row.import_type) as HotSalesImportType,
    filename: String(row.filename),
    status: String(row.status) as HotSalesImportBatch["status"],
...
function mapImportRow(row: Record<string, unknown>): HotSalesImportRow {
  ...
  return {
    ...
    writeStatus: String(row.write_status) as HotSalesImportRow["writeStatus"],
    createdAt: new Date(String(row.created_at)),
  };
}
...
export async function createImportBatch(input: {
  ...
  try {
    ...
    const batchResult = await client.query(
      `INSERT INTO hot_sales_import_batch
    ...
    for (const row of input.rows) {
      await client.query(
        `INSERT INTO hot_sales_import_row
...
export async function getImportBatchById(
  batchId: string,
): Promise<HotSalesImportBatch | null> {
  const result = await pool.query(
    `SELECT * FROM hot_sales_import_batch WHERE id = $1`,
    [batchId],
  );
...
export async function listImportRowsForBatch(
  batchId: string,
): Promise<HotSalesImportRow[]> {
  const result = await pool.query(
    `SELECT * FROM hot_sales_import_row WHERE batch_id = $1 ORDER BY row_number`,
    [batchId],
  );
...
export async function cancelImportBatch(batchId: string): Promise<void> {
  await pool.query(
    `UPDATE hot_sales_import_batch SET status = 'cancelled' WHERE id = $1 AND status = 'dry_run'`,
    [batchId],
  );
}
...
export async function commitImportBatch(
  ...
  try {
    ...
    for (const row of validRows) {
      await client.query(
        `UPDATE hot_sales_import_row SET write_status = $2 WHERE id = $1`,
        [row.id, committedRowIds.has(row.id) ? "committed" : "failed"],
      );
    }
    for (const row of rows.filter((r) => r.validationErrors.length > 0)) {
      await client.query(
        `UPDATE hot_sales_import_row SET write_status = 'skipped' WHERE id = $1`,
        [row.id],
      );
    }
    await client.query(
      `UPDATE hot_sales_import_batch
    ...
    await client.query("ROLLBACK");
    await pool.query(
      `UPDATE hot_sales_import_batch SET status = 'failed' WHERE id = $1`,
      [batchId],
    );
    throw error;
  } finally {
