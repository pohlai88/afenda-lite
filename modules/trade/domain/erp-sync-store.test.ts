...
import { buildSyncIdempotencyKey, retrySyncJob } from "@/modules/trade/domain/erp-sync-store";
...
describe("retrySyncJob", () => {
  ...
  it("performs no SQL UPDATE when HOT_SALES_ERP_SYNC_ENABLED is false", async () => {
  ...
  it("returns failed/dead jobs to pending when flag is on", async () => {
    ...
    expect(sql).toContain("UPDATE hot_sales_sync_job");
