...
import { isSensitivePermission } from "@/modules/trade/domain/rbac-catalog";
...
const mocks = vi.hoisted(() => {
  ...
  const query = vi.fn(async (sql: string, params?: unknown[]) => {
    if (sql.includes("hot_sales_rbac_audit")) {
      auditRows.push({
    ...
    if (
      sql.includes("INSERT INTO hot_sales_role") &&
    ...
    if (sql.includes("SELECT permission_code FROM hot_sales_role_permission")) {
      return { rows: [] };
    }
...
describe("hot_sales_rbac_audit writes", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("records audit rows when seeding sensitive template grants", async () => {
    await seedHotSalesRbacCatalog("actor-seed");
