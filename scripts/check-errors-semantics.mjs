...
const CAPABILITY_USAGE_PATTERN =
	/\b(?:errorIngress|errorOpenApi|errorProject|errorResult|errorWire)\b/u;
const CANONICAL_TYPE_IMPORT_PATTERN =
	/import\s+type\s+\{[^}]*\b(?:CanonicalErrorCode|Failure|Result|ResultFailure)\b[^}]*\}\s+from\s+["']@afenda\/errors["']/su;
...
function analyzeSource({ relativePath, content }) {
	...
	const hasCanonicalEvidence =
		...
		CAPABILITY_USAGE_PATTERN.test(content) ||
		CANONICAL_TYPE_IMPORT_PATTERN.test(content) ||
...
export function formatHumanReport(report) {
	const lines = ["check-errors-semantics: repository report"];
	lines.push(
	...
	lines.push(
		report.summary.status === "ok"
			? "check-errors-semantics: ok"
			: "check-errors-semantics: FAIL",
	);
	return lines.join("\n");
}
...
export function runCli(argv = process.argv.slice(2), root = DEFAULT_ROOT) {
	...
	const typedGate = spawnSync(
		process.execPath,
		["packages/foundation/errors/scripts/check-semantics.ts"],
		{ cwd: root, encoding: "utf8" },
	);
	if (typedGate.status !== 0) {
		console.error("check-errors-semantics: typed capability analysis failed");
		console.error(typedGate.stderr || typedGate.stdout);
		return 1;
	}
