...
function writeText(root, relativeFile, contents) {
	const file = join(root, relativeFile);
	mkdirSync(join(file, ".."), { recursive: true });
...
describe("checkTsconfigGovernance", () => {
	...
	it("reads consumer tsconfigs as JSONC", () => {
		const root = createFixture();
		// tsconfig.json is JSONC in practice; several checked-in consumers document
		// their INV-6 `types` supersets with comments. A strict JSON read reports
		// those as parse failures and silently stops governing them.
		writeText(
			root,
	...
	"extends": "@afenda/config/tsconfig/node-library.json",
	"compilerOptions": {
		// arrays replace rather than merge across extends (INV-6)
		"types": ["node"],
	},
}
`,
	...
	it("still governs central options inside a commented tsconfig", () => {
		const root = createFixture();
		writeText(
			root,
			"packages/example/tsconfig.json",
			`{
	// deliberately reopens a centrally owned option
	"extends": "@afenda/config/tsconfig/node-library.json",
	"compilerOptions": { "strict": false }
}
`,
