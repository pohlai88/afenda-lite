...
describe("no-editor-biome-drift hook policy", () => {
	it("denies posture regressions", () => {
		assert.equal(runHook(denyPayload).permission, "deny");
	});

	it("allows the committed settings", () => {
		assert.equal(runHook(allowPayload).permission, "allow");
	});
});
