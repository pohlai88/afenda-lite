...
function fixture() {
	...
	writeFileSync(
		join(root, "packages/runtime/http/package.json"),
		JSON.stringify({ exports: { ".": "./src/index.ts" } }),
	);
	writeFileSync(
		join(root, "packages/runtime/http/src/index.ts"),
...
test("accepts the leaf HTTP capability", () => {
	const root = fixture();
	try {
		assert.deepEqual(checkHttpBoundary(root), []);
	} finally {
...
test("rejects workspace edges, subpaths, deep imports, and deleted facades", () => {
	...
	try {
		...
		writeFileSync(
			...
			'import { createCorrelationId } from "@afenda/http/internal";\n',
		);
		const violations = checkHttpBoundary(root);
		assert.ok(
			violations.some((value) => value.includes("only the root export")),
		);
		assert.ok(
		...
		assert.ok(
			violations.some((value) => value.includes("deep @afenda/http import")),
		);
		assert.ok(
			violations.some((value) => value.includes("deleted HTTP surface")),
		);
	} finally {
		rmSync(root, { recursive: true, force: true });
	}
