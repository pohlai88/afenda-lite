			...
			validateCommandsQueries([
				...
				baseManifest({
					...
					authorizationCommands: {
						"shared.cmd": "master_data.payment_term_manage",
					},
					permissions: ["master_data.payment_term_manage"],
				}),
			]),
		),
	);
		...
		expectFail(
			...
			validateCommandsQueries([
				...
				baseManifest({
					...
					packageName: "@afenda/master-data",
					queries: ["shared.q"],
					authorizationQueries: { "shared.q": "master_data.party_read" },
					permissions: ["master_data.party_read"],
				}),
			]),
		) ?? "",
	);
