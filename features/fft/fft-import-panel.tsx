...
import type { FftImportType } from "@/modules/fft/domain/import-types";
...
export function TradeImportNavLink({
  ...
  return (
    <Link
      href={fftHref(`/admin/events/${eventId}/imports`)}
...
export function FftImportPanel({
  locale,
  eventId,
  depositEnabled,
  pickupEnabled,
  ...
  const importOptions: ImportOption[] = [
    ...
    { value: "bulk_order", label: "Bulk orders" },
    {
      ...
      disabledReason: "Requires FFT_DEPOSIT_ENABLED",
    },
    {
      value: "pickup_confirmation",
      label: "Pickup confirmation",
      disabled: !pickupEnabled,
      disabledReason: "Requires FFT_PICKUP_OPS_ENABLED",
    },
  ];
  ...
  const [importType, setImportType] = useState<FftImportType>(firstEnabled);
      ...
      <div className="rounded-lg border p-4 space-y-4">
        <div className="space-y-2">
          ...
          <select
            ...
            value={importType}
            onChange={(e) => {
              setImportType(e.target.value as FftImportType);
              setDryRun(null);
              setError(null);
            }}
          >
          ...
          onSubmit={(e) => {
            ...
            startTransition(async () => {
              ...
              if (!result || !("ok" in result) || result.ok !== true) {
                setError(getFftActionError(result) ?? "Import dry-run failed");
                setDryRun(null);
                return;
              }
              setDryRun({
