...
import type { FftSyncJobDetailDto } from "@/modules/fft/domain/erp/types";
...
export function FftErpSyncNavLink({ locale }: { locale: FftLocale }) {
  return (
    <Link
      href={fftHref("/admin/erp-sync")}
...
export function FftErpSyncPanel({
  locale,
  jobs,
}: {
  locale: FftLocale;
  jobs: FftSyncJobDetailDto[];
}) {
...
function DlqDetail({ job }: { job: FftSyncJobDetailDto }) {
  return (
