...
import type { FftDepositListItem, FftFinanceAuditEntry } from "@/modules/fft/domain/types";
import { fftHref, type FftLocale } from "@/modules/fft/i18n/fft-i18n";

export function FftDepositPanel({
  locale,
  eventId,
  deposits,
  audit,
  ...
  locale: FftLocale;
  eventId: string;
  deposits: FftDepositListItem[];
  audit: FftFinanceAuditEntry[];
  canManage: boolean;
}) {
  return (
      ...
      <section className="space-y-3">
        ...
        {deposits.length === 0 ? (
          <p className="text-muted-foreground text-sm">
            {locale === "vi"
              ? "Chưa có bản ghi cọc. Bật FFT_DEPOSIT_ENABLED và đặt cọc bắt buộc trên sự kiện."
              : "No deposit records yet. Enable FFT_DEPOSIT_ENABLED and require deposit on the event."}
          </p>
        ) : (
              ...
              <li key={d.id} className="rounded-lg border p-4">
                ...
                {canManage ? (
                  <div className="mt-4 grid gap-4 lg:grid-cols-2">
                    ...
                    />
                    <FftDepositReceiptForm
                      locale={locale}
                      eventId={eventId}
                      depositId={d.id}
                      orderId={d.orderId}
                    />
                    <FftDepositAdjustmentForm
                      locale={locale}
                      eventId={eventId}
                      depositId={d.id}
                      orderId={d.orderId}
...
function FftDepositDetailsForm({
  locale,
  eventId,
  depositId,
  orderId,
...
function FftDepositReceiptForm({
  locale,
  eventId,
  depositId,
  orderId,
...
function FftDepositAdjustmentForm({
  locale,
  eventId,
  depositId,
  orderId,
...
export function FftDepositsNavLink({
  locale,
  eventId,
}: {
  locale: FftLocale;
  ...
  return (
    <Link
      href={fftHref(`/admin/events/${eventId}/deposits`)}
