...
import type { FftPickupListItem, FftPickupWindow } from "@/modules/fft/domain/types";
import { fftHref, type FftLocale } from "@/modules/fft/i18n/fft-i18n";

export function FftPickupNavLink({
  locale,
  eventId,
}: {
  locale: FftLocale;
  ...
  return (
    <Link
      href={fftHref(`/admin/events/${eventId}/pickup`)}
...
export function FftPickupPanel({
  locale,
  eventId,
  windows,
  queue,
  ...
  locale: FftLocale;
  eventId: string;
  windows: FftPickupWindow[];
  queue: FftPickupListItem[];
  canManage: boolean;
}) {
  return (
    <div className="space-y-8">
      {canManage ? (
        <section className="space-y-3">
          ...
          <FftPickupWindowForm locale={locale} eventId={eventId} />
...
function FftPickupExceptionForm({
  locale,
  eventId,
  orderId,
}: {
