...
| `modules/platform/schemas/action-result.ts` | Web Action adapters | `ActionResult` aliases root `@afenda/errors`; `actionFieldMessage` remains local (**Landed**) |
| `packages/foundation/errors` (`@afenda/errors`) | Transport-neutral error kernel | Root-only `errorResult` / `errorIngress` / `errorProject` / `errorWire` / `errorOpenApi` and canonical `Result` (**Landed**) |
