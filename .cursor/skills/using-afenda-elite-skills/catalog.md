...
| `afenda-elite-kernel` | local-elite-farm | local | keep | Deterministic reusable-kernel lifecycle: scaffold · apply · implement · semantic cutover · upgrade · verify · reopen · digest-scoped readiness seal | New reusable package capability · shared-concept centralization and final cutover · existing package expansion · `@afenda/errors` / `@afenda/env` / `@afenda/testing` integration or upgrade · sealed-target upgrade · kernel readiness evidence | AGENTS.md · owning farm · disk package contract · docs-V2/monorepo | Verified target owner and farm; frozen semantic owner and facade before cutover | `.cursor/skills/afenda-elite-kernel/SKILL.md` · `references/{lifecycle,seal-contract,errors-kernel,env-kernel,testing-kernel}.md` · `scripts/inspect-target.mjs` | Orchestrates existing farms; registered targets `@afenda/errors`, `@afenda/env`, and `@afenda/testing`; does not authorize a generic `@afenda/kernel`; seal ≠ Module Enterprise Readiness |
