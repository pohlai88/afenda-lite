...
| `afenda-erp-storybook-stories` | local-elite-farm | local | keep | Orchestra: CSF3 stories ↔ metadata contracts ↔ ContractDocs — ERP evidence levels; hierarchical Overviews; sync change-order; sparse status; title/scroll ownership | Upgrade/author ui-system stories · edit `metadata/contracts` · contract↔story sync · anti-gallery | ADR-010 · story-coverage · status-badge + button + card | vendor `storybook-story-writing` (CSF3) · `technical-writing` (internal-guide for clauses) | `.cursor/skills/afenda-erp-storybook-stories/SKILL.md` | Exemplars: status-badge/button (benchmark) · card (governed); product UI stays ui-compose; contracts never barrel-exported |
...
| `afenda-package-protection` | local-agent-ops | local | keep | Protected package pre-edit unlock and SHA-256 integrity digest | `protect:check` · intentional `protect:update` after owning gates | Package disk contract · owning farm/kernel evidence | Explicit local edit token for digest refresh | `.cursor/skills/afenda-package-protection/SKILL.md` · protected package scripts | Integrity mechanism only; does not define semantics, verification, readiness, or seal authority |
...
| `afenda-elite-kernel` | local-elite-farm | local | keep | Deterministic reusable-kernel lifecycle: scaffold · apply · implement · semantic cutover · upgrade · verify · reopen · digest-scoped readiness seal | Shared-concept centralization and final cutover · existing package expansion · registered config / errors / env / testing / db / audit / events / search / notifications / logger / HTTP / security / metrics / OpenAPI / rate-limit / cache / auth / admin integration or upgrade · sealed-target upgrade · kernel readiness evidence | AGENTS.md · owning farm · disk package contract · docs-V2/monorepo | Verified target owner and farm; frozen semantic owner and facade before cutover | `.cursor/skills/afenda-elite-kernel/SKILL.md` · `references/{lifecycle,seal-contract,config-kernel,errors-kernel,env-kernel,testing-kernel,db-kernel,audit-kernel,events-kernel,search-kernel,notifications-kernel,logger-kernel,http-kernel,security-kernel,metrics-kernel,openapi-kernel,rate-limit-kernel,cache-kernel,auth-kernel,admin-kernel}.md` · `scripts/inspect-target.mjs` | Orchestrates registered package contracts; no generic `@afenda/kernel`; seal ≠ Module Enterprise Readiness |
...
| `afenda-elite-payroll` | local-domain-farm | local | keep | `@afenda/payroll` package shape — domain farms, schemas/, store/, drizzle/memory adapters · boundaries · domain invariants · testing · security · phased workflow | Payroll commands · schemas · store · adapters · payroll_* sole mutator · architect planning · verifier review | Scratch docs-V2/_scratch/erp/human-resource.md · docs-V2/_scratch/payroll-cursor-agent-pack · package disk · SCAFFOLDING.md · payroll-*.mdc rules | Package exists under `packages/erp/payroll` | `.cursor/skills/afenda-elite-payroll/SKILL.md` · package-tree.md · implementation.md · boundaries.md · domain.md · testing.md · security.md · workflow.md · `.cursor/agents/payroll-architect.md` · `.cursor/agents/payroll-verifier.md` | Structural mirror of HR; no peer `@afenda/human-resources` import; subagents architect (Plan) + verifier (phase gate); Scratch pack progressive only |
| `afenda-elite-corporate-administration` | local-domain-farm | local | keep | `@afenda/corporate-administration` statutory and entity-management requirements · feature-first layout · composed runtime · ca_* sole mutator · Investor Relations exclusion · fourteen-boundary acceptance | CA commands · schemas · store · migrations · adapters · legal-company · governance · authority · entity operations · ca_* | `packages/erp/corporate-administration/PRD.md` · package disk · apps/web CA actions/features | Package exists under `packages/erp/corporate-administration` | `.cursor/skills/afenda-elite-corporate-administration/SKILL.md` · reference.md · verification.md | Requirements-driven; one requirement group per mission; Investor Relations owns securities/capital/investors/ownership/distributions |
