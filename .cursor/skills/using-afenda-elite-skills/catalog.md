...
| `afenda-elite-ui-compose` | local-elite-farm | local | keep | Product UI consistency lock — QUALITY ORDER (incl. SCALABILITY-FIRST), barrel compose, lock tables, anti-handroll, hard rule 15 / UI-CAP-* findings; F* + C* Vitest + verification matrix | Compose product UI · fix handroll · visual consistency · capability gaps | ADR-010 · ARCH-024 · tokens.css · apps/web Geist map | none | `.cursor/skills/afenda-elite-ui-compose/SKILL.md` | Done = capability check + verification matrix (floor `pnpm check:ui-system` = F*+C*+package/web tests; build when matrix requires RSC/structural/CSS-font); no local compensation for reusable gaps; load before frontend-ui-engineering; companion `reference.md` gate + UI-CAP SSOT |
