...
**LOAD `afenda-elite-ui-compose` before Design System / Typography / Color sections** in this skill. Those sections defer to compose locked tables (type, spacing, radius, semantic color, density, anti-handroll) and SCALABILITY-FIRST / hard rule 15 (no local compensation for missing reusable barrel capability — `UI-CAP-*`) — do not invent parallel scales or generic tokens (`text-muted`, `bg-surface`) that are not in Afenda `tokens.css`. After compose, use this skill for a11y, state, and responsive method only.
