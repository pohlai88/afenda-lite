...
| Shared | `utils.ts`, `format.ts`, `breakpoints.ts`, `pagination-range.ts`, `form-constraints.ts`, `clipboard.ts`, `app-url.ts`, `audit.ts`, `observability.ts`, `playground-embed.ts` |
...
|------|-------|
| Neon Auth | `auth/*` |
| Schemas | `schemas/auth.ts`, `schemas/users.ts` (`UserId`, set-role / ban) |
...
## Gone (do not recreate under `lib/`)

`lib/domain/`, `lib/schemas/`, `lib/env/`, `lib/routing/`, `lib/auth/`, `lib/copy/`, `lib/utils.ts`, `lib/format.ts`

Residue / runners: [residue-inventory.md](residue-inventory.md) (Pass 2 complete — `lib/` is `entry` / `pages` / `playground` only).
