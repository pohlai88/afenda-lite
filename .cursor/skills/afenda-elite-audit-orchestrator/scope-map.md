...
| `platform-rank1` / `http` / `metrics` / `audit` / `search` / `notifications` / `events` / `master-data` / `security` / `openapi` | router → monorepo-discipline → api-contract (when RH / OAS) | docs-V2/monorepo · LAYERS.md · docs-V2/{observability,search,notifications,events,master-data,api} | package READMEs · DNA packs · api-contract `openapi.md` | per-package `test` + `typecheck`; `pnpm openapi:generate` + `check:openapi` when OpenAPI surface touched; master-data also `audit:tenancy-nulls` |
