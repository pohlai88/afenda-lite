...
import { ensureTradeAllowlistForOperator } from "@/testing/e2e/trade-allowlist";
...
test.describe("Feed Farm Trade core cycle @journey", () => {
  ...
  test("order → allocate → transfer → complete (G3/G4)", async ({ page }) => {
    ...
    await page.goto("/trade/my-orders");
  ...
  test("audit + export + clone (G6/G7/G8)", async ({ page }) => {
    ...
    await expect(page).toHaveURL(
      new RegExp(`/trade/admin/events/(?!${sourceEventId})[^/]+/setup`),
      { timeout: 30_000 },
    );
