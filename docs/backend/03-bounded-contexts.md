...
**Platform model:** one **Afenda-Lite** SaaS · **two product modules** (Declarations + Trade / Feed Farm Trade) on shared Platform + Identity. Module boundaries are domain trees and entitlements — **not** separate apps or infra lanes. Platform/infra changes (env, DB pool, auth, shell, proxy, CI, Vercel) apply to **both modules together**. Product name: [docs/adr/001-afenda-lite-product-identity.md](../adr/001-afenda-lite-product-identity.md).
