...
- Spectral / `npm run check:openapi` non-zero
...
- Repo checkout with composed env (`npm run env:compose`) when handlers need DB
...
| ------ | -------------- |
| `npm run check:openapi` fails: missing YAML | Generator never run or artifact deleted |
