# ADR-009 Feed Farm Trade ERP Sync
...
| **Accepted** | 2026-07-10 |
| **Date** | 2026-07-10 |
| **Owners** | Feed Farm Trade / Trade · Integrations |
| **Scope** | Idempotent sync between Feed Farm Trade domain and external ERP/finance system |
| **Out of scope** | Choosing a specific ERP vendor in code without integration spec; replacing 2B operational deposits |

**Related:** [002-finance-deposit-pickup-ops.md](./ADR-007-feed-farm-trade-finance-deposit-pickup-ops.md) · [../spec/phase-2a-prd.md](../spec/GUIDE-016-feed-farm-trade-phase-2a-prd.md) §7 · [./phase-2bcd-slices.md](../spec/GUIDE-018-feed-farm-trade-phase-2b-2d-slices.md)
...
Feed Farm Trade will accumulate authoritative **operational** records (orders, allocation, deposits, pickup) while **official settlement** remains in ERP/Finance (ADR-002). Phase 2D bridges the gap without making Feed Farm Trade the ledger.

Prerequisites:

- Stable `fft_order.id`, event IDs, customer keys
...
| Principle | Rule |
|-----------|------|
| **Feed Farm Trade availability** | Sync failure must **not** block local order/allocation/pickup ops unless explicitly configured per entity type |
| **Idempotency** | Every outbound message has `idempotency_key`; ERP adapter dedupes |
| **Direction (default)** | **Push** Feed Farm Trade → ERP for orders/fulfillment/deposit summaries; **pull** optional for customer/product masters (per integration pack) |
| **Finance SoT** | ERP remains settlement SoT; Feed Farm Trade may receive **status callbacks** (e.g. deposit_settled) as read-only updates |
| **Staging** | No direct ERP write from HTTP request — async job queue |

### 4.2 Entity mapping

```text
fft_external_mapping  — entity_type, local_id, external_system, external_id, last_synced_at
fft_sync_job          — job_type, payload_ref, status, scheduled_at
fft_sync_attempt      — job_id, attempt_no, started_at, finished_at, status
fft_sync_error        — attempt_id, code, message, retryable, raw_payload_ref
```
...
External system identifier: `erp_system` enum (e.g. `generic`, `customer_acme_erp`) — **no hardcoded vendor SDK** in core domain; adapter interface in `modules/fft/domain/erp/`.
...
5. Env flag `FFT_ERP_SYNC_ENABLED` (default false)
...
| ERP deposit status disagrees with Feed Farm Trade ops deposit | **ERP callback wins** for settlement flag only; ops deposit record retained with `erp_status` field |
...
```text
modules/fft/domain/erp/types.ts      — ErpAdapter, PushResult, PullResult
modules/fft/domain/erp/generic-noop.ts — default no-op when sync disabled
modules/fft/domain/erp/<vendor>/     — optional vendor pack (not in initial 2D-1 slice)
```
...
| Feed Farm Trade pulls settlement authority from ERP into order row only | Loses audit; ADR-002 projection model cleaner |
...
- [ ] FFT_ERP_SYNC_ENABLED default false
...
Set `FFT_ERP_SYNC_ENABLED=false`; stop job runner. Mappings retained. Feed Farm Trade continues as operational SoT per ADR-002.
