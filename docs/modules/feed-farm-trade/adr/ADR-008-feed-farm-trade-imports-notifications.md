# ADR-008 Feed Farm Trade Imports Notifications
...
| **Scope** | Structured Excel import pipeline + Feed Farm Trade transactional email |
| **Out of scope** | Declaration-portal mail, Neon Auth mail, Zalo/WhatsApp/SMS/Teams (defer) |

**Related:** [../spec/phase-2a-prd.md](../spec/GUIDE-016-feed-farm-trade-phase-2a-prd.md) §6 · [001-rbac.md](./ADR-006-feed-farm-trade-rbac.md) · [002-finance-deposit-pickup-ops.md](./ADR-007-feed-farm-trade-finance-deposit-pickup-ops.md) · [./phase-2bcd-slices.md](../spec/GUIDE-018-feed-farm-trade-phase-2b-2d-slices.md)
...
Separately, Feed Farm Trade needs event/order notifications (allocation, transfer, deposit, pickup) without coupling to:
...
```text
fft_import_batch   — type, filename, status, actor, counts, committed_at
fft_import_row     — row_number, payload_json, validation_errors, write_status
```
...
**Decision:** Feed Farm Trade notifications use a **dedicated transactional email lane**, isolated from portal mail.

| Concern | Choice |
|---------|--------|
| Provider | App-owned API key via env (`FFT_EMAIL_*` or shared `RESEND_API_KEY` with distinct from-address) |
| From address | Product-controlled (e.g. `fft@<APP_DOMAIN>` or verified sender) — **not** `auth@mail.myneon.app` |
| Neon Auth | **Do not use** for Feed Farm Trade business events |
| Declaration invites | Unchanged — Neon org invitations only |

**Tables:**

```text
fft_notification_template  — event_key, locale, subject, body_markdown
fft_notification_event     — enable flag per event_key + channel
fft_notification_delivery  — idempotency_key, status, provider_id, error, sent_at
```
...
5. Env flag `FFT_NOTIFICATIONS_ENABLED` (default false until 2C slice acceptance).
...
|-------------|--------------|
| Reuse Neon Auth mail for Feed Farm Trade | Violates portal auth boundary; wrong sender trust model |
...
- [ ] No Neon Auth provider for Feed Farm Trade notifications
...
- [ ] FFT_NOTIFICATIONS_ENABLED gates outbound mail
```
...
Disable `FFT_NOTIFICATIONS_ENABLED`; imports can remain read-only/dry-run off. Retain tables; stop scheduler/worker if added later.
