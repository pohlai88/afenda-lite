...
| **Out of scope** | Finance settlement SoT, pickup ops schema, ERP sync, notification provider, declaration-portal IAM; **platform** hard tenancy / `fft.access` entry (see [ADR-002](../../../adr/backend/ADR-002-platform-tenancy-rbac.md) · [ARCH-023](../../../architecture/turborepo/ARCH-023-multi-tenancy.md)) |

**Related:** [../archive/phase-2-feedback.md](../archive/GUIDE-020-feed-farm-trade-phase-2-feedback-archive.md) (authoritative planning) · [../spec/phase-2a-prd.md](../spec/GUIDE-016-feed-farm-trade-phase-2a-prd.md) · [../spec/phase-1-prd.md](../spec/GUIDE-015-feed-farm-trade-phase-1-prd.md) (Phase 1 accepted @ `1bc1294` / `fft-phase-1`) · [../archive/phase-2-scoping.md](../archive/GUIDE-021-feed-farm-trade-phase-2-scoping-archive.md) (candidate list; 7-role framing superseded)

**Gate:** This ADR, [../spec/phase-2a-prd.md](../spec/GUIDE-016-feed-farm-trade-phase-2a-prd.md), and [../spec/phase-2a-slices.md](../spec/GUIDE-017-feed-farm-trade-phase-2a-slices.md) are **Accepted / closed**. **Operational rollout** — [../ops/gate-register.md](../ops/RB-002-feed-farm-trade-gate-register.md). Do not start 2B–2D or reopen 2A product scope.
...
Phase 1 Feed Farm Trade uses:

- **Admin** — portal `isAdminSession`
- **Sales** — email allowlist (`fft_sales_member`) with own-orders visibility
...
Adopt a **permission-catalog RBAC** model for Feed Farm Trade:
...
6. **Phase 1 compatibility:** Until cutover completes, existing Admin + Sales allowlist behavior must continue to work (bootstrap mapping or dual-read). Rollback reference remains tag `fft-phase-1`.
7. **Portal operator admin** (`isAdminSession`) maps to a platform-scoped Super Admin / equivalent template — not a second parallel auth system inside Feed Farm Trade.
...
|---------------|------------------------|
| Super Admin | Platform / full Feed Farm Trade control |
...
```text
fft_permission          — catalog (code PK, description, sensitivity)
fft_role                — client-owned role (name, active, is_template_seed?)
fft_role_permission     — role ↔ permission
fft_role_assignment     — user ↔ role ↔ scope_type ↔ scope_id
fft_rbac_audit          — role/permission/assignment changes
```

Exact columns, Neon Auth user id linkage, and migration from `fft_sales_member` are deferred to approved 2A slices.
...
| **Neon Auth org roles only** | Useful for membership; Feed Farm Trade needs event/team/BU scopes and a permission catalog owned by the product |
...
- Aligns with reusable Feed Farm Trade engine (not one-company hardcoding)
...
- [ADR-002](./ADR-007-feed-farm-trade-finance-deposit-pickup-ops.md) finance + pickup ops — **Accepted** 2026-07-10
- [ADR-003](./ADR-008-feed-farm-trade-imports-notifications.md) notifications provider — **Accepted** 2026-07-10
- [ADR-004](./ADR-009-feed-farm-trade-erp-sync.md) ERP sync contract — **Accepted** 2026-07-10
- Implementation: [../spec/phase-2bcd-slices.md](../spec/GUIDE-018-feed-farm-trade-phase-2b-2d-slices.md) (**Proposed**)
...
If 2A RBAC is abandoned after partial work: keep additive tables unused; continue Phase 1 `isAdminSession` + sales allowlist; baseline remains `fft-phase-1`.
