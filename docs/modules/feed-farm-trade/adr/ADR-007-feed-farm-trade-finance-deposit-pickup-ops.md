# ADR-007 Feed Farm Trade Finance Deposit Pickup Ops
...
| **Scope** | Operational deposit tracking + pickup/fulfillment workflow under `/fft` |
| **Out of scope** | Payment settlement engine, ERP sync (ADR-004), Excel import (2C), notification delivery (ADR-003) |

**Related:** [../spec/phase-2a-prd.md](../spec/GUIDE-016-feed-farm-trade-phase-2a-prd.md) §5 · [../archive/phase-2-feedback.md](../archive/GUIDE-020-feed-farm-trade-phase-2-feedback-archive.md) · [001-rbac.md](./ADR-006-feed-farm-trade-rbac.md) · [./phase-2bcd-slices.md](../spec/GUIDE-018-feed-farm-trade-phase-2b-2d-slices.md)

**Gate:** ADR accepted — **no schema or application code** until [../spec/phase-2bcd-slices.md](../spec/GUIDE-018-feed-farm-trade-phase-2b-2d-slices.md) is **Approved** per phase (2B-1 … 2B-4).

---

## Context

Phase 1 stores a lightweight `fft_order.deposit_status` (`not_required | pending | paid | waived`) and `fulfilled_quantity` for pickup support. That is insufficient for:
...
| Layer | SoT | Rule |
|-------|-----|------|
| **Official settlement** | External ERP/Finance (future ADR-004) | Feed Farm Trade does **not** become payment settlement SoT in 2B |
| **Operational deposits** | Feed Farm Trade `fft_deposit*` tables | Staff record operational deposit state for sales workflow |
| **Order summary** | `fft_order.deposit_status` | **Projection only** — derived from deposit records, not authoritative |
...
```text
fft_deposit           — order-linked deposit record (amount, currency, due date, status)
fft_deposit_receipt   — receipt reference, paid_at, recorded_by
fft_deposit_adjustment — waive / refund / forfeit / correction (reason required)
fft_finance_audit     — append-only finance mutation log
```
...
```text
fft_pickup_window      — event-scoped pickup slots
fft_pickup_assignment  — order/customer ↔ window
fft_fulfillment_record — qty confirmed, actor, timestamp
fft_pickup_exception   — no-show, partial, cancel, override (reason + audit)
```
...
|-------------|--------------|
| Feed Farm Trade as finance settlement SoT | Scope creep; conflicts with ERP; regulatory/compliance risk |
...
- [ADR-003](./ADR-008-feed-farm-trade-imports-notifications.md) — deposit/pickup notification triggers
- [ADR-004](./ADR-009-feed-farm-trade-erp-sync.md) — deposit master sync when ERP contract exists
...
Feature-flag `FFT_DEPOSIT_ENABLED` / `FFT_PICKUP_OPS_ENABLED` (env, default false until slice acceptance). If rolled back: stop writes to new tables; continue Phase 1 `deposit_status` + `fulfilled_quantity` manual path; tables remain unused.
