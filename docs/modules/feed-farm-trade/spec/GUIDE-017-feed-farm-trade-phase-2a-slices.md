# GUIDE-017 Feed Farm Trade Phase 2A Slices
...
| **Approved** | 2026-07-09 |
| **Date** | 2026-07-09 |
| **Contract** | [./phase-2a-prd.md](./GUIDE-016-feed-farm-trade-phase-2a-prd.md) (**Accepted**) |
| **RBAC ADR** | [../adr/001-rbac.md](../adr/ADR-006-feed-farm-trade-rbac.md) (**Accepted**) |
| **Phase 1 baseline** | `1bc1294` · tag `fft-phase-1` |

### Gate

This slice plan is **Approved** and **implementation closed** (tag `fft-phase-2a` → `8e650ff`). **Active work:** operational rollout — [../ops/gate-register.md](../ops/RB-002-feed-farm-trade-gate-register.md). Do **not** start Phase 2B, 2C, or 2D or reopen 2A product scope.

---

## Execution clarifications (binding)

1. **RBAC cutover feature flag** — `FFT_RBAC_ENABLED` (default unset/`false` = Phase 1 Admin + sales allowlist only). When `true`, dual-read: allowlist **or** RBAC assignment. Rollback = set flag false (or unset); no need to drop tables.
...
3. **Sensitive permissions** — Codes marked sensitive (e.g. `allocation.override`, `role.manage`, `deposit.manage`) require **explicit** grant on a role; granting/revoking them **must** write `fft_rbac_audit`.
...
+ dedicated /fft/[locale]/admin/events/new
```
...
2A-6  user role assignment UI
  ↓
2A-7  apply guards to existing /fft flows
  ↓
...
| | |
|--|--|
| **Goal** | Map Admin + allowlist; `FFT_RBAC_ENABLED` switch |
...
| | |
|--|--|
| **Goal** | Evidence + closure note; optional tag `fft-phase-2a` |
...
- `FFT_RBAC_ENABLED` — rollback switch (default off = Phase 1 path)
...
- **Next lane:** [../ops/release-readiness.md](../ops/RB-003-feed-farm-trade-release-readiness.md) — operational packaging  
- **Ops tracker:** [../ops/rollout.md](../ops/RB-004-feed-farm-trade-ops-rollout.md) — rollout checklist  
- **Gate SSOT:** [../ops/gate-register.md](../ops/RB-002-feed-farm-trade-gate-register.md) — **active**; Gates 1–7 closed; stop drift
...
- Commits: Feed Farm Trade 2A only — **no** layout/repo-migration WIP
- Preserve `fft-phase-1` rollback reference
...
- Dropping `fft_sales_member` in the first 2A cut (dual-read first)  
- Neon Auth org-role redesign outside Feed Farm Trade tables  
...
```text
Status: Approved (2026-07-09) · Implementation closed (tag fft-phase-2a @ 8e650ff)
