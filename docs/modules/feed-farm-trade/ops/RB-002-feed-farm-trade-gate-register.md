...
**Skim banner:** Gates 1–6 historical evidence may show `FFT_RBAC_ENABLED=false`. **Current production** is Gate 7 — `FFT_RBAC_ENABLED=true`. Platform hard tenancy (ADR-002 / `027`) is separate from this FFT domain flag — see [ARCH-023](../../../architecture/turborepo/ARCH-023-multi-tenancy.md).
...
**Phase 2A ops rollout:** Gates **1–7** ✅ complete (2026-07-10). Production runtime: `br-tiny-hill-ao82jp6f` / `ep-dawn-bird`, `FFT_RBAC_ENABLED=true` on Vercel. **2B–2D:** ADRs Accepted; implementation gated on [../spec/phase-2bcd-slices.md](../spec/GUIDE-018-feed-farm-trade-phase-2b-2d-slices.md) approval.
