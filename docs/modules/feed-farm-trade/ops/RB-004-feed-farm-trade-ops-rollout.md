# RB-004 Feed Farm Trade Ops Rollout
...
| **Readiness docs** | Commit `0fd22f4` · [./release-readiness.md](./RB-003-feed-farm-trade-release-readiness.md) |
| **Gate SSOT** | [./gate-register.md](./RB-002-feed-farm-trade-gate-register.md) — **read first** |

## Scope statement

This tracker covers **operational rollout** of Phase 2A RBAC after the product boundary tag `fft-phase-2a`.

**Gate status and drift rules:** [./gate-register.md](./RB-002-feed-farm-trade-gate-register.md) (canonical).
...
- [x] Product boundary confirmed: `fft-phase-2a` → `8e650ff`
- [x] Readiness docs confirmed: `0fd22f4`
- [x] [./release-readiness.md](./RB-003-feed-farm-trade-release-readiness.md) reviewed
- [x] Cross-links verified from slices / PRD-V2 / feedback / S19

## Migration

- [x] Apply `014_fft_rbac.sql`
- [x] Confirm migration applied successfully
- [x] Confirm app still runs with `FFT_RBAC_ENABLED=false`
...
- [x] Admin matrix passed (Gate 4 admin) — see [gate register](./RB-002-feed-farm-trade-gate-register.md)
- [x] **Gate 4B:** Sales allowlist matrix — **closed as data/setup** (see [gate register](./RB-002-feed-farm-trade-gate-register.md#gate-4b--sales-allowlist-matrix-closed--datasetup))
- [x] Confirm legacy/Phase 1 admin path still works (Gate 4 admin)
- [x] Confirm no RBAC-only behavior is active (production `FFT_RBAC_ENABLED=false`)

### Gate 4B — sales allowlist (flag off)

See [./gate-register.md § Gate 4B](./RB-002-feed-farm-trade-gate-register.md#gate-4b--sales-allowlist-matrix-active-work).
...
- [x] Rows 6–10 re-run (2026-07-10) — **5/5 pass** — see [gate register](./RB-002-feed-farm-trade-gate-register.md#production-evidence-2026-07-09-data-setup--2026-07-10-matrix)
...
- [x] Confirm sensitive grant seed/update writes `fft_rbac_audit`

## Pre-enable matrix

Verify **UI and action-level** access (UI hiding alone is not enough):

- [x] `/fft/[locale]/admin/rbac` loads for intended admin
- [x] `/fft/[locale]/admin/rbac` denies unauthorized user
- [x] `/fft/[locale]/admin/events/new` loads for authorized user
...
- [x] Set `FFT_RBAC_ENABLED=true` (local controlled run only)
...
- [x] Capture evidence — [gate register § Gate 6](./RB-002-feed-farm-trade-gate-register.md#gate-6--controlled-rbac-matrix-local-only); code under test `51e9a5b`; register closeout `25c3891`; local `.env` restored to `FFT_RBAC_ENABLED=false`

## Production enable

- [x] Confirm controlled environment evidence is clean (Gate 6)
- [x] Enable production flag — **done** (Gate 7 · 2026-07-10 · `FFT_RBAC_ENABLED=true` on Vercel)
...
Post-tag hotfix `4d203a7` (FftShell next-intl) must be on `main` before the next production deploy — see [gate register](./RB-002-feed-farm-trade-gate-register.md#hotfix-merge-requirement).
...
**Production RBAC:** ✅ `FFT_RBAC_ENABLED=true`; compact smoke **17/17 PASS**.  
Evidence: [gate register § Gate 7](./RB-002-feed-farm-trade-gate-register.md#gate-7--production-rbac-enable-complete).

**Rollback:** `FFT_RBAC_ENABLED=false` → `env:compose` → `sync:vercel` → `vercel deploy --prod`.
