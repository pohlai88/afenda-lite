# GUIDE-020 Feed Farm Trade Phase 2 Feedback Archive
...
**Phase 1 baseline:** commit `1bc1294` · tag `fft-phase-1`  
**Candidate-list note (superseded for sequencing / 7-role framing):** [./phase-2-scoping.md](./GUIDE-021-feed-farm-trade-phase-2-scoping-archive.md)

**Phase 2A implementation closed** at tag `fft-phase-2a` → `8e650ff`. **Active:** [../ops/gate-register.md](../ops/RB-002-feed-farm-trade-gate-register.md) (ops rollout only; Gates 1–7 closed). Checklists: [../ops/release-readiness.md](../ops/RB-003-feed-farm-trade-release-readiness.md) · [../ops/rollout.md](../ops/RB-004-feed-farm-trade-ops-rollout.md). Do not start 2B–2D.

1. ~~[../adr/001-rbac.md](../adr/ADR-006-feed-farm-trade-rbac.md)~~ Accepted  
2. ~~[../spec/phase-2a-prd.md](../spec/GUIDE-016-feed-farm-trade-phase-2a-prd.md)~~ Accepted  
3. ~~[../spec/phase-2a-slices.md](../spec/GUIDE-017-feed-farm-trade-phase-2a-slices.md)~~ Approved  
...
| ------------------------------------------------------ | ------------------------- |
| Is Feed Farm Trade the source of truth for deposits?         | Yes / No                  |
| Or is ERP/Finance system the source of truth?          | Yes / No                  |
| Should Feed Farm Trade only show operational deposit status? | Likely yes until ERP sync |
| Should payment settlement happen inside Feed Farm Trade?     | Probably no for now       |

My recommendation:

```text
Until ERP sync is approved, Feed Farm Trade deposit records should be operational records only.
...
```text
fft_pickup_window
fft_pickup_assignment
fft_fulfillment_record
fft_pickup_exception
```
...
Connect Feed Farm Trade to ERP only after internal role, finance, pickup, and state models are stable.
...
| Customer master    | ERP source or Feed Farm Trade event input? |
| Product master     | ERP source or event-specific setup?  |
| Inventory/supply   | ERP, Ops, or manual confirmation?    |
| Finance settlement | ERP source or Feed Farm Trade source?      |
...
- Sync does not block local Feed Farm Trade operations unless configured.
```
...
```text
Phase 2 must not break existing Phase 1 events, orders, allocation, exports, or tag fft-phase-1 rollback reference.
```
...
Do not couple Feed Farm Trade Phase 2 with:
...
```text
docs/modules/feed-farm-trade/../adr/001-rbac.md
docs/modules/feed-farm-trade/../spec/phase-2a-prd.md
```

## Recommended additional ADRs

```text
docs/modules/feed-farm-trade/adr/ADR-007-feed-farm-trade-finance-deposit-pickup-ops.md
docs/modules/feed-farm-trade/adr/ADR-008-feed-farm-trade-imports-notifications.md
docs/modules/feed-farm-trade/adr/ADR-009-feed-farm-trade-erp-sync.md
docs/modules/feed-farm-trade/spec/GUIDE-018-feed-farm-trade-phase-2b-2d-slices.md
```

## PRD-V2-Phase2 structure

```text
# Feed Farm Trade — PRD-V2 Phase 2
...
1. docs/modules/feed-farm-trade/../adr/001-rbac.md
2. docs/modules/feed-farm-trade/../spec/phase-2a-prd.md

Also include these cross-cutting rules:
- Preserve Phase 1 baseline at tag fft-phase-1.
- Keep piglet as template data only.
- Use additive migrations where possible.
- Do not couple Feed Farm Trade Phase 2 with layout migration or declaration-domain refactors.
