...
**Honest debt (D0).** The handoff contract carries consumed leave *requests*, not a closing leave balance at termination: `packages/data-plane/events/src/schemas/hr-payroll-handoff.ts` has no balance-at-termination field. The balance therefore arrives as an explicit initiate input that Payroll encashes verbatim, which keeps the D4 rule intact (leave balance is an HR fact, never a Payroll computation) but leaves the delivery path un-sealed until D0 fact-widening lands. Until then the balance is caller-asserted rather than handoff-pinned — unlike compensation, which is pinned. This is the only un-pinned input in the settlement and must be closed with D0.
