...
**CLOSED 2026-08-05.** `final-settlement` initiates from the HR termination fact carried on the accepted workforce handoff (`employmentStatus ∈ {notice, terminated}`), and **pins** that handoff's compensation — base amount, currency, scale, rounding, pay frequency, source versions — into a hashed snapshot at initiate. Calculation prices only from that snapshot, so a compensation revision that supersedes the handoff afterwards cannot restate a settlement (same discipline as D3). Caller input is limited to non-statutory facts: the HR-delivered leave-balance days (encashed verbatim — Payroll never derives a balance), notice / in-lieu, and recoveries. **Statutory treatment routes through the same fail-closed calculator seam payroll runs use** (`PayrollStatutoryCapability.requireCalculator` + `isProductionApproved` over the pay group's active statutory rules): absent the capability, or with any rule whose calculator is unregistered or not production-approved, calculate refuses with `CONFLICT`. Today's only calculator is `synthetic_only`, so production activation fails closed — a settlement can never assert its own statutory amounts. C6: if the origin run is already `calculating`/`calculated`/`finalized`, or the period is `closed`, initiate records `clearance_required` and does not mutate the origin run; calculate refuses until an explicit `clearanceReason` is supplied. Finalize enforces calculate-actor ≠ finalize-actor (C9). The terminal statement is a **query**, not a review command, and preserves the payslip subject split: `payroll.payslip.read-own` discloses only the actor's own settlement, `payroll.payslip.read-all` any subject's; it is derived from sealed lines and totals rather than stored. Production migrate remains ops-gated (`0053_payroll_final_settlement`).

**D0 debt closed 2026-08-06.** `hr-payroll-handoff.ts` now carries `leaveBalanceAtTermination`, and `pinCompensation` refuses (`CONFLICT`) when the accepted handoff omits it, so the closing balance is handoff-pinned exactly like compensation — no caller-asserted input remains in the settlement. HR derives the balance with the Leave capability's own exact arithmetic (`addLeaveQuantity`), never a handoff-local decimal routine. The request fingerprint carries caller-supplied fields only: the vestigial `leaveBalanceDays: "0"` placeholder is gone.
...
| 10 | D0 fact widening | D0 | ✅ **Stages 1–3 closed 2026-08-06** — HR capture (`hr_statutory_profile` + `hr_prior_employer_ytd`, migration `0056_hr_statutory_profile`, ops-gated); handoff widening (`statutoryProfile` / `priorEmployerYtd` / `leaveBalanceAtTermination`, absent optionals omitted so `payloadHash` is stable across the widening); payroll YTD port owns the single hire-year merge and is **fail-closed** — an absent capability refuses with `CONFLICT` instead of zero-filling. Caveats: assemble is **restriction-gated** (a restriction-active subject fails `CONFLICT`, so payroll cannot ingest a handoff whose statutory facts were withheld), and the history YTD read is unbatched (`ytd-fanout-unbatched` in the payroll architecture-debt fixture) |
...
Remaining critical path: **A2 sourcing (longest lead) → Phase E.** Phases B, C, D0, and D3–D5 are closed.
