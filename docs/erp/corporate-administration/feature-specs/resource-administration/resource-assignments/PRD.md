# Product Requirements Document: Resource Assignments

> **Document purpose.**
> This PRD defines the `resource-administration/resource-assignments` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-RESOURCE-RESOURCE_ASSIGNMENTS-001
feature_id: resource-assignments
feature_name: Resource Assignments
...
feature_source_path: src/features/resource-administration/resource-assignments
prd_path: docs/erp/corporate-administration/feature-specs/resource-administration/resource-assignments/PRD.md
...
required_ports: []
emitted_events:
  - ResourceAssignmentPlanned
  - ResourceIssued
  - ResourceAssignmentExtended
  - ResourceReturned
  - ResourceReturnMarkedOverdue
  - ResourceAssignmentCancelled
consumed_by: []
estimated_slices: 11
```
...
| Field | Value |
| --- | --- |
| Feature name | Resource Assignments |
| Feature identifier | `resource-assignments` |
| Feature group | `resource-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for allocation and return of devices, lines, equipment, and administrative resources |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for resource assignments |
...
> Allocation and return of devices, lines, equipment, and administrative resources.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Resource Assignments enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for resource assignments when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `ResourceAssignment, AssignmentAcknowledgement`.
...
| administrative asset or access resource | resource-administration | opaque resource identifier | same organization |
...
| --- | --- | --- |
| `resource-assignments` | Allocation and return of devices, lines, equipment, and administrative resources | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `resource-assignments.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `ResourceAssignment` | aggregate | `ResourceAssignment` | Allocation and return of an administrative resource |
| `AssignmentAcknowledgement` | entity | `ResourceAssignment` | Issue and return acknowledgement |
...
| `overdue-return` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `plan` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.resource-assignments.manage` | no |
| TR-02 | Current permitted state | `issue` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.resource-assignments.manage` | no |
| TR-03 | Current permitted state | `extend` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.resource-assignments.manage` | no |
| TR-04 | Current permitted state | `record-return` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.resource-assignments.manage` | no |
| TR-05 | Current permitted state | `mark-overdue` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.resource-assignments.manage` | no |
| TR-06 | Current permitted state | `cancel` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.resource-assignments.manage` | no |
...
Feature invariants use `CA-RESOURCE_ASSIGNMENTS-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.resource-administration.resource-assignments.plan` | `plan` | command | `PlanResourceAssignmentInput` | `ResourceAssignmentView` | `ca.resource-assignments.manage` | no | required | request ID | yes | `ResourceAssignmentPlanned` |
| `ca.resource-administration.resource-assignments.issue` | `issue` | command | `IssueResourceAssignmentInput` | `ResourceAssignmentView` | `ca.resource-assignments.manage` | no | required | request ID | yes | `ResourceAssignmentPlanned` |
| `ca.resource-administration.resource-assignments.extend` | `extend` | command | `ExtendResourceAssignmentInput` | `ResourceAssignmentView` | `ca.resource-assignments.manage` | no | required | request ID | yes | `ResourceAssignmentPlanned` |
| `ca.resource-administration.resource-assignments.record-return` | `record_return` | command | `RecordReturnResourceAssignmentInput` | `ResourceAssignmentView` | `ca.resource-assignments.manage` | no | required | request ID | yes | `ResourceAssignmentPlanned` |
| `ca.resource-administration.resource-assignments.mark-overdue` | `mark_overdue` | command | `MarkOverdueResourceAssignmentInput` | `ResourceAssignmentView` | `ca.resource-assignments.manage` | no | required | request ID | yes | `ResourceAssignmentPlanned` |
| `ca.resource-administration.resource-assignments.cancel` | `cancel` | command | `CancelResourceAssignmentInput` | `ResourceAssignmentView` | `ca.resource-assignments.manage` | no | required | request ID | yes | `ResourceAssignmentPlanned` |
| `ca.resource-administration.resource-assignments.get` | `get` | query | `GetResourceAssignmentInput` | `ResourceAssignmentView` | `ca.resource-assignments.read` | — | none | — | no | — |
| `ca.resource-administration.resource-assignments.list` | `list` | query | `ListResourceAssignmentInput` | `Page<ResourceAssignmentView>` | `ca.resource-assignments.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.resource-assignments.read` | Approved get and list queries |
| `ca.resource-assignments.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `ResourceAssignmentPlanned` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ResourceIssued` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ResourceAssignmentExtended` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ResourceReturned` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ResourceReturnMarkedOverdue` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ResourceAssignmentCancelled` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/resource-administration/resource-assignments
```
...
```text
packages/erp/corporate-administration/src/features/resource-administration/resource-assignments/
...
Primary proposed metric: percentage of active resource assignments records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `resource-assignments` as the canonical feature identifier under `resource-administration` | proposed |
