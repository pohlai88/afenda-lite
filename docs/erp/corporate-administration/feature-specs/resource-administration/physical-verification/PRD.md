# Product Requirements Document: Physical Verification

> **Document purpose.**
> This PRD defines the `resource-administration/physical-verification` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-RESOURCE-PHYSICAL_VERIFICATION-001
feature_id: physical-verification
feature_name: Physical Verification
feature_group: resource-administration
...
feature_source_path: src/features/resource-administration/physical-verification
prd_path: docs/erp/corporate-administration/feature-specs/resource-administration/physical-verification/PRD.md
...
required_ports: []
emitted_events:
  - PhysicalVerificationPlanned
  - PhysicalVerificationStarted
  - VerificationObservationRecorded
  - VerificationDiscrepancyRecorded
  - VerificationDiscrepancyReconciled
  - PhysicalVerificationCompleted
  - PhysicalVerificationCancelled
consumed_by: []
estimated_slices: 11
```
...
| Field | Value |
| --- | --- |
| Feature name | Physical Verification |
| Feature identifier | `physical-verification` |
| Feature group | `resource-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for counts, confirmations, discrepancies, missing resources, and evidence |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for physical verification |
...
> Counts, confirmations, discrepancies, missing resources, and evidence.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Physical Verification enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for physical verification when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `PhysicalVerification, VerificationLine, VerificationDiscrepancy`.
...
| --- | --- | --- |
| `physical-verification` | Counts, confirmations, discrepancies, missing resources, and evidence | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `physical-verification.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `PhysicalVerification` | aggregate | `PhysicalVerification` | Physical count or confirmation exercise |
| `VerificationLine` | entity | `PhysicalVerification` | Expected and observed resource result |
| `VerificationDiscrepancy` | entity | `PhysicalVerification` | Missing, excess, damaged, or unverified discrepancy |
...
| `in-progress` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `reconciliation` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `completed` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `plan` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.physical-verification.manage` | no |
| TR-02 | Current permitted state | `start` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.physical-verification.manage` | no |
| TR-03 | Current permitted state | `record-observation` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.physical-verification.manage` | no |
| TR-04 | Current permitted state | `record-discrepancy` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.physical-verification.manage` | no |
| TR-05 | Current permitted state | `reconcile` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.physical-verification.manage` | no |
| TR-06 | Current permitted state | `complete` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.physical-verification.manage` | no |
| TR-07 | Current permitted state | `cancel` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.physical-verification.manage` | no |
...
Feature invariants use `CA-PHYSICAL_VERIFICATION-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.resource-administration.physical-verification.plan` | `plan` | command | `PlanPhysicalVerificationInput` | `PhysicalVerificationView` | `ca.physical-verification.manage` | no | required | request ID | yes | `PhysicalVerificationPlanned` |
| `ca.resource-administration.physical-verification.start` | `start` | command | `StartPhysicalVerificationInput` | `PhysicalVerificationView` | `ca.physical-verification.manage` | no | required | request ID | yes | `PhysicalVerificationPlanned` |
| `ca.resource-administration.physical-verification.record-observation` | `record_observation` | command | `RecordObservationPhysicalVerificationInput` | `PhysicalVerificationView` | `ca.physical-verification.manage` | no | required | request ID | yes | `PhysicalVerificationPlanned` |
| `ca.resource-administration.physical-verification.record-discrepancy` | `record_discrepancy` | command | `RecordDiscrepancyPhysicalVerificationInput` | `PhysicalVerificationView` | `ca.physical-verification.manage` | no | required | request ID | yes | `PhysicalVerificationPlanned` |
| `ca.resource-administration.physical-verification.reconcile` | `reconcile` | command | `ReconcilePhysicalVerificationInput` | `PhysicalVerificationView` | `ca.physical-verification.manage` | no | required | request ID | yes | `PhysicalVerificationPlanned` |
| `ca.resource-administration.physical-verification.complete` | `complete` | command | `CompletePhysicalVerificationInput` | `PhysicalVerificationView` | `ca.physical-verification.manage` | no | required | request ID | yes | `PhysicalVerificationPlanned` |
| `ca.resource-administration.physical-verification.cancel` | `cancel` | command | `CancelPhysicalVerificationInput` | `PhysicalVerificationView` | `ca.physical-verification.manage` | no | required | request ID | yes | `PhysicalVerificationPlanned` |
| `ca.resource-administration.physical-verification.get` | `get` | query | `GetPhysicalVerificationInput` | `PhysicalVerificationView` | `ca.physical-verification.read` | — | none | — | no | — |
| `ca.resource-administration.physical-verification.list` | `list` | query | `ListPhysicalVerificationInput` | `Page<PhysicalVerificationView>` | `ca.physical-verification.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.physical-verification.read` | Approved get and list queries |
| `ca.physical-verification.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `PhysicalVerificationPlanned` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `PhysicalVerificationStarted` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `VerificationObservationRecorded` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `VerificationDiscrepancyRecorded` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `VerificationDiscrepancyReconciled` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `PhysicalVerificationCompleted` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `PhysicalVerificationCancelled` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/resource-administration/physical-verification
```
...
```text
packages/erp/corporate-administration/src/features/resource-administration/physical-verification/
...
Primary proposed metric: percentage of active physical verification records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `physical-verification` as the canonical feature identifier under `resource-administration` | proposed |
