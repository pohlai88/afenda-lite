# Product Requirements Document: Access Resources

> **Document purpose.**
> This PRD defines the `resource-administration/access-resources` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-RESOURCE-ACCESS_RESOURCES-001
feature_id: access-resources
feature_name: Access Resources
...
feature_source_path: src/features/resource-administration/access-resources
prd_path: docs/erp/corporate-administration/feature-specs/resource-administration/access-resources/PRD.md
...
required_ports: []
emitted_events:
  - AccessResourceRegistered
  - AccessResourceIssued
  - AccessResourceReturned
  - AccessResourceReportedLost
  - AccessResourceRevoked
  - AccessResourceReplaced
  - AccessResourceRetired
consumed_by: []
estimated_slices: 11
```
...
| Field | Value |
| --- | --- |
| Feature name | Access Resources |
| Feature identifier | `access-resources` |
| Feature group | `resource-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for sole ownership of key, badge, access-card issue, custody, return, loss, and revocation |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for access resources |
...
> Sole ownership of key, badge, access-card issue, custody, return, loss, and revocation.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Access Resources enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for access resources when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `AccessResource, AccessResourceIssue`.
...
| --- | --- | --- |
| `access-resources` | Sole ownership of key, badge, access-card issue, custody, return, loss, and revocation | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `access-resources.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `AccessResource` | aggregate | `AccessResource` | Key, badge, or access-card custody record |
| `AccessResourceIssue` | history | `AccessResource` | Issue, return, loss, and revocation chronology |
...
| --- | --- | --- | --- |
| `available` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `issued` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
...
| `returned` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `register` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.access-resources.manage` | no |
| TR-02 | Current permitted state | `issue` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.access-resources.manage` | no |
| TR-03 | Current permitted state | `record-return` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.access-resources.manage` | no |
| TR-04 | Current permitted state | `report-lost` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.access-resources.manage` | no |
| TR-05 | Current permitted state | `revoke` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.access-resources.manage` | yes |
| TR-06 | Current permitted state | `replace` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.access-resources.manage` | no |
| TR-07 | Current permitted state | `retire` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.access-resources.manage` | no |
...
Feature invariants use `CA-ACCESS_RESOURCES-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.resource-administration.access-resources.register` | `register` | command | `RegisterAccessResourceInput` | `AccessResourceView` | `ca.access-resources.manage` | no | required | request ID | yes | `AccessResourceRegistered` |
| `ca.resource-administration.access-resources.issue` | `issue` | command | `IssueAccessResourceInput` | `AccessResourceView` | `ca.access-resources.manage` | no | required | request ID | yes | `AccessResourceRegistered` |
| `ca.resource-administration.access-resources.record-return` | `record_return` | command | `RecordReturnAccessResourceInput` | `AccessResourceView` | `ca.access-resources.manage` | no | required | request ID | yes | `AccessResourceRegistered` |
| `ca.resource-administration.access-resources.report-lost` | `report_lost` | command | `ReportLostAccessResourceInput` | `AccessResourceView` | `ca.access-resources.manage` | no | required | request ID | yes | `AccessResourceRegistered` |
| `ca.resource-administration.access-resources.revoke` | `revoke` | command | `RevokeAccessResourceInput` | `AccessResourceView` | `ca.access-resources.manage` | yes | required | request ID | yes | `AccessResourceRegistered` |
| `ca.resource-administration.access-resources.replace` | `replace` | command | `ReplaceAccessResourceInput` | `AccessResourceView` | `ca.access-resources.manage` | no | required | request ID | yes | `AccessResourceRegistered` |
| `ca.resource-administration.access-resources.retire` | `retire` | command | `RetireAccessResourceInput` | `AccessResourceView` | `ca.access-resources.manage` | no | required | request ID | yes | `AccessResourceRegistered` |
| `ca.resource-administration.access-resources.get` | `get` | query | `GetAccessResourceInput` | `AccessResourceView` | `ca.access-resources.read` | — | none | — | no | — |
| `ca.resource-administration.access-resources.list` | `list` | query | `ListAccessResourceInput` | `Page<AccessResourceView>` | `ca.access-resources.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.access-resources.read` | Approved get and list queries |
| `ca.access-resources.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `AccessResourceRegistered` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AccessResourceIssued` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AccessResourceReturned` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AccessResourceReportedLost` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AccessResourceRevoked` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AccessResourceReplaced` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AccessResourceRetired` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/resource-administration/access-resources
```
...
```text
packages/erp/corporate-administration/src/features/resource-administration/access-resources/
...
Primary proposed metric: percentage of active access resources records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `access-resources` as the canonical feature identifier under `resource-administration` | proposed |
