# Product Requirements Document: Service Subscriptions

> **Document purpose.**
> This PRD defines the `agreement-administration/service-subscriptions` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-AGREEMENT-SERVICE_SUBSCRIPTIONS-001
feature_id: service-subscriptions
feature_name: Service Subscriptions
feature_group: agreement-administration
...
feature_source_path: src/features/agreement-administration/service-subscriptions
prd_path: docs/erp/corporate-administration/feature-specs/agreement-administration/service-subscriptions/PRD.md
...
depends_on_features: ["obligations-calendar"]
...
required_ports: []
emitted_events:
  - ServiceSubscriptionRegistered
  - ServiceSubscriptionActivated
  - SubscriptionEntitlementAssigned
  - SubscriptionEntitlementUnassigned
  - SubscriptionPlanChanged
  - ServiceSubscriptionSuspended
  - SubscriptionTerminationRequested
  - ServiceSubscriptionTerminated
  - ServiceSubscriptionRenewed
consumed_by: []
estimated_slices: 11
```
...
| Field | Value |
| --- | --- |
| Feature name | Service Subscriptions |
| Feature identifier | `service-subscriptions` |
| Feature group | `agreement-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for phone, internet, Zoom, SaaS, utilities, recurring service entitlement, assignment, renewal, and termination |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for service subscriptions |
...
> Phone, internet, Zoom, SaaS, utilities, recurring service entitlement, assignment, renewal, and termination.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Service Subscriptions enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for service subscriptions when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `ServiceSubscription, SubscriptionAssignment, SubscriptionTerm`.
...
| obligation | compliance-administration/obligations-calendar | opaque obligation identifier | same organization |
| person or business unit | Human Resources / organization directory | opaque assignee identifier | same organization |
...
| --- | --- | --- |
| `service-subscriptions` | Phone, internet, Zoom, SaaS, utilities, recurring service entitlement, assignment, renewal, and termination | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `service-subscriptions.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `ServiceSubscription` | aggregate | `ServiceSubscription` | Internal administrative service entitlement |
| `SubscriptionAssignment` | entity | `ServiceSubscription` | Seat, line, number, or entitlement assignment |
| `SubscriptionTerm` | entity | `ServiceSubscription` | Plan, notice, renewal, and termination terms |
...
| `pending-termination` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `register` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.service-subscriptions.manage` | no |
| TR-02 | Current permitted state | `activate` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.service-subscriptions.manage` | yes |
| TR-03 | Current permitted state | `assign` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.service-subscriptions.manage` | no |
| TR-04 | Current permitted state | `unassign` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.service-subscriptions.manage` | no |
| TR-05 | Current permitted state | `change-plan` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.service-subscriptions.manage` | no |
| TR-06 | Current permitted state | `suspend` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.service-subscriptions.manage` | no |
| TR-07 | Current permitted state | `request-termination` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.service-subscriptions.manage` | no |
| TR-08 | Current permitted state | `terminate` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.service-subscriptions.manage` | yes |
| TR-09 | Current permitted state | `renew` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.service-subscriptions.manage` | no |
...
Feature invariants use `CA-SERVICE_SUBSCRIPTIONS-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.agreement-administration.service-subscriptions.register` | `register` | command | `RegisterServiceSubscriptionInput` | `ServiceSubscriptionView` | `ca.service-subscriptions.manage` | no | required | request ID | yes | `ServiceSubscriptionRegistered` |
| `ca.agreement-administration.service-subscriptions.activate` | `activate` | command | `ActivateServiceSubscriptionInput` | `ServiceSubscriptionView` | `ca.service-subscriptions.manage` | yes | required | request ID | yes | `ServiceSubscriptionRegistered` |
| `ca.agreement-administration.service-subscriptions.assign` | `assign` | command | `AssignServiceSubscriptionInput` | `ServiceSubscriptionView` | `ca.service-subscriptions.manage` | no | required | request ID | yes | `ServiceSubscriptionRegistered` |
| `ca.agreement-administration.service-subscriptions.unassign` | `unassign` | command | `UnassignServiceSubscriptionInput` | `ServiceSubscriptionView` | `ca.service-subscriptions.manage` | no | required | request ID | yes | `ServiceSubscriptionRegistered` |
| `ca.agreement-administration.service-subscriptions.change-plan` | `change_plan` | command | `ChangePlanServiceSubscriptionInput` | `ServiceSubscriptionView` | `ca.service-subscriptions.manage` | no | required | request ID | yes | `ServiceSubscriptionRegistered` |
| `ca.agreement-administration.service-subscriptions.suspend` | `suspend` | command | `SuspendServiceSubscriptionInput` | `ServiceSubscriptionView` | `ca.service-subscriptions.manage` | no | required | request ID | yes | `ServiceSubscriptionRegistered` |
| `ca.agreement-administration.service-subscriptions.request-termination` | `request_termination` | command | `RequestTerminationServiceSubscriptionInput` | `ServiceSubscriptionView` | `ca.service-subscriptions.manage` | no | required | request ID | yes | `ServiceSubscriptionRegistered` |
| `ca.agreement-administration.service-subscriptions.terminate` | `terminate` | command | `TerminateServiceSubscriptionInput` | `ServiceSubscriptionView` | `ca.service-subscriptions.manage` | yes | required | request ID | yes | `ServiceSubscriptionRegistered` |
| `ca.agreement-administration.service-subscriptions.renew` | `renew` | command | `RenewServiceSubscriptionInput` | `ServiceSubscriptionView` | `ca.service-subscriptions.manage` | no | required | request ID | yes | `ServiceSubscriptionRegistered` |
| `ca.agreement-administration.service-subscriptions.get` | `get` | query | `GetServiceSubscriptionInput` | `ServiceSubscriptionView` | `ca.service-subscriptions.read` | — | none | — | no | — |
| `ca.agreement-administration.service-subscriptions.list` | `list` | query | `ListServiceSubscriptionInput` | `Page<ServiceSubscriptionView>` | `ca.service-subscriptions.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.service-subscriptions.read` | Approved get and list queries |
| `ca.service-subscriptions.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `ServiceSubscriptionRegistered` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ServiceSubscriptionActivated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `SubscriptionEntitlementAssigned` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `SubscriptionEntitlementUnassigned` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `SubscriptionPlanChanged` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ServiceSubscriptionSuspended` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `SubscriptionTerminationRequested` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ServiceSubscriptionTerminated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ServiceSubscriptionRenewed` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/agreement-administration/service-subscriptions
```
...
Primary proposed metric: percentage of active service subscriptions records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `service-subscriptions` as the canonical feature identifier under `agreement-administration` | proposed |
