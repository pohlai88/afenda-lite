# Product Requirements Document: Group Structure

> **Document purpose.**
> This PRD defines the `entity-administration/group-structure` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-ENTITY-GROUP_STRUCTURE-001
feature_id: group-structure
feature_name: Group Structure
...
feature_source_path: src/features/entity-administration/group-structure
prd_path: docs/erp/corporate-administration/feature-specs/entity-administration/group-structure/PRD.md
...
required_ports: []
emitted_events:
  - GroupStructureRegistered
  - GroupStructureUpdated
  - GroupStructureActivated
  - GroupStructureSuspended
  - GroupStructureClosed
consumed_by: []
estimated_slices: 11
```
...
| Field | Value |
| --- | --- |
| Feature name | Group Structure |
| Feature identifier | `group-structure` |
| Feature group | `entity-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for parent, subsidiary, controlled-entity, and administrative legal relationships |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for group structure |
...
> Parent, subsidiary, controlled-entity, and administrative legal relationships.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Group Structure enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for group structure when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `EntityRelationship, RelationshipPeriod`.
...
| company | entity-administration/company | opaque company identifiers | approved same-group lineage |
...
* shareholding percentages unless expressly approved by Investor Relations;
...
| --- | --- | --- |
| `group-structure` | Parent, subsidiary, controlled-entity, and administrative legal relationships | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `group-structure.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `EntityRelationship` | aggregate | `GroupStructure` | Parent, subsidiary, controlled-entity, or administrative relationship |
| `RelationshipPeriod` | history | `GroupStructure` | Effective relationship chronology |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `register` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.group-structure.manage` | no |
| TR-02 | Current permitted state | `update` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.group-structure.manage` | no |
| TR-03 | Current permitted state | `activate` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.group-structure.manage` | yes |
| TR-04 | Current permitted state | `suspend` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.group-structure.manage` | no |
| TR-05 | Current permitted state | `close` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.group-structure.manage` | no |
...
Feature invariants use `CA-GROUP_STRUCTURE-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.entity-administration.group-structure.register` | `register` | command | `RegisterGroupStructureInput` | `GroupStructureView` | `ca.group-structure.manage` | no | required | request ID | yes | `GroupStructureRegistered` |
| `ca.entity-administration.group-structure.update` | `update` | command | `UpdateGroupStructureInput` | `GroupStructureView` | `ca.group-structure.manage` | no | required | request ID | yes | `GroupStructureRegistered` |
| `ca.entity-administration.group-structure.activate` | `activate` | command | `ActivateGroupStructureInput` | `GroupStructureView` | `ca.group-structure.manage` | yes | required | request ID | yes | `GroupStructureRegistered` |
| `ca.entity-administration.group-structure.suspend` | `suspend` | command | `SuspendGroupStructureInput` | `GroupStructureView` | `ca.group-structure.manage` | no | required | request ID | yes | `GroupStructureRegistered` |
| `ca.entity-administration.group-structure.close` | `close` | command | `CloseGroupStructureInput` | `GroupStructureView` | `ca.group-structure.manage` | no | required | request ID | yes | `GroupStructureRegistered` |
| `ca.entity-administration.group-structure.get` | `get` | query | `GetGroupStructureInput` | `GroupStructureView` | `ca.group-structure.read` | — | none | — | no | — |
| `ca.entity-administration.group-structure.list` | `list` | query | `ListGroupStructureInput` | `Page<GroupStructureView>` | `ca.group-structure.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.group-structure.read` | Approved get and list queries |
| `ca.group-structure.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `GroupStructureRegistered` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `GroupStructureUpdated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `GroupStructureActivated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `GroupStructureSuspended` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `GroupStructureClosed` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/entity-administration/group-structure
```
...
```text
packages/erp/corporate-administration/src/features/entity-administration/group-structure/
...
Primary proposed metric: percentage of active group structure records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `group-structure` as the canonical feature identifier under `entity-administration` | proposed |
