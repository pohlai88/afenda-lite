# Product Requirements Document: Establishments

> **Document purpose.**
> This PRD defines the `entity-administration/establishments` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-ENTITY-ESTABLISHMENTS-001
feature_id: establishments
feature_name: Establishments
...
feature_source_path: src/features/entity-administration/establishments
prd_path: docs/erp/corporate-administration/feature-specs/entity-administration/establishments/PRD.md
...
| Field | Value |
| --- | --- |
| Feature name | Establishments |
| Feature identifier | `establishments` |
| Feature group | `entity-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for registered offices, branches, representative offices, registrations, and status history |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for establishments |
...
> Registered offices, branches, representative offices, registrations, and status history.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Establishments enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for establishments when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `Establishment, EstablishmentRegistration`.
...
| company | entity-administration/company | opaque company identifier | same organization |
...
| --- | --- | --- |
| `establishments` | Registered offices, branches, representative offices, registrations, and status history | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `establishments.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `Establishment` | aggregate | `Establishment` | Registered office, branch, or representative-office administration |
| `EstablishmentRegistration` | history | `Establishment` | Registration and status chronology |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `register` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.establishments.manage` | no |
| TR-02 | Current permitted state | `update` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.establishments.manage` | no |
| TR-03 | Current permitted state | `activate` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.establishments.manage` | yes |
| TR-04 | Current permitted state | `suspend` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.establishments.manage` | no |
| TR-05 | Current permitted state | `close` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.establishments.manage` | no |
...
Feature invariants use `CA-ESTABLISHMENTS-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.entity-administration.establishments.register` | `register` | command | `RegisterEstablishmentInput` | `EstablishmentView` | `ca.establishments.manage` | no | required | request ID | yes | `EstablishmentRegistered` |
| `ca.entity-administration.establishments.update` | `update` | command | `UpdateEstablishmentInput` | `EstablishmentView` | `ca.establishments.manage` | no | required | request ID | yes | `EstablishmentRegistered` |
| `ca.entity-administration.establishments.activate` | `activate` | command | `ActivateEstablishmentInput` | `EstablishmentView` | `ca.establishments.manage` | yes | required | request ID | yes | `EstablishmentRegistered` |
| `ca.entity-administration.establishments.suspend` | `suspend` | command | `SuspendEstablishmentInput` | `EstablishmentView` | `ca.establishments.manage` | no | required | request ID | yes | `EstablishmentRegistered` |
| `ca.entity-administration.establishments.close` | `close` | command | `CloseEstablishmentInput` | `EstablishmentView` | `ca.establishments.manage` | no | required | request ID | yes | `EstablishmentRegistered` |
| `ca.entity-administration.establishments.get` | `get` | query | `GetEstablishmentInput` | `EstablishmentView` | `ca.establishments.read` | — | none | — | no | — |
| `ca.entity-administration.establishments.list` | `list` | query | `ListEstablishmentInput` | `Page<EstablishmentView>` | `ca.establishments.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.establishments.read` | Approved get and list queries |
| `ca.establishments.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `EstablishmentRegistered` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `EstablishmentUpdated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `EstablishmentActivated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `EstablishmentSuspended` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `EstablishmentClosed` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/entity-administration/establishments
```
...
```text
packages/erp/corporate-administration/src/features/entity-administration/establishments/
...
Primary proposed metric: percentage of active establishments records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `establishments` as the canonical feature identifier under `entity-administration` | proposed |
