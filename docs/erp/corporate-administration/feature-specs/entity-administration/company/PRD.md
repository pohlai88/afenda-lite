# Product Requirements Document: Company

> **Document purpose.**
> This PRD defines the `entity-administration/company` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
feature_source_path: src/features/entity-administration/company
prd_path: docs/erp/corporate-administration/feature-specs/entity-administration/company/PRD.md
...
| Feature identifier | `company` |
| Feature group | `entity-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for legal identity, jurisdiction, names, identifiers, activities, lifecycle, and chronology |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for company |
...
> Legal identity, jurisdiction, names, identifiers, activities, lifecycle, and chronology.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Company enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for company when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `Company, CompanyName, CompanyIdentifier`.
...
| --- | --- | --- |
| `company` | Legal identity, jurisdiction, names, identifiers, activities, lifecycle, and chronology | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `company.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `Company` | aggregate | `Company` | Authoritative legal-entity identity and chronology |
| `CompanyName` | history | `Company` | Legal, former, and trading-name chronology |
| `CompanyIdentifier` | entity | `Company` | Registration and jurisdiction identifiers |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `register` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.company.manage` | no |
| TR-02 | Current permitted state | `update` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.company.manage` | no |
| TR-03 | Current permitted state | `activate` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.company.manage` | yes |
| TR-04 | Current permitted state | `suspend` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.company.manage` | no |
| TR-05 | Current permitted state | `close` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.company.manage` | no |
...
Feature invariants use `CA-COMPANY-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.entity-administration.company.register` | `register` | command | `RegisterCompanyInput` | `CompanyView` | `ca.company.manage` | no | required | request ID | yes | `CompanyRegistered` |
| `ca.entity-administration.company.update` | `update` | command | `UpdateCompanyInput` | `CompanyView` | `ca.company.manage` | no | required | request ID | yes | `CompanyRegistered` |
| `ca.entity-administration.company.activate` | `activate` | command | `ActivateCompanyInput` | `CompanyView` | `ca.company.manage` | yes | required | request ID | yes | `CompanyRegistered` |
| `ca.entity-administration.company.suspend` | `suspend` | command | `SuspendCompanyInput` | `CompanyView` | `ca.company.manage` | no | required | request ID | yes | `CompanyRegistered` |
| `ca.entity-administration.company.close` | `close` | command | `CloseCompanyInput` | `CompanyView` | `ca.company.manage` | no | required | request ID | yes | `CompanyRegistered` |
| `ca.entity-administration.company.get` | `get` | query | `GetCompanyInput` | `CompanyView` | `ca.company.read` | — | none | — | no | — |
| `ca.entity-administration.company.list` | `list` | query | `ListCompanyInput` | `Page<CompanyView>` | `ca.company.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.company.read` | Approved get and list queries |
| `ca.company.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `CompanyRegistered` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `CompanyUpdated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `CompanyActivated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `CompanySuspended` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `CompanyClosed` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/entity-administration/company
```
...
```text
packages/erp/corporate-administration/src/features/entity-administration/company/
...
Primary proposed metric: percentage of active company records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `company` as the canonical feature identifier under `entity-administration` | proposed |
