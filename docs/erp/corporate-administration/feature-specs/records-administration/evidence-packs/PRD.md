# Product Requirements Document: Evidence Packs

> **Document purpose.**
> This PRD defines the `records-administration/evidence-packs` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-RECORDS-EVIDENCE_PACKS-001
feature_id: evidence-packs
feature_name: Evidence Packs
...
feature_source_path: src/features/records-administration/evidence-packs
prd_path: docs/erp/corporate-administration/feature-specs/records-administration/evidence-packs/PRD.md
...
emitted_events:
  ...
  - EvidenceItemRemoved
  - EvidenceGapRecorded
  - EvidencePackCompleted
  - EvidencePackSealed
  - EvidencePackReleased
  - EvidencePackSuperseded
consumed_by: []
estimated_slices: 11
```
...
| Field | Value |
| --- | --- |
| Feature name | Evidence Packs |
| Feature identifier | `evidence-packs` |
| Feature group | `records-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for compiled evidence for audits, filings, renewals, and governance events |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for evidence packs |
...
> Compiled evidence for audits, filings, renewals, and governance events.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Evidence Packs enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for evidence packs when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `EvidencePack, EvidenceItem, EvidenceGap`.
...
* binary file storage;
* mutation of source feature records;
...
| --- | --- | --- |
| `evidence-packs` | Compiled evidence for audits, filings, renewals, and governance events | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `evidence-packs.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `EvidencePack` | aggregate | `EvidencePack` | Compiled evidence for a governed purpose |
| `EvidenceItem` | entity | `EvidencePack` | Reference to required or supplied evidence |
| `EvidenceGap` | entity | `EvidencePack` | Missing or insufficient evidence |
...
| `assembling` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `complete` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `sealed` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `create` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.evidence-packs.manage` | no |
| TR-02 | Current permitted state | `add-item` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.evidence-packs.manage` | no |
| TR-03 | Current permitted state | `remove-item` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.evidence-packs.manage` | no |
| TR-04 | Current permitted state | `record-gap` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.evidence-packs.manage` | no |
| TR-05 | Current permitted state | `complete` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.evidence-packs.manage` | no |
| TR-06 | Current permitted state | `seal` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.evidence-packs.manage` | yes |
| TR-07 | Current permitted state | `release` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.evidence-packs.manage` | no |
| TR-08 | Current permitted state | `supersede` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.evidence-packs.manage` | no |
...
Feature invariants use `CA-EVIDENCE_PACKS-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.records-administration.evidence-packs.create` | `create` | command | `CreateEvidencePackInput` | `EvidencePackView` | `ca.evidence-packs.manage` | no | required | request ID | yes | `EvidencePackCreated` |
| `ca.records-administration.evidence-packs.add-item` | `add_item` | command | `AddItemEvidencePackInput` | `EvidencePackView` | `ca.evidence-packs.manage` | no | required | request ID | yes | `EvidencePackCreated` |
| `ca.records-administration.evidence-packs.remove-item` | `remove_item` | command | `RemoveItemEvidencePackInput` | `EvidencePackView` | `ca.evidence-packs.manage` | no | required | request ID | yes | `EvidencePackCreated` |
| `ca.records-administration.evidence-packs.record-gap` | `record_gap` | command | `RecordGapEvidencePackInput` | `EvidencePackView` | `ca.evidence-packs.manage` | no | required | request ID | yes | `EvidencePackCreated` |
| `ca.records-administration.evidence-packs.complete` | `complete` | command | `CompleteEvidencePackInput` | `EvidencePackView` | `ca.evidence-packs.manage` | no | required | request ID | yes | `EvidencePackCreated` |
| `ca.records-administration.evidence-packs.seal` | `seal` | command | `SealEvidencePackInput` | `EvidencePackView` | `ca.evidence-packs.manage` | yes | required | request ID | yes | `EvidencePackCreated` |
| `ca.records-administration.evidence-packs.release` | `release` | command | `ReleaseEvidencePackInput` | `EvidencePackView` | `ca.evidence-packs.manage` | no | required | request ID | yes | `EvidencePackCreated` |
| `ca.records-administration.evidence-packs.supersede` | `supersede` | command | `SupersedeEvidencePackInput` | `EvidencePackView` | `ca.evidence-packs.manage` | no | required | request ID | yes | `EvidencePackCreated` |
| `ca.records-administration.evidence-packs.get` | `get` | query | `GetEvidencePackInput` | `EvidencePackView` | `ca.evidence-packs.read` | — | none | — | no | — |
| `ca.records-administration.evidence-packs.list` | `list` | query | `ListEvidencePackInput` | `Page<EvidencePackView>` | `ca.evidence-packs.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.evidence-packs.read` | Approved get and list queries |
| `ca.evidence-packs.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `EvidencePackCreated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `EvidenceItemAdded` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `EvidenceItemRemoved` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `EvidenceGapRecorded` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `EvidencePackCompleted` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `EvidencePackSealed` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `EvidencePackReleased` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `EvidencePackSuperseded` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/records-administration/evidence-packs
```
...
```text
packages/erp/corporate-administration/src/features/records-administration/evidence-packs/
...
Primary proposed metric: percentage of active evidence packs records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `evidence-packs` as the canonical feature identifier under `records-administration` | proposed |
