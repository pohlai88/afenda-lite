# Product Requirements Document: Document Register

> **Document purpose.**
> This PRD defines the `records-administration/document-register` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-RECORDS-DOCUMENT_REGISTER-001
feature_id: document-register
feature_name: Document Register
...
feature_source_path: src/features/records-administration/document-register
prd_path: docs/erp/corporate-administration/feature-specs/records-administration/document-register/PRD.md
...
| Field | Value |
| --- | --- |
| Feature name | Document Register |
| Feature identifier | `document-register` |
| Feature group | `records-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for registered corporate documents and governed file references |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for document register |
...
> Registered corporate documents and governed file references.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Document Register enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for document register when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `RegisteredDocument, DocumentReference`.
...
* generic folders and workspace storage;
...
| --- | --- | --- |
| `document-register` | Registered corporate documents and governed file references | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `document-register.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `RegisteredDocument` | aggregate | `DocumentRegister` | Corporate document registration and governed file reference |
| `DocumentReference` | entity | `DocumentRegister` | Opaque file reference and metadata |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `register` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.document-register.manage` | no |
| TR-02 | Current permitted state | `update-metadata` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.document-register.manage` | no |
| TR-03 | Current permitted state | `link-reference` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.document-register.manage` | no |
| TR-04 | Current permitted state | `supersede` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.document-register.manage` | no |
| TR-05 | Current permitted state | `withdraw` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.document-register.manage` | no |
| TR-06 | Current permitted state | `archive` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.document-register.manage` | no |
...
Feature invariants use `CA-DOCUMENT_REGISTER-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.records-administration.document-register.register` | `register` | command | `RegisterDocumentRegisterInput` | `DocumentRegisterView` | `ca.document-register.manage` | no | required | request ID | yes | `DocumentRegistered` |
| `ca.records-administration.document-register.update-metadata` | `update_metadata` | command | `UpdateMetadataDocumentRegisterInput` | `DocumentRegisterView` | `ca.document-register.manage` | no | required | request ID | yes | `DocumentRegistered` |
| `ca.records-administration.document-register.link-reference` | `link_reference` | command | `LinkReferenceDocumentRegisterInput` | `DocumentRegisterView` | `ca.document-register.manage` | no | required | request ID | yes | `DocumentRegistered` |
| `ca.records-administration.document-register.supersede` | `supersede` | command | `SupersedeDocumentRegisterInput` | `DocumentRegisterView` | `ca.document-register.manage` | no | required | request ID | yes | `DocumentRegistered` |
| `ca.records-administration.document-register.withdraw` | `withdraw` | command | `WithdrawDocumentRegisterInput` | `DocumentRegisterView` | `ca.document-register.manage` | no | required | request ID | yes | `DocumentRegistered` |
| `ca.records-administration.document-register.archive` | `archive` | command | `ArchiveDocumentRegisterInput` | `DocumentRegisterView` | `ca.document-register.manage` | no | required | request ID | yes | `DocumentRegistered` |
| `ca.records-administration.document-register.get` | `get` | query | `GetDocumentRegisterInput` | `DocumentRegisterView` | `ca.document-register.read` | — | none | — | no | — |
| `ca.records-administration.document-register.list` | `list` | query | `ListDocumentRegisterInput` | `Page<DocumentRegisterView>` | `ca.document-register.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.document-register.read` | Approved get and list queries |
| `ca.document-register.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `DocumentRegistered` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `DocumentMetadataUpdated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `DocumentReferenceLinked` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `DocumentSuperseded` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `DocumentWithdrawn` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `DocumentArchived` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/records-administration/document-register
```
...
```text
packages/erp/corporate-administration/src/features/records-administration/document-register/
...
Primary proposed metric: percentage of active document register records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `document-register` as the canonical feature identifier under `records-administration` | proposed |
