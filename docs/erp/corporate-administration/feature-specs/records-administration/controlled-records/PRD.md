# Product Requirements Document: Controlled Records

> **Document purpose.**
> This PRD defines the `records-administration/controlled-records` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-RECORDS-CONTROLLED_RECORDS-001
feature_id: controlled-records
feature_name: Controlled Records
...
feature_source_path: src/features/records-administration/controlled-records
prd_path: docs/erp/corporate-administration/feature-specs/records-administration/controlled-records/PRD.md
...
required_ports: []
emitted_events:
  - ControlledRecordDraftCreated
  - ControlledRecordSubmittedForReview
  - ControlledRecordApproved
  - ControlledRecordMadeEffective
  - ControlledRecordSuperseded
  - ControlledRecordWithdrawn
  - ControlledRecordArchived
consumed_by: []
estimated_slices: 11
```
...
| Field | Value |
| --- | --- |
| Feature name | Controlled Records |
| Feature identifier | `controlled-records` |
| Feature group | `records-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for controlled versions, classification, approval state, access policy, and supersession |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for controlled records |
...
> Controlled versions, classification, approval state, access policy, and supersession.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Controlled Records enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for controlled records when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `ControlledRecord, RecordVersion, RecordApproval`.
...
| --- | --- | --- |
| `controlled-records` | Controlled versions, classification, approval state, access policy, and supersession | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `controlled-records.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `ControlledRecord` | aggregate | `ControlledRecord` | Governed corporate record |
| `RecordVersion` | entity | `ControlledRecord` | Immutable controlled version |
| `RecordApproval` | history | `ControlledRecord` | Approval and effective-state chronology |
...
| `approved` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
...
| `superseded` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | yes |
...
| `archived` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |

## 8.2 Proposed transition catalog

| Transition ID | From | Trigger | To | Guard | Permission | Approval |
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `create-draft` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.controlled-records.manage` | no |
| TR-02 | Current permitted state | `submit-review` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.controlled-records.manage` | no |
| TR-03 | Current permitted state | `approve` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.controlled-records.manage` | yes |
| TR-04 | Current permitted state | `make-effective` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.controlled-records.manage` | no |
| TR-05 | Current permitted state | `supersede` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.controlled-records.manage` | no |
| TR-06 | Current permitted state | `withdraw` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.controlled-records.manage` | no |
| TR-07 | Current permitted state | `archive` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.controlled-records.manage` | no |
...
Feature invariants use `CA-CONTROLLED_RECORDS-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.records-administration.controlled-records.create-draft` | `create_draft` | command | `CreateDraftControlledRecordInput` | `ControlledRecordView` | `ca.controlled-records.manage` | no | required | request ID | yes | `ControlledRecordDraftCreated` |
| `ca.records-administration.controlled-records.submit-review` | `submit_review` | command | `SubmitReviewControlledRecordInput` | `ControlledRecordView` | `ca.controlled-records.manage` | no | required | request ID | yes | `ControlledRecordDraftCreated` |
| `ca.records-administration.controlled-records.approve` | `approve` | command | `ApproveControlledRecordInput` | `ControlledRecordView` | `ca.controlled-records.manage` | yes | required | request ID | yes | `ControlledRecordDraftCreated` |
| `ca.records-administration.controlled-records.make-effective` | `make_effective` | command | `MakeEffectiveControlledRecordInput` | `ControlledRecordView` | `ca.controlled-records.manage` | no | required | request ID | yes | `ControlledRecordDraftCreated` |
| `ca.records-administration.controlled-records.supersede` | `supersede` | command | `SupersedeControlledRecordInput` | `ControlledRecordView` | `ca.controlled-records.manage` | no | required | request ID | yes | `ControlledRecordDraftCreated` |
| `ca.records-administration.controlled-records.withdraw` | `withdraw` | command | `WithdrawControlledRecordInput` | `ControlledRecordView` | `ca.controlled-records.manage` | no | required | request ID | yes | `ControlledRecordDraftCreated` |
| `ca.records-administration.controlled-records.archive` | `archive` | command | `ArchiveControlledRecordInput` | `ControlledRecordView` | `ca.controlled-records.manage` | no | required | request ID | yes | `ControlledRecordDraftCreated` |
| `ca.records-administration.controlled-records.get` | `get` | query | `GetControlledRecordInput` | `ControlledRecordView` | `ca.controlled-records.read` | — | none | — | no | — |
| `ca.records-administration.controlled-records.list` | `list` | query | `ListControlledRecordInput` | `Page<ControlledRecordView>` | `ca.controlled-records.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.controlled-records.read` | Approved get and list queries |
| `ca.controlled-records.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `ControlledRecordDraftCreated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ControlledRecordSubmittedForReview` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ControlledRecordApproved` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ControlledRecordMadeEffective` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ControlledRecordSuperseded` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ControlledRecordWithdrawn` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ControlledRecordArchived` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/records-administration/controlled-records
```
...
```text
packages/erp/corporate-administration/src/features/records-administration/controlled-records/
...
Primary proposed metric: percentage of active controlled records records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `controlled-records` as the canonical feature identifier under `records-administration` | proposed |
