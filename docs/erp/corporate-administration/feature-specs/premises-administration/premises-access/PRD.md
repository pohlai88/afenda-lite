# Product Requirements Document: Premises Access

> **Document purpose.**
> This PRD defines the `premises-administration/premises-access` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-PREMISES-PREMISES_ACCESS-001
feature_id: premises-access
feature_name: Premises Access
...
feature_source_path: src/features/premises-administration/premises-access
prd_path: docs/erp/corporate-administration/feature-specs/premises-administration/premises-access/PRD.md
...
required_ports: []
emitted_events:
  - PremisesAccessGranted
  - PremisesAccessAmended
  - PremisesAccessSuspended
  - PremisesAccessRestored
  - PremisesAccessRevoked
  - PremisesAccessExpired
consumed_by: []
estimated_slices: 11
```
...
| Field | Value |
| --- | --- |
| Feature name | Premises Access |
| Feature identifier | `premises-access` |
| Feature group | `premises-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for premises-level access grants referencing access resources; no credential lifecycle ownership |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for premises access |
...
> Premises-level access grants referencing access resources; no credential lifecycle ownership.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Premises Access enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for premises access when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `PremisesAccessGrant, AccessScope`.
...
| access resource | resource-administration/access-resources | opaque credential identifier | same organization |
...
* credential issue, return, loss, or replacement lifecycle;
* technical access-control configuration;
...
| --- | --- | --- |
| `premises-access` | Premises-level access grants referencing access resources; no credential lifecycle ownership | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `premises-access.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `PremisesAccessGrant` | aggregate | `PremisesAccess` | Premises-level access grant |
| `AccessScope` | entity | `PremisesAccess` | Permitted premises, zones, and effective period |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `grant` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.premises-access.manage` | no |
| TR-02 | Current permitted state | `amend` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.premises-access.manage` | no |
| TR-03 | Current permitted state | `suspend` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.premises-access.manage` | no |
| TR-04 | Current permitted state | `restore` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.premises-access.manage` | no |
| TR-05 | Current permitted state | `revoke` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.premises-access.manage` | yes |
| TR-06 | Current permitted state | `expire` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.premises-access.manage` | no |
...
Feature invariants use `CA-PREMISES_ACCESS-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.premises-administration.premises-access.grant` | `grant` | command | `GrantPremisesAccessInput` | `PremisesAccessView` | `ca.premises-access.manage` | no | required | request ID | yes | `PremisesAccessGranted` |
| `ca.premises-administration.premises-access.amend` | `amend` | command | `AmendPremisesAccessInput` | `PremisesAccessView` | `ca.premises-access.manage` | no | required | request ID | yes | `PremisesAccessGranted` |
| `ca.premises-administration.premises-access.suspend` | `suspend` | command | `SuspendPremisesAccessInput` | `PremisesAccessView` | `ca.premises-access.manage` | no | required | request ID | yes | `PremisesAccessGranted` |
| `ca.premises-administration.premises-access.restore` | `restore` | command | `RestorePremisesAccessInput` | `PremisesAccessView` | `ca.premises-access.manage` | no | required | request ID | yes | `PremisesAccessGranted` |
| `ca.premises-administration.premises-access.revoke` | `revoke` | command | `RevokePremisesAccessInput` | `PremisesAccessView` | `ca.premises-access.manage` | yes | required | request ID | yes | `PremisesAccessGranted` |
| `ca.premises-administration.premises-access.expire` | `expire` | command | `ExpirePremisesAccessInput` | `PremisesAccessView` | `ca.premises-access.manage` | no | required | request ID | yes | `PremisesAccessGranted` |
| `ca.premises-administration.premises-access.get` | `get` | query | `GetPremisesAccessInput` | `PremisesAccessView` | `ca.premises-access.read` | — | none | — | no | — |
| `ca.premises-administration.premises-access.list` | `list` | query | `ListPremisesAccessInput` | `Page<PremisesAccessView>` | `ca.premises-access.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.premises-access.read` | Approved get and list queries |
| `ca.premises-access.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `PremisesAccessGranted` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `PremisesAccessAmended` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `PremisesAccessSuspended` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `PremisesAccessRestored` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `PremisesAccessRevoked` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `PremisesAccessExpired` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/premises-administration/premises-access
```
...
```text
packages/erp/corporate-administration/src/features/premises-administration/premises-access/
...
Primary proposed metric: percentage of active premises access records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `premises-access` as the canonical feature identifier under `premises-administration` | proposed |
