# Product Requirements Document: Premises

> **Document purpose.**
> This PRD defines the `premises-administration/premises` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-PREMISES-PREMISES-001
feature_id: premises
feature_name: Premises
...
feature_source_path: src/features/premises-administration/premises
prd_path: docs/erp/corporate-administration/feature-specs/premises-administration/premises/PRD.md
...
| Field | Value |
| --- | --- |
| Feature name | Premises |
| Feature identifier | `premises` |
| Feature group | `premises-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for administrative premises identity and responsible company |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for premises |
...
> Administrative premises identity and responsible company.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Premises enables authorized users to:
...
This feature solely owns the meaning and lifecycle of: `Premise, PremiseResponsibility`.
...
| --- | --- | --- |
| `premises` | Administrative premises identity and responsible company | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `premises.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `Premise` | aggregate | `Premise` | Administrative premises identity |
| `PremiseResponsibility` | history | `Premise` | Responsible company and owner chronology |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `register` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.premises.manage` | no |
| TR-02 | Current permitted state | `update` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.premises.manage` | no |
| TR-03 | Current permitted state | `activate` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.premises.manage` | yes |
| TR-04 | Current permitted state | `suspend` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.premises.manage` | no |
| TR-05 | Current permitted state | `close` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.premises.manage` | no |
...
Feature invariants use `CA-PREMISES-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.premises-administration.premises.register` | `register` | command | `RegisterPremiseInput` | `PremiseView` | `ca.premises.manage` | no | required | request ID | yes | `PremiseRegistered` |
| `ca.premises-administration.premises.update` | `update` | command | `UpdatePremiseInput` | `PremiseView` | `ca.premises.manage` | no | required | request ID | yes | `PremiseRegistered` |
| `ca.premises-administration.premises.activate` | `activate` | command | `ActivatePremiseInput` | `PremiseView` | `ca.premises.manage` | yes | required | request ID | yes | `PremiseRegistered` |
| `ca.premises-administration.premises.suspend` | `suspend` | command | `SuspendPremiseInput` | `PremiseView` | `ca.premises.manage` | no | required | request ID | yes | `PremiseRegistered` |
| `ca.premises-administration.premises.close` | `close` | command | `ClosePremiseInput` | `PremiseView` | `ca.premises.manage` | no | required | request ID | yes | `PremiseRegistered` |
| `ca.premises-administration.premises.get` | `get` | query | `GetPremiseInput` | `PremiseView` | `ca.premises.read` | — | none | — | no | — |
| `ca.premises-administration.premises.list` | `list` | query | `ListPremiseInput` | `Page<PremiseView>` | `ca.premises.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.premises.read` | Approved get and list queries |
| `ca.premises.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `PremiseRegistered` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `PremiseUpdated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `PremiseActivated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `PremiseSuspended` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `PremiseClosed` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/premises-administration/premises
```
...
```text
packages/erp/corporate-administration/src/features/premises-administration/premises/
...
Primary proposed metric: percentage of active premises records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `premises` as the canonical feature identifier under `premises-administration` | proposed |
