# Product Requirements Document: Occupancy

> **Document purpose.**
> This PRD defines the `premises-administration/occupancy` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-PREMISES-OCCUPANCY-001
feature_id: occupancy
...
feature_source_path: src/features/premises-administration/occupancy
prd_path: docs/erp/corporate-administration/feature-specs/premises-administration/occupancy/PRD.md
...
| Field | Value |
| --- | --- |
| Feature name | Occupancy |
| Feature identifier | `occupancy` |
| Feature group | `premises-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for lease or occupancy period, usable areas, and administrative occupancy facts |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for occupancy |
...
> Lease or occupancy period, usable areas, and administrative occupancy facts.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Occupancy enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for occupancy when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `Occupancy, OccupiedArea`.
...
| --- | --- | --- |
| `occupancy` | Lease or occupancy period, usable areas, and administrative occupancy facts | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `occupancy.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `Occupancy` | aggregate | `Occupancy` | Administrative occupancy or lease-period facts |
| `OccupiedArea` | entity | `Occupancy` | Usable area and administrative allocation |
...
| `holding-over` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `ended` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `register` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.occupancy.manage` | no |
| TR-02 | Current permitted state | `activate` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.occupancy.manage` | yes |
| TR-03 | Current permitted state | `amend` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.occupancy.manage` | no |
| TR-04 | Current permitted state | `record-holding-over` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.occupancy.manage` | no |
| TR-05 | Current permitted state | `terminate` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.occupancy.manage` | yes |
| TR-06 | Current permitted state | `end` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.occupancy.manage` | no |
...
Feature invariants use `CA-OCCUPANCY-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.premises-administration.occupancy.register` | `register` | command | `RegisterOccupancyInput` | `OccupancyView` | `ca.occupancy.manage` | no | required | request ID | yes | `OccupancyRegistered` |
| `ca.premises-administration.occupancy.activate` | `activate` | command | `ActivateOccupancyInput` | `OccupancyView` | `ca.occupancy.manage` | yes | required | request ID | yes | `OccupancyRegistered` |
| `ca.premises-administration.occupancy.amend` | `amend` | command | `AmendOccupancyInput` | `OccupancyView` | `ca.occupancy.manage` | no | required | request ID | yes | `OccupancyRegistered` |
| `ca.premises-administration.occupancy.record-holding-over` | `record_holding_over` | command | `RecordHoldingOverOccupancyInput` | `OccupancyView` | `ca.occupancy.manage` | no | required | request ID | yes | `OccupancyRegistered` |
| `ca.premises-administration.occupancy.terminate` | `terminate` | command | `TerminateOccupancyInput` | `OccupancyView` | `ca.occupancy.manage` | yes | required | request ID | yes | `OccupancyRegistered` |
| `ca.premises-administration.occupancy.end` | `end` | command | `EndOccupancyInput` | `OccupancyView` | `ca.occupancy.manage` | no | required | request ID | yes | `OccupancyRegistered` |
| `ca.premises-administration.occupancy.get` | `get` | query | `GetOccupancyInput` | `OccupancyView` | `ca.occupancy.read` | — | none | — | no | — |
| `ca.premises-administration.occupancy.list` | `list` | query | `ListOccupancyInput` | `Page<OccupancyView>` | `ca.occupancy.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.occupancy.read` | Approved get and list queries |
| `ca.occupancy.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `OccupancyRegistered` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `OccupancyActivated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `OccupancyAmended` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `OccupancyHoldingOverRecorded` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `OccupancyTerminated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `OccupancyEnded` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/premises-administration/occupancy
```
...
```text
packages/erp/corporate-administration/src/features/premises-administration/occupancy/
...
Primary proposed metric: percentage of active occupancy records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `occupancy` as the canonical feature identifier under `premises-administration` | proposed |
