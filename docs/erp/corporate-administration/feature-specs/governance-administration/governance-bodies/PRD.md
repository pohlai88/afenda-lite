# Product Requirements Document: Governance Bodies

> **Document purpose.**
> This PRD defines the `governance-administration/governance-bodies` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-GOVERNANCE-GOVERNANCE_BODIES-001
feature_id: governance-bodies
feature_name: Governance Bodies
...
feature_source_path: src/features/governance-administration/governance-bodies
prd_path: docs/erp/corporate-administration/feature-specs/governance-administration/governance-bodies/PRD.md
...
required_ports: []
emitted_events:
  - GovernanceBodyRegistered
  - GovernanceBodyUpdated
  - GovernanceBodyActivated
  - GovernanceBodySuspended
...
| Field | Value |
| --- | --- |
| Feature name | Governance Bodies |
| Feature identifier | `governance-bodies` |
| Feature group | `governance-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for boards, committees, constitution, membership structure, and body lifecycle |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for governance bodies |
...
> Boards, committees, constitution, membership structure, and body lifecycle.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Governance Bodies enables authorized users to:
...
This feature solely owns the meaning and lifecycle of: `GovernanceBody, BodyConstitution`.
...
| --- | --- | --- |
| `governance-bodies` | Boards, committees, constitution, membership structure, and body lifecycle | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `governance-bodies.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `GovernanceBody` | aggregate | `GovernanceBody` | Board, committee, or other governance body |
| `BodyConstitution` | history | `GovernanceBody` | Purpose, composition, and effective constitution |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `register` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.governance-bodies.manage` | no |
| TR-02 | Current permitted state | `update` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.governance-bodies.manage` | no |
| TR-03 | Current permitted state | `activate` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.governance-bodies.manage` | yes |
| TR-04 | Current permitted state | `suspend` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.governance-bodies.manage` | no |
| TR-05 | Current permitted state | `close` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.governance-bodies.manage` | no |
...
Feature invariants use `CA-GOVERNANCE_BODIES-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.governance-administration.governance-bodies.register` | `register` | command | `RegisterGovernanceBodyInput` | `GovernanceBodyView` | `ca.governance-bodies.manage` | no | required | request ID | yes | `GovernanceBodyRegistered` |
| `ca.governance-administration.governance-bodies.update` | `update` | command | `UpdateGovernanceBodyInput` | `GovernanceBodyView` | `ca.governance-bodies.manage` | no | required | request ID | yes | `GovernanceBodyRegistered` |
| `ca.governance-administration.governance-bodies.activate` | `activate` | command | `ActivateGovernanceBodyInput` | `GovernanceBodyView` | `ca.governance-bodies.manage` | yes | required | request ID | yes | `GovernanceBodyRegistered` |
| `ca.governance-administration.governance-bodies.suspend` | `suspend` | command | `SuspendGovernanceBodyInput` | `GovernanceBodyView` | `ca.governance-bodies.manage` | no | required | request ID | yes | `GovernanceBodyRegistered` |
| `ca.governance-administration.governance-bodies.close` | `close` | command | `CloseGovernanceBodyInput` | `GovernanceBodyView` | `ca.governance-bodies.manage` | no | required | request ID | yes | `GovernanceBodyRegistered` |
| `ca.governance-administration.governance-bodies.get` | `get` | query | `GetGovernanceBodyInput` | `GovernanceBodyView` | `ca.governance-bodies.read` | — | none | — | no | — |
| `ca.governance-administration.governance-bodies.list` | `list` | query | `ListGovernanceBodyInput` | `Page<GovernanceBodyView>` | `ca.governance-bodies.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.governance-bodies.read` | Approved get and list queries |
| `ca.governance-bodies.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `GovernanceBodyRegistered` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `GovernanceBodyUpdated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `GovernanceBodyActivated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `GovernanceBodySuspended` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `GovernanceBodyClosed` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/governance-administration/governance-bodies
```
...
```text
packages/erp/corporate-administration/src/features/governance-administration/governance-bodies/
...
Primary proposed metric: percentage of active governance bodies records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `governance-bodies` as the canonical feature identifier under `governance-administration` | proposed |
