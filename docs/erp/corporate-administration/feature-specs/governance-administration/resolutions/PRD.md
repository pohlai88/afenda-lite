# Product Requirements Document: Resolutions

> **Document purpose.**
> This PRD defines the `governance-administration/resolutions` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-GOVERNANCE-RESOLUTIONS-001
feature_id: resolutions
feature_name: Resolutions
feature_group: governance-administration
...
feature_source_path: src/features/governance-administration/resolutions
prd_path: docs/erp/corporate-administration/feature-specs/governance-administration/resolutions/PRD.md
...
| Field | Value |
| --- | --- |
| Feature name | Resolutions |
| Feature identifier | `resolutions` |
| Feature group | `governance-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for voting, written decisions, adoption, rejection, supersession, and implementation references |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for resolutions |
...
> Voting, written decisions, adoption, rejection, supersession, and implementation references.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Resolutions enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for resolutions when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `Resolution, Vote, ImplementationReference`.
...
| meeting | governance-administration/meetings | opaque meeting identifier | same organization |
...
| --- | --- | --- |
| `resolutions` | Voting, written decisions, adoption, rejection, supersession, and implementation references | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `resolutions.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `Resolution` | aggregate | `Resolution` | Proposed and decided governance resolution |
| `Vote` | entity | `Resolution` | Vote or written-consent fact |
| `ImplementationReference` | entity | `Resolution` | Governed reference to downstream implementation |
...
| `proposed` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `adopted` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `draft` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.resolutions.manage` | no |
| TR-02 | Current permitted state | `propose` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.resolutions.manage` | no |
| TR-03 | Current permitted state | `record-vote` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.resolutions.manage` | no |
| TR-04 | Current permitted state | `adopt` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.resolutions.manage` | no |
| TR-05 | Current permitted state | `reject` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.resolutions.manage` | no |
| TR-06 | Current permitted state | `supersede` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.resolutions.manage` | no |
| TR-07 | Current permitted state | `withdraw` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.resolutions.manage` | no |
...
Feature invariants use `CA-RESOLUTIONS-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.governance-administration.resolutions.draft` | `draft` | command | `DraftResolutionInput` | `ResolutionView` | `ca.resolutions.manage` | no | required | request ID | yes | `ResolutionDrafted` |
| `ca.governance-administration.resolutions.propose` | `propose` | command | `ProposeResolutionInput` | `ResolutionView` | `ca.resolutions.manage` | no | required | request ID | yes | `ResolutionDrafted` |
| `ca.governance-administration.resolutions.record-vote` | `record_vote` | command | `RecordVoteResolutionInput` | `ResolutionView` | `ca.resolutions.manage` | no | required | request ID | yes | `ResolutionDrafted` |
| `ca.governance-administration.resolutions.adopt` | `adopt` | command | `AdoptResolutionInput` | `ResolutionView` | `ca.resolutions.manage` | no | required | request ID | yes | `ResolutionDrafted` |
| `ca.governance-administration.resolutions.reject` | `reject` | command | `RejectResolutionInput` | `ResolutionView` | `ca.resolutions.manage` | no | required | request ID | yes | `ResolutionDrafted` |
| `ca.governance-administration.resolutions.supersede` | `supersede` | command | `SupersedeResolutionInput` | `ResolutionView` | `ca.resolutions.manage` | no | required | request ID | yes | `ResolutionDrafted` |
| `ca.governance-administration.resolutions.withdraw` | `withdraw` | command | `WithdrawResolutionInput` | `ResolutionView` | `ca.resolutions.manage` | no | required | request ID | yes | `ResolutionDrafted` |
| `ca.governance-administration.resolutions.get` | `get` | query | `GetResolutionInput` | `ResolutionView` | `ca.resolutions.read` | — | none | — | no | — |
| `ca.governance-administration.resolutions.list` | `list` | query | `ListResolutionInput` | `Page<ResolutionView>` | `ca.resolutions.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.resolutions.read` | Approved get and list queries |
| `ca.resolutions.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `ResolutionDrafted` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ResolutionProposed` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ResolutionAdopted` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ResolutionRejected` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ResolutionSuperseded` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `ResolutionWithdrawn` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/governance-administration/resolutions
```
...
```text
packages/erp/corporate-administration/src/features/governance-administration/resolutions/
...
Primary proposed metric: percentage of active resolutions records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `resolutions` as the canonical feature identifier under `governance-administration` | proposed |
