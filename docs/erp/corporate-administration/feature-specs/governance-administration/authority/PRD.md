# Product Requirements Document: Authority

> **Document purpose.**
> This PRD defines the `governance-administration/authority` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-GOVERNANCE-AUTHORITY-001
feature_id: authority
...
feature_source_path: src/features/governance-administration/authority
prd_path: docs/erp/corporate-administration/feature-specs/governance-administration/authority/PRD.md
...
required_ports: []
emitted_events:
  - AuthorityInstrumentDrafted
  - AuthorityInstrumentActivated
  - AuthorityInstrumentAmended
  - AuthorityInstrumentSuspended
  - AuthorityInstrumentRevoked
  - AuthorityInstrumentExpired
consumed_by: []
estimated_slices: 11
```
...
| Feature identifier | `authority` |
| Feature group | `governance-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for delegations, signing mandates, bank mandates, powers of attorney, and limits |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for authority |
...
> Delegations, signing mandates, bank mandates, powers of attorney, and limits.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Authority enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for authority when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `AuthorityInstrument, AuthorityLimit, AuthorizedParty`.
...
| --- | --- | --- |
| `authority` | Delegations, signing mandates, bank mandates, powers of attorney, and limits | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `authority.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `AuthorityInstrument` | aggregate | `Authority` | Delegation, mandate, or power of attorney |
| `AuthorityLimit` | entity | `Authority` | Scope, monetary, temporal, or counterparty limit |
| `AuthorizedParty` | entity | `Authority` | Holder or authorized signatory reference |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `draft` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.authority.manage` | no |
| TR-02 | Current permitted state | `activate` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.authority.manage` | yes |
| TR-03 | Current permitted state | `amend` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.authority.manage` | no |
| TR-04 | Current permitted state | `suspend` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.authority.manage` | no |
| TR-05 | Current permitted state | `revoke` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.authority.manage` | yes |
| TR-06 | Current permitted state | `expire` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.authority.manage` | no |
...
Feature invariants use `CA-AUTHORITY-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.governance-administration.authority.draft` | `draft` | command | `DraftAuthorityInput` | `AuthorityView` | `ca.authority.manage` | no | required | request ID | yes | `AuthorityInstrumentDrafted` |
| `ca.governance-administration.authority.activate` | `activate` | command | `ActivateAuthorityInput` | `AuthorityView` | `ca.authority.manage` | yes | required | request ID | yes | `AuthorityInstrumentDrafted` |
| `ca.governance-administration.authority.amend` | `amend` | command | `AmendAuthorityInput` | `AuthorityView` | `ca.authority.manage` | no | required | request ID | yes | `AuthorityInstrumentDrafted` |
| `ca.governance-administration.authority.suspend` | `suspend` | command | `SuspendAuthorityInput` | `AuthorityView` | `ca.authority.manage` | no | required | request ID | yes | `AuthorityInstrumentDrafted` |
| `ca.governance-administration.authority.revoke` | `revoke` | command | `RevokeAuthorityInput` | `AuthorityView` | `ca.authority.manage` | yes | required | request ID | yes | `AuthorityInstrumentDrafted` |
| `ca.governance-administration.authority.expire` | `expire` | command | `ExpireAuthorityInput` | `AuthorityView` | `ca.authority.manage` | no | required | request ID | yes | `AuthorityInstrumentDrafted` |
| `ca.governance-administration.authority.get` | `get` | query | `GetAuthorityInput` | `AuthorityView` | `ca.authority.read` | — | none | — | no | — |
| `ca.governance-administration.authority.list` | `list` | query | `ListAuthorityInput` | `Page<AuthorityView>` | `ca.authority.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.authority.read` | Approved get and list queries |
| `ca.authority.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `AuthorityInstrumentDrafted` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AuthorityInstrumentActivated` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AuthorityInstrumentAmended` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AuthorityInstrumentSuspended` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AuthorityInstrumentRevoked` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AuthorityInstrumentExpired` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/governance-administration/authority
```
...
```text
packages/erp/corporate-administration/src/features/governance-administration/authority/
...
Primary proposed metric: percentage of active authority records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `authority` as the canonical feature identifier under `governance-administration` | proposed |
