# Product Requirements Document: Meetings

> **Document purpose.**
> This PRD defines the `governance-administration/meetings` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-GOVERNANCE-MEETINGS-001
feature_id: meetings
...
feature_source_path: src/features/governance-administration/meetings
prd_path: docs/erp/corporate-administration/feature-specs/governance-administration/meetings/PRD.md
...
| Feature identifier | `meetings` |
| Feature group | `governance-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for notice, agenda, attendance, quorum, adjournment, and closure |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for meetings |
...
> Notice, agenda, attendance, quorum, adjournment, and closure.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Meetings enables authorized users to:
...
This feature solely owns the meaning and lifecycle of: `Meeting, AgendaItem, Attendance`.
...
| governance body | governance-administration/governance-bodies | opaque body identifier | same organization |
...
| --- | --- | --- |
| `meetings` | Notice, agenda, attendance, quorum, adjournment, and closure | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `meetings.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `Meeting` | aggregate | `Meeting` | Governance meeting and lifecycle |
| `AgendaItem` | entity | `Meeting` | Ordered meeting subject |
| `Attendance` | entity | `Meeting` | Attendance and quorum evidence |
...
| `noticed` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `convened` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `adjourned` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `schedule` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.meetings.manage` | no |
| TR-02 | Current permitted state | `issue-notice` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.meetings.manage` | no |
| TR-03 | Current permitted state | `record-attendance` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.meetings.manage` | no |
| TR-04 | Current permitted state | `adjourn` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.meetings.manage` | no |
| TR-05 | Current permitted state | `close` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.meetings.manage` | no |
| TR-06 | Current permitted state | `cancel` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.meetings.manage` | no |
...
Feature invariants use `CA-MEETINGS-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.governance-administration.meetings.schedule` | `schedule` | command | `ScheduleMeetingInput` | `MeetingView` | `ca.meetings.manage` | no | required | request ID | yes | `MeetingScheduled` |
| `ca.governance-administration.meetings.issue-notice` | `issue_notice` | command | `IssueNoticeMeetingInput` | `MeetingView` | `ca.meetings.manage` | no | required | request ID | yes | `MeetingScheduled` |
| `ca.governance-administration.meetings.record-attendance` | `record_attendance` | command | `RecordAttendanceMeetingInput` | `MeetingView` | `ca.meetings.manage` | no | required | request ID | yes | `MeetingScheduled` |
| `ca.governance-administration.meetings.adjourn` | `adjourn` | command | `AdjournMeetingInput` | `MeetingView` | `ca.meetings.manage` | no | required | request ID | yes | `MeetingScheduled` |
| `ca.governance-administration.meetings.close` | `close` | command | `CloseMeetingInput` | `MeetingView` | `ca.meetings.manage` | no | required | request ID | yes | `MeetingScheduled` |
| `ca.governance-administration.meetings.cancel` | `cancel` | command | `CancelMeetingInput` | `MeetingView` | `ca.meetings.manage` | no | required | request ID | yes | `MeetingScheduled` |
| `ca.governance-administration.meetings.get` | `get` | query | `GetMeetingInput` | `MeetingView` | `ca.meetings.read` | — | none | — | no | — |
| `ca.governance-administration.meetings.list` | `list` | query | `ListMeetingInput` | `Page<MeetingView>` | `ca.meetings.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.meetings.read` | Approved get and list queries |
| `ca.meetings.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `MeetingScheduled` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `MeetingNoticeIssued` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `MeetingConvened` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `MeetingAdjourned` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `MeetingClosed` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `MeetingCancelled` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/governance-administration/meetings
```
...
```text
packages/erp/corporate-administration/src/features/governance-administration/meetings/
...
Primary proposed metric: percentage of active meetings records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `meetings` as the canonical feature identifier under `governance-administration` | proposed |
