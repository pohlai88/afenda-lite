# Product Requirements Document: Statutory Filings

> **Document purpose.**
> This PRD defines the `compliance-administration/statutory-filings` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-COMPLIANCE-STATUTORY_FILINGS-001
feature_id: statutory-filings
feature_name: Statutory Filings
feature_group: compliance-administration
...
feature_source_path: src/features/compliance-administration/statutory-filings
prd_path: docs/erp/corporate-administration/feature-specs/compliance-administration/statutory-filings/PRD.md
...
required_ports: []
emitted_events:
  - StatutoryFilingRegistered
  - FilingPreparationStarted
  - StatutoryFilingMarkedReady
  - StatutoryFilingSubmitted
  - StatutoryFilingAcknowledged
  - StatutoryFilingRejected
  - StatutoryFilingWithdrawn
consumed_by: []
estimated_slices: 11
```
...
| Field | Value |
| --- | --- |
| Feature name | Statutory Filings |
| Feature identifier | `statutory-filings` |
| Feature group | `compliance-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for filing requirements, preparation, submission, acknowledgement, and evidence |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for statutory filings |
...
> Filing requirements, preparation, submission, acknowledgement, and evidence.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Statutory Filings enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for statutory filings when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `StatutoryFiling, FilingSubmission`.
...
| --- | --- | --- |
| `statutory-filings` | Filing requirements, preparation, submission, acknowledgement, and evidence | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `statutory-filings.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `StatutoryFiling` | aggregate | `StatutoryFiling` | Required statutory filing lifecycle |
| `FilingSubmission` | history | `StatutoryFiling` | Submission and acknowledgement chronology |
...
| --- | --- | --- | --- |
| `required` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `preparing` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `ready` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `submitted` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `acknowledged` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `rejected` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `withdrawn` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |

## 8.2 Proposed transition catalog

| Transition ID | From | Trigger | To | Guard | Permission | Approval |
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `register` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.statutory-filings.manage` | no |
| TR-02 | Current permitted state | `start-preparation` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.statutory-filings.manage` | no |
| TR-03 | Current permitted state | `mark-ready` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.statutory-filings.manage` | no |
| TR-04 | Current permitted state | `record-submission` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.statutory-filings.manage` | no |
| TR-05 | Current permitted state | `record-acknowledgement` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.statutory-filings.manage` | no |
| TR-06 | Current permitted state | `record-rejection` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.statutory-filings.manage` | no |
| TR-07 | Current permitted state | `withdraw` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.statutory-filings.manage` | no |
...
Feature invariants use `CA-STATUTORY_FILINGS-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.compliance-administration.statutory-filings.register` | `register` | command | `RegisterStatutoryFilingInput` | `StatutoryFilingView` | `ca.statutory-filings.manage` | no | required | request ID | yes | `StatutoryFilingRegistered` |
| `ca.compliance-administration.statutory-filings.start-preparation` | `start_preparation` | command | `StartPreparationStatutoryFilingInput` | `StatutoryFilingView` | `ca.statutory-filings.manage` | no | required | request ID | yes | `StatutoryFilingRegistered` |
| `ca.compliance-administration.statutory-filings.mark-ready` | `mark_ready` | command | `MarkReadyStatutoryFilingInput` | `StatutoryFilingView` | `ca.statutory-filings.manage` | no | required | request ID | yes | `StatutoryFilingRegistered` |
| `ca.compliance-administration.statutory-filings.record-submission` | `record_submission` | command | `RecordSubmissionStatutoryFilingInput` | `StatutoryFilingView` | `ca.statutory-filings.manage` | no | required | request ID | yes | `StatutoryFilingRegistered` |
| `ca.compliance-administration.statutory-filings.record-acknowledgement` | `record_acknowledgement` | command | `RecordAcknowledgementStatutoryFilingInput` | `StatutoryFilingView` | `ca.statutory-filings.manage` | no | required | request ID | yes | `StatutoryFilingRegistered` |
| `ca.compliance-administration.statutory-filings.record-rejection` | `record_rejection` | command | `RecordRejectionStatutoryFilingInput` | `StatutoryFilingView` | `ca.statutory-filings.manage` | no | required | request ID | yes | `StatutoryFilingRegistered` |
| `ca.compliance-administration.statutory-filings.withdraw` | `withdraw` | command | `WithdrawStatutoryFilingInput` | `StatutoryFilingView` | `ca.statutory-filings.manage` | no | required | request ID | yes | `StatutoryFilingRegistered` |
| `ca.compliance-administration.statutory-filings.get` | `get` | query | `GetStatutoryFilingInput` | `StatutoryFilingView` | `ca.statutory-filings.read` | — | none | — | no | — |
| `ca.compliance-administration.statutory-filings.list` | `list` | query | `ListStatutoryFilingInput` | `Page<StatutoryFilingView>` | `ca.statutory-filings.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.statutory-filings.read` | Approved get and list queries |
| `ca.statutory-filings.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `StatutoryFilingRegistered` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `FilingPreparationStarted` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `StatutoryFilingMarkedReady` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `StatutoryFilingSubmitted` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `StatutoryFilingAcknowledged` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `StatutoryFilingRejected` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `StatutoryFilingWithdrawn` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/compliance-administration/statutory-filings
```
...
```text
packages/erp/corporate-administration/src/features/compliance-administration/statutory-filings/
...
Primary proposed metric: percentage of active statutory filings records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `statutory-filings` as the canonical feature identifier under `compliance-administration` | proposed |
