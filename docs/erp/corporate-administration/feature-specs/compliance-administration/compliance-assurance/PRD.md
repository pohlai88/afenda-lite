# Product Requirements Document: Compliance Assurance

> **Document purpose.**
> This PRD defines the `compliance-administration/compliance-assurance` feature within Corporate Administration. It translates the approved domain architecture into an executable product and engineering contract without redefining module architecture.
>
...
```yaml
prd_id: CA-COMPLIANCE-COMPLIANCE_ASSURANCE-001
feature_id: compliance-assurance
feature_name: Compliance Assurance
...
feature_source_path: src/features/compliance-administration/compliance-assurance
prd_path: docs/erp/corporate-administration/feature-specs/compliance-administration/compliance-assurance/PRD.md
...
required_ports: []
emitted_events:
  - AssuranceReviewPlanned
  - AssuranceReviewStarted
  - AssuranceExceptionRecorded
  - RemediationAssigned
  - AssuranceExceptionResolved
  - AssuranceReviewClosed
  - AssuranceReviewCancelled
consumed_by: []
estimated_slices: 11
```
...
| Field | Value |
| --- | --- |
| Feature name | Compliance Assurance |
| Feature identifier | `compliance-assurance` |
| Feature group | `compliance-administration` |
| Domain | Corporate Administration |
| One-line definition | The system of record for administrative reviews, exceptions, remediation, and evidence completeness |
...
| Trigger for existence | The organization requires one authoritative, tenant-safe chronology for compliance assurance |
...
> Administrative reviews, exceptions, remediation, and evidence completeness.

This PRD must not materially widen that responsibility.

## 1.2 Feature purpose

Compliance Assurance enables authorized users to:
...
Administration users cannot reliably determine the current, authorized, organization-scoped truth for compliance assurance when records, dates, ownership, lifecycle decisions, and evidence are distributed or inconsistently maintained, resulting in missed obligations, ambiguous accountability, weak chronology, and costly assurance reconstruction.
...
This feature solely owns the meaning and lifecycle of: `AssuranceReview, AssuranceException, RemediationAction`.
...
| --- | --- | --- |
| `compliance-assurance` | Administrative reviews, exceptions, remediation, and evidence completeness | Generic “admin item” or another module’s business term |
...
Operation definitions live in `operation-registry.ts`; public outcomes use `Result<Data, Code>`; production persistence uses `compliance-assurance.drizzle.ts`; business operations remain in the feature root; events use completed facts in past tense.
...
| --- | --- | --- | --- |
| `AssuranceReview` | aggregate | `ComplianceAssurance` | Administrative compliance review |
| `AssuranceException` | entity | `ComplianceAssurance` | Control or evidence exception |
| `RemediationAction` | entity | `ComplianceAssurance` | Corrective action and closure evidence |
...
| `in-review` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `exception-open` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
| `remediation` | Proposed lifecycle state; exact entry and exit guards require business approval | Defined by approved operations | no |
...
| --- | --- | --- | --- | --- | --- | --- |
| TR-01 | Current permitted state | `plan` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.compliance-assurance.manage` | no |
| TR-02 | Current permitted state | `start` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.compliance-assurance.manage` | no |
| TR-03 | Current permitted state | `record-exception` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.compliance-assurance.manage` | no |
| TR-04 | Current permitted state | `assign-remediation` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.compliance-assurance.manage` | no |
| TR-05 | Current permitted state | `resolve-exception` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.compliance-assurance.manage` | no |
| TR-06 | Current permitted state | `close` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.compliance-assurance.manage` | no |
| TR-07 | Current permitted state | `cancel` | Resulting state defined by approved lifecycle policy | Authorization, tenant lineage, required evidence, and expected version | `ca.compliance-assurance.manage` | no |
...
Feature invariants use `CA-COMPLIANCE_ASSURANCE-INV-nn` and must include tenant ownership, one semantic owner, chronology preservation, adapter parity, and no secret storage.

# 10. Operation catalog

| Operation ID | Method | Kind | Input | Output | Permission | Approval | Txn | Idempotency | Audit | Emits |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ca.compliance-administration.compliance-assurance.plan` | `plan` | command | `PlanComplianceAssuranceInput` | `ComplianceAssuranceView` | `ca.compliance-assurance.manage` | no | required | request ID | yes | `AssuranceReviewPlanned` |
| `ca.compliance-administration.compliance-assurance.start` | `start` | command | `StartComplianceAssuranceInput` | `ComplianceAssuranceView` | `ca.compliance-assurance.manage` | no | required | request ID | yes | `AssuranceReviewPlanned` |
| `ca.compliance-administration.compliance-assurance.record-exception` | `record_exception` | command | `RecordExceptionComplianceAssuranceInput` | `ComplianceAssuranceView` | `ca.compliance-assurance.manage` | no | required | request ID | yes | `AssuranceReviewPlanned` |
| `ca.compliance-administration.compliance-assurance.assign-remediation` | `assign_remediation` | command | `AssignRemediationComplianceAssuranceInput` | `ComplianceAssuranceView` | `ca.compliance-assurance.manage` | no | required | request ID | yes | `AssuranceReviewPlanned` |
| `ca.compliance-administration.compliance-assurance.resolve-exception` | `resolve_exception` | command | `ResolveExceptionComplianceAssuranceInput` | `ComplianceAssuranceView` | `ca.compliance-assurance.manage` | no | required | request ID | yes | `AssuranceReviewPlanned` |
| `ca.compliance-administration.compliance-assurance.close` | `close` | command | `CloseComplianceAssuranceInput` | `ComplianceAssuranceView` | `ca.compliance-assurance.manage` | no | required | request ID | yes | `AssuranceReviewPlanned` |
| `ca.compliance-administration.compliance-assurance.cancel` | `cancel` | command | `CancelComplianceAssuranceInput` | `ComplianceAssuranceView` | `ca.compliance-assurance.manage` | no | required | request ID | yes | `AssuranceReviewPlanned` |
| `ca.compliance-administration.compliance-assurance.get` | `get` | query | `GetComplianceAssuranceInput` | `ComplianceAssuranceView` | `ca.compliance-assurance.read` | — | none | — | no | — |
| `ca.compliance-administration.compliance-assurance.list` | `list` | query | `ListComplianceAssuranceInput` | `Page<ComplianceAssuranceView>` | `ca.compliance-assurance.read` | — | none | — | no | — |
...
| Permission | Grants |
| --- | --- |
| `ca.compliance-assurance.read` | Approved get and list queries |
| `ca.compliance-assurance.manage` | Approved mutations |
...
| --- | --- | --- | --- | --- |
| `AssuranceReviewPlanned` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AssuranceReviewStarted` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AssuranceExceptionRecorded` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `RemediationAssigned` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AssuranceExceptionResolved` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AssuranceReviewClosed` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
| `AssuranceReviewCancelled` | Corresponding successful command | organization ID, aggregate ID, effective business facts, schema version | Declared application or module consumers | per aggregate |
...
```text
/o/{organizationSlug}/corporate-administration/compliance-administration/compliance-assurance
```
...
```text
packages/erp/corporate-administration/src/features/compliance-administration/compliance-assurance/
...
Primary proposed metric: percentage of active compliance assurance records that are current, assigned, within required dates, and complete with required evidence.
...
| --- | --- | --- |
| FD-01 | Use `compliance-assurance` as the canonical feature identifier under `compliance-administration` | proposed |
