...
| **Namespace** | `docs/frontend/adr/` (not Backend ADR-001) |
...
**Feed Farm Trade (FFT)** is the module name + engine + path identity on **Afenda-Lite** (Declarations | FFT — shared AdminCN/auth/DB/env/CI; not a second app or infra course). Product name SSOT: [docs/adr/001-afenda-lite-product-identity.md](../../adr/001-afenda-lite-product-identity.md).
...
| Engine / env / ops docs | FFT — `FFT_*` env keys; living SSOT `docs/fft/` ([RUNTIME](../../fft/RUNTIME.md), [gate-register](../../fft/ops/gate-register.md)) |
