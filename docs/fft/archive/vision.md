# Product Requirement: Reusable **Feed Farm Trade Event** Website / ERP Service

## 1. Project Name

## **Reusable Feed Farm Trade Event Portal**

A web-based service connected or branchable from the ERP system, allowing the company to create, manage, and repeat **Feed Farm Trade / Spot-Selling events** with configurable rules, product categories, customer priority, quantity allocation, deposits, transfers, pickup, and reporting.
...
The website should allow Admin users to create a **Feed Farm Trade event** within the ERP without coding each time.
...
Each Feed Farm Trade event can have its own:

| Configurable Item | Example                                                    |
| ----------------- | ---------------------------------------------------------- |
| Event name        | GP2 Piglet Feed Farm Trade July 2026                             |
...
The business user who manages Feed Farm Trade events.

Permissions:

* Create new Feed Farm Trade event.
...
* View active Feed Farm Trade events.
...
The dashboard should show all Feed Farm Trade events.

### Required Dashboard Information

| Field                    | Description                                                                              |
| ------------------------ | ---------------------------------------------------------------------------------------- |
| Event name               | Name of the Feed Farm Trade event                                                              |
...
For the current Feed Farm Trade case:
...
> This Feed Farm Trade program is standalone and will not be affected by other routine policies, trade-offs, rebates, incentives, promotions, or existing programs.
...
The Feed Farm Trade website should be designed as an ERP branch service.
...
| ERP Data                    | Direction                                  |
| --------------------------- | ------------------------------------------ |
| User accounts               | ERP → Feed Farm Trade                            |
| Sales team structure        | ERP → Feed Farm Trade                            |
| Customer master data        | ERP → Feed Farm Trade                            |
| Customer priority           | ERP → Feed Farm Trade or maintained in Feed Farm Trade |
| Product master data         | ERP → Feed Farm Trade                            |
| Inventory / supply quantity | ERP / Operation → Feed Farm Trade                |
| Sales order creation        | Feed Farm Trade → ERP                            |
| Deposit / payment status    | ERP / Finance → Feed Farm Trade                  |
| Invoice / billing           | ERP → Feed Farm Trade                            |
| Fulfillment status          | ERP / Operation → Feed Farm Trade                |
| Final support amount        | Feed Farm Trade → ERP / Finance                  |
...
* Admin can create a new Feed Farm Trade event without developer support.
...
# 28. Example Event Configuration for Current Piglet Feed Farm Trade

## Event Name

**GP2 Piglet Feed Farm Trade — July to September 2026**

## Event Rule

| Item                            | Setting                                    |
| ------------------------------- | ------------------------------------------ |
| Event type                      | Feed Farm Trade / Spot-Selling                   |
...
```text
Build a reusable Feed Farm Trade Event web application as a branch service from our ERP.
...
Build a reusable **Feed Farm Trade Event Engine**.
