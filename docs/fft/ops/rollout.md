# Feed Farm Trade Phase 2A RBAC — Ops Rollout Checklist
...
- [x] Capture evidence — [gate register § Gate 6](./gate-register.md#gate-6--controlled-rbac-matrix-local-only); code under test `51e9a5b`; register closeout `25c3891`; local `.env` restored to `FFT_RBAC_ENABLED=false`
...
Post-tag hotfix `4d203a7` (FftShell next-intl) must be on `main` before the next production deploy — see [gate register](./gate-register.md#hotfix-merge-requirement).
