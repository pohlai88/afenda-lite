...
**Skim banner:** Gates 1–6 historical evidence may show `FFT_RBAC_ENABLED=false`. **Current production** is Gate 7 — `FFT_RBAC_ENABLED=true`. Platform hard tenancy (ADR-002 / `027`) is separate from this FFT domain flag — see [multi-tenant-ecosystem.md](../../architecture/multi-tenant-ecosystem.md).
