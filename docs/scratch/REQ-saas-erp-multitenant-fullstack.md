...
Positive/adverse/recovery detail for REQ-T-OPS-006: a deployment missing the e-invoicing vendor key still serves every non-e-invoicing workflow normally (**positive**); a user attempting an e-invoicing-dependent action against that deployment receives a defined "integration not configured" response, not a 500 or a silent no-op (**adverse**); adding the missing key through the approved environment-variable promotion process and redeploying the same build artifact re-enables the capability on the next request after that redeploy — this is a controlled deployment-configuration change per ARCH-027's Target `@afenda/env` model, not a dynamic/hot-reloaded configuration service — and no data written while the integration was unavailable is lost or corrupted, while unrelated capabilities remain healthy across the redeploy (**recovery**).
