...
- **Phase I1** (identity & edge) — **I1.1–I1.4** closed; **I2.1–I2.4** closed; **I3.1** closed (roles · assignments · RBAC audit beyond list ports · Tier-2 `hasPermission`); next Ops mission **I3.2**
...
| **I3** | Product verticals | PARTIAL (I3.1 DONE) | Ops | Identity · Declarations · FFT read (freeze) |
...
| | |
| --- | --- |
| **Status** | PARTIAL (next **I3.2**) |
| **Lane** | Ops (+ readiness) |
| **Goal** | Complete allowed product paths inside the FFT freeze envelope |

| Stage | Outcome | Boundary | Status |
| ----- | ------- | -------- | ------ |
| **I3.1** Identity / Platform | Roles · assignments · RBAC audit beyond list ports | Platform / MOD | **DONE** 2026-07-17 |
| **I3.2** Declarations | Client list → submit/read under hard tenancy | Declarations MOD | WAIT |
| **I3.3** FFT (frozen) | Operator read shell + Phase 2A RBAC only | FFT-MOD-008 Allowed/Forbidden | WAIT |
| **I3.4** Org-admin shell | Operator UX composes Identity/Platform ports | AdminCN only if shell work needs it | WAIT |
...
| UI | Org-admin LIST_ONLY lifted for assign/revoke; Neon `userId` field (no member-directory port); role names resolved from assignable catalog |
...
| Out-of-bar | Authenticated browser assign/invite E2E → **I4**; Neon member-directory picker → later org-admin deepen; Declarations → **I3.2** |
...
| Authn / authz | PARTIAL | Session helpers · `proxy.ts` · `/auth/*` · `/join` · invite · coarse shells · Tier-2 `hasPermission` on invite/assign/revoke | Full permission-matrix surfaces · authenticated browser E2E |
| Data / tenancy | PARTIAL | Drizzle schema · `withOrg` reads · hard-org writes (`recordRbacAudit` · role assign/revoke) | Broader mutation matrix · full two-org isolation suite · restore · migrate discipline |
| Domains | PARTIAL | List + assignable roles · invite/assign/revoke + audit · feature → domain → db Vitest gate | Declarations deepen (I3.2) · adverse journeys · FFT beyond freeze = FROZEN |
