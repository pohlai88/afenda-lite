...
| **Living inventory** | [ARCH-023](../../architecture/turborepo/ARCH-023-multi-tenancy.md) — architecture SSOT: Neon tenancy model choice, config inventory, Neon production efficiency (§6), residual (D4 deferred M5; D5 accepted non-goal; D7–D8 closed), multi-org ready M1–M4 · efficiency ladder A–E closed 2026-07-12 |

**Related:** [ADR-001](./ADR-001-modular-monolith-hexagonal.md) · [ADR-006](../../modules/feed-farm-trade/adr/ADR-006-feed-farm-trade-rbac.md) · Neon Auth Organization plugin (owner/admin/member fixed)
...
- Post-v1 multi-org requirements live in [ARCH-023](../../architecture/turborepo/ARCH-023-multi-tenancy.md) — do not claim “multi-org ready” until that DoD.
...
| Project-per-tenant as default / efficiency fix | Conflicts with one Modular Monolith + one Vercel deploy; reopen only for compliance / per-tenant PITR (ARCH-023 Decision lock **R5/D5**) |
| First-org `ORDER BY … LIMIT 1` stamp | Wrong-tenant risk; use active/sole membership or explicit `--organization-id` |

**Coding freeze pointer:** [ARCH-023 Decision lock](../../architecture/turborepo/ARCH-023-multi-tenancy.md) — Rejected / Deferred / Allowed next (2026-07-12).
