...
| Layer | Path |
|-------|------|
| Kernel SSOT | root-only `@afenda/errors` capability facade |
| Web adapters | `apps/web/modules/platform/schemas/action-result.ts` · `api-error.ts` |

| Outcome | Shape |
|---------|-------|
| Success | `{ ok: true, data: T }` |
| Failure | `{ ok: false, code, message, messageKey, details? }` |
| Helpers | `errorResult` · `errorIngress` · `errorProject` |

Expected failures return `{ ok: false }` — throw only for unexpected bugs. Never tutorial `{ success, data }`. The root `@afenda/errors` facade owns normalization and every shared projection; consumers do not construct `AppError` or interpret codes. Closed codes include `RATE_LIMITED` (429) and `SERVICE_UNAVAILABLE` (503); Route Handlers derive `Retry-After` through `errorProject.http`. Auth BFF also stamps `X-RateLimit-*` + `Server-Timing` — see [middleware-dna.md](middleware-dna.md). Detailed authority: [errors/architecture.md](errors/architecture.md).
...
| Correlation | Unexpected Action failure → `errorResult.fail("INTERNAL_ERROR", { correlationId })` |
