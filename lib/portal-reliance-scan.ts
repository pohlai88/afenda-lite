...
function componentSpecifierToPath(specifier: string): string | null {
  if (!specifier.startsWith("@/components/")) {
    return null;
  }

  const relative = specifier.slice("@/components/".length);
  const base = `components/${relative}`;

  if (existsSync(join(process.cwd(), `${base}.tsx`))) {
    return `${base}.tsx`;
  }
  if (existsSync(join(process.cwd(), `${base}.ts`))) {
    return `${base}.ts`;
  }
  if (existsSync(join(process.cwd(), base, "index.tsx"))) {
...
export function collectTransitiveSourceFiles(
  ...
  while (queue.length > 0) {
    ...
    for (const spec of extractComponentImports(content)) {
      const resolved = componentSpecifierToPath(spec);
