...
const CA_MODULE = "corporate-administration" as const;
...
export function createProductionMutationPorts(): MutationPorts {
	const audit = createSqlAuditFactPort();
	const outbox = createSqlOutboxPort();
	return {
		audit,
		outbox,
		async record() {
			return {
				ok: false,
				code: "INTERNAL_ERROR",
				message: "Production CA mutations require a transaction-scoped store",
			};
		},
	};
}
