...
import {
	...
	encodePremiseCursor,
	type LegalEstablishmentCursorKey,
	legalEstablishmentCursorScope,
	type PremiseCursorKey,
	premiseCursorScope,
} from "../pagination";
import {
...
export function createMemoryCorporateAdministrationEstablishmentStore(): EstablishmentStore {
	...
	return {
		...
		async listLegalEstablishmentsAsOf(input) {
			...
			const ordered = [...establishments.values()]
				.filter(
					(row) =>
				...
				.filter(
					(row) =>
						cursor.data === null ||
						compareLegalEstablishmentCursor(row, cursor.data) > 0,
				)
		...
		async listPremisesAsOf(input) {
			...
			const ordered = [...premises.values()]
				...
				.filter(
					(row) =>
						cursor.data === null || comparePremiseCursor(row, cursor.data) > 0,
				)
				.slice(0, input.pagination.limit + 1);
			const pageRows = ordered.slice(0, input.pagination.limit);
			const last = pageRows.at(-1);
...
function compareLegalEstablishmentCursor(
	row: LegalEstablishment,
	cursor: LegalEstablishmentCursorKey,
): number {
	return (
		row.establishmentType.localeCompare(cursor[0]) ||
		row.jurisdictionCode.localeCompare(cursor[1]) ||
		row.normalizedRegistrationIdentifier.localeCompare(cursor[2]) ||
		row.id.localeCompare(cursor[3])
	);
}

function comparePremiseCursor(row: Premise, cursor: PremiseCursorKey): number {
	return (
		row.premiseType.localeCompare(cursor[0]) ||
		row.displayName.localeCompare(cursor[1]) ||
		row.id.localeCompare(cursor[2])
	);
}
