...
import {
	...
	type LegalCompanyTimelineCursorKey,
	legalCompanyCursorScope,
	legalCompanyTimelineCursorKey,
	legalCompanyTimelineCursorScope,
	legalCompanyTimelineKindRank,
} from "../pagination";
import { isVisibleAtKnownTime, matchesAsOf } from "../rules";
import {
		...
		CompanyActivityStore
{
	...
	async listLegalCompanies(
		...
		try {
			...
			const cursor = decodeLegalCompanyCursor(
			...
			if (cursor.data !== null) {
				conditions.push(
					sql`(${caLegalCompany.normalizedCompanyCode}, ${caLegalCompany.id}) > (${cursor.data[0]}, ${cursor.data[1]})`,
				);
			}
			const rows = await this.#database
				.select()
				...
				.orderBy(
					asc(caLegalCompany.normalizedCompanyCode),
					asc(caLegalCompany.id),
				)
				...
				nextCursor:
					rows.length > input.pagination.limit && last !== undefined
						? encodeLegalCompanyCursor(cursorScope, [
								...
								legalCompanyIdSchema.parse(last.id),
							])
						: null,
			});
		} catch (error) {
	...
	async listCompanyNames(
		...
		try {
			...
			const cursor = decodeCompanyNameCursor(query.cursor, cursorScope);
			if (!cursor.ok) {
				return cursor;
			}
			const conditions = [
			...
			if (cursor.data !== null) {
				conditions.push(
					sql`(
						${caCompanyName.nameType} > ${cursor.data[0]}
						OR (${caCompanyName.nameType} = ${cursor.data[0]} AND ${caCompanyName.languageCode} > ${cursor.data[1]})
						OR (${caCompanyName.nameType} = ${cursor.data[0]} AND ${caCompanyName.languageCode} = ${cursor.data[1]} AND ${caCompanyName.effectiveFrom} < ${cursor.data[2]})
						OR (${caCompanyName.nameType} = ${cursor.data[0]} AND ${caCompanyName.languageCode} = ${cursor.data[1]} AND ${caCompanyName.effectiveFrom} = ${cursor.data[2]} AND ${caCompanyName.recordedAt} < ${cursor.data[3]})
						OR (${caCompanyName.nameType} = ${cursor.data[0]} AND ${caCompanyName.languageCode} = ${cursor.data[1]} AND ${caCompanyName.effectiveFrom} = ${cursor.data[2]} AND ${caCompanyName.recordedAt} = ${cursor.data[3]} AND ${caCompanyName.id} > ${cursor.data[4]})
					)`,
				);
			}
			...
			return errorResult.ok({
				...
				nextCursor:
					rows.length > pageSize && last !== undefined
						? encodeCompanyNameCursor(cursorScope, [
								companyNameTypeSchema.parse(last.nameType),
								last.languageCode,
								canonicalDateSchema.parse(last.effectiveFrom),
								toCanonicalInstant(last.recordedAt),
								companyNameIdSchema.parse(last.id),
							])
						: null,
			});
		} catch (error) {
	...
	async listCompanyIdentifiers(
		...
		try {
			...
			const cursor = decodeCompanyIdentifierCursor(query.cursor, cursorScope);
			if (!cursor.ok) {
				return cursor;
			}
			...
			if (cursor.data !== null) {
				conditions.push(
					sql`(
						${caCompanyIdentifier.identifierType} > ${cursor.data[0]}
						OR (${caCompanyIdentifier.identifierType} = ${cursor.data[0]} AND ${caCompanyIdentifier.jurisdictionCode} > ${cursor.data[1]})
						OR (${caCompanyIdentifier.identifierType} = ${cursor.data[0]} AND ${caCompanyIdentifier.jurisdictionCode} = ${cursor.data[1]} AND ${caCompanyIdentifier.issuingAuthorityCode} > ${cursor.data[2]})
						OR (${caCompanyIdentifier.identifierType} = ${cursor.data[0]} AND ${caCompanyIdentifier.jurisdictionCode} = ${cursor.data[1]} AND ${caCompanyIdentifier.issuingAuthorityCode} = ${cursor.data[2]} AND ${caCompanyIdentifier.effectiveFrom} < ${cursor.data[3]})
						OR (${caCompanyIdentifier.identifierType} = ${cursor.data[0]} AND ${caCompanyIdentifier.jurisdictionCode} = ${cursor.data[1]} AND ${caCompanyIdentifier.issuingAuthorityCode} = ${cursor.data[2]} AND ${caCompanyIdentifier.effectiveFrom} = ${cursor.data[3]} AND ${caCompanyIdentifier.recordedAt} < ${cursor.data[4]})
						OR (${caCompanyIdentifier.identifierType} = ${cursor.data[0]} AND ${caCompanyIdentifier.jurisdictionCode} = ${cursor.data[1]} AND ${caCompanyIdentifier.issuingAuthorityCode} = ${cursor.data[2]} AND ${caCompanyIdentifier.effectiveFrom} = ${cursor.data[3]} AND ${caCompanyIdentifier.recordedAt} = ${cursor.data[4]} AND ${caCompanyIdentifier.id} > ${cursor.data[5]})
					)`,
				);
			}
			const pageSize = query.pageSize ?? 50;
			...
			return errorResult.ok({
				...
				nextCursor:
					rows.length > pageSize && last !== undefined
						? encodeCompanyIdentifierCursor(cursorScope, [
								companyIdentifierTypeSchema.parse(last.identifierType),
								last.jurisdictionCode,
								companyIdentifierAuthoritySchema.parse(
									last.issuingAuthorityCode,
								),
								canonicalDateSchema.parse(last.effectiveFrom),
								toCanonicalInstant(last.recordedAt),
								companyIdentifierIdSchema.parse(last.id),
							])
						: null,
			});
		} catch (error) {
	...
	async listCompanyActivitiesAsOf(
		...
		try {
			...
			const cursor = decodeCompanyActivityCursor(query.cursor, cursorScope);
			if (!cursor.ok) {
				return cursor;
			}
			const conditions = buildCompanyActivityAsOfConditions(query);
			if (cursor.data !== null) {
				conditions.push(
					sql`(
						${caCompanyActivity.classification} > ${cursor.data[0]}
						OR (${caCompanyActivity.classification} = ${cursor.data[0]} AND ${caCompanyActivity.activityCode} > ${cursor.data[1]})
						OR (${caCompanyActivity.classification} = ${cursor.data[0]} AND ${caCompanyActivity.activityCode} = ${cursor.data[1]} AND ${caCompanyActivity.effectiveFrom} > ${cursor.data[2]})
						OR (${caCompanyActivity.classification} = ${cursor.data[0]} AND ${caCompanyActivity.activityCode} = ${cursor.data[1]} AND ${caCompanyActivity.effectiveFrom} = ${cursor.data[2]} AND ${caCompanyActivity.id} > ${cursor.data[3]})
					)`,
				);
			}
			const pageSize = query.pageSize ?? 50;
			const rows = await this.#database
				...
				.limit(pageSize + 1);
			const pageRows = rows.slice(0, pageSize);
			const items: CompanyActivityListItem[] = [];
			for (const row of pageRows) {
				const mapped = mapCompanyActivityRow(row);
				if (!mapped.ok) {
					return mapped;
				}
				items.push(toCompanyActivityListItem(mapped.data));
			}
			const last = pageRows.at(-1);
			return errorResult.ok({
				items,
				nextCursor:
					rows.length > pageSize && last !== undefined
						? encodeCompanyActivityCursor(cursorScope, [
								last.classification === "registered_object" ||
								last.classification === "regulated" ||
								last.classification === "operational"
									? last.classification
									: "operational",
								last.activityCode,
								canonicalDateSchema.parse(last.effectiveFrom),
								companyActivityIdSchema.parse(last.id),
							])
						: null,
			});
		} catch (error) {
	...
	async findOverlappingCompanyActivity(
		query: CompanyActivityOverlapQuery,
	): Promise<Result<CompanyActivity | null>> {
		try {
			...
			const rows = await this.#database
				...
				.where(
					and(
						...
						eq(caCompanyActivity.classification, query.classification),
						eq(caCompanyActivity.activityCode, query.activityCode),
						eq(caCompanyActivity.jurisdictionCode, query.jurisdictionCode),
						eq(caCompanyActivity.status, "active"),
						sql`${caCompanyActivity.effectiveFrom} < ${candidateTo} AND ${query.effectivePeriod.from} < COALESCE(${caCompanyActivity.effectiveTo}, '9999-12-31'::date)`,
					),
				)
				.orderBy(
					asc(caCompanyActivity.effectiveFrom),
	...
	async listCompaniesByStatus(
		...
		try {
			...
			if (query.asOf === undefined) {
				conditions.push(eq(caLegalCompany.state, query.status));
			} else {
				const companyOrganizationId = sql.raw(
					'"ca_legal_company"."organization_id"',
				);
				const companyId = sql.raw('"ca_legal_company"."id"');
				const knownAtPredicate =
					query.knownAt === undefined
						? sql``
						: sql`AND history.recorded_at <= ${new Date(query.knownAt)}`;
				const statusAsOf = sql<string | null>`(
					SELECT history.status
					FROM ca_company_status_history AS history
					WHERE history.organization_id = ${companyOrganizationId}
						AND history.legal_company_id = ${companyId}
						AND history.effective_from <= ${query.asOf}
						AND (history.effective_to IS NULL OR ${query.asOf} < history.effective_to)
						${knownAtPredicate}
					ORDER BY history.effective_from DESC, history.recorded_at DESC, history.id DESC
					LIMIT 1
				)`;
				conditions.push(sql`${statusAsOf} = ${query.status}`);
			}
			if (cursor.data !== null) {
				conditions.push(
					sql`(${caLegalCompany.normalizedCompanyCode}, ${caLegalCompany.id}) > (${cursor.data[0]}, ${cursor.data[1]})`,
			...
			const pageRows = rows.slice(0, query.pagination.limit);
			const items: LegalCompanyListItem[] = [];
			for (const row of pageRows) {
				const company = mapLegalCompanyRow(row);
				if (!company.ok) {
			...
			return errorResult.ok({
				items,
				nextCursor:
					rows.length > query.pagination.limit && last !== undefined
	...
	async getLegalCompanyTimeline(
		...
		try {
			...
			const profileConditions = [
				eq(caCompanyJurisdictionProfile.organizationId, input.organizationId),
				eq(caCompanyJurisdictionProfile.legalCompanyId, input.legalCompanyId),
			];
			...
			if (input.knownAt === undefined) {
				profileConditions.push(
					sql`${caCompanyJurisdictionProfile.supersededAt} IS NULL`,
				);
			} else {
				companyConditions.push(
					sql`${caLegalCompany.updatedAt} <= ${input.knownAt}`,
				);
				profileConditions.push(
					sql`${caCompanyJurisdictionProfile.recordedAt} <= ${input.knownAt} AND (${caCompanyJurisdictionProfile.supersededAt} IS NULL OR ${input.knownAt} < ${caCompanyJurisdictionProfile.supersededAt})`,
				);
				statusConditions.push(
					sql`${caCompanyStatusHistory.recordedAt} <= ${input.knownAt}`,
				);
			}
			if (cursor.data !== null) {
				companyConditions.push(
					timelineAfterCursorCondition(
						caLegalCompany.updatedAt,
						caLegalCompany.id,
						"profile",
						cursor.data,
					),
				...
				profileConditions.push(
					timelineAfterCursorCondition(
						caCompanyJurisdictionProfile.recordedAt,
						caCompanyJurisdictionProfile.id,
				...
				statusConditions.push(
					timelineAfterCursorCondition(
						caCompanyStatusHistory.recordedAt,
						caCompanyStatusHistory.id,
						"company_status",
						cursor.data,
					),
				);
			...
			const [companyRows, profileRows, statusRows] = await Promise.all([
				this.#database
					.select()
					.from(caLegalCompany)
					.where(and(...companyConditions))
					.orderBy(asc(caLegalCompany.updatedAt), asc(caLegalCompany.id))
					.limit(1),
				this.#database
					.select()
					.from(caCompanyJurisdictionProfile)
					...
					.orderBy(
						asc(caCompanyJurisdictionProfile.recordedAt),
						asc(caCompanyJurisdictionProfile.id),
					)
					.limit(pageSize + 1),
				this.#database
					.select()
					...
					.orderBy(
						asc(caCompanyStatusHistory.recordedAt),
						asc(caCompanyStatusHistory.id),
					)
					.limit(pageSize + 1),
			]);
			...
			if (companyRow !== undefined) {
				const company = mapLegalCompanyRow(companyRow);
				if (!company.ok) {
					return company;
				}
				entries.push({
			...
			for (const row of profileRows) {
				const profile = mapJurisdictionProfileRow(row);
				if (!profile.ok) {
					return profile;
				}
				entries.push({ ...profile.data, kind: "jurisdiction_profile" });
			}
			for (const row of statusRows) {
				entries.push({
					...mapCompanyStatusHistoryRow(row),
					kind: "company_status",
				});
			}
...
function timelineAfterCursorCondition(
	recordedAt: unknown,
	entryId: unknown,
	kind: LegalCompanyTimelineCursorKey[1],
	cursor: LegalCompanyTimelineCursorKey,
) {
	const kindRank = legalCompanyTimelineKindRank(kind);
	const cursorKindRank = legalCompanyTimelineKindRank(cursor[1]);
	if (kindRank < cursorKindRank) {
		return sql`${recordedAt} > ${cursor[0]}`;
	}
	if (kindRank > cursorKindRank) {
		return sql`${recordedAt} >= ${cursor[0]}`;
	}
	return sql`(${recordedAt} > ${cursor[0]} OR (${recordedAt} = ${cursor[0]} AND ${entryId} > ${cursor[2]}))`;
}
...
function buildCompanyActivityAsOfConditions(query: CompanyActivitiesAsOfQuery) {
	...
	if (query.classificationSystem !== undefined) {
		conditions.push(
			eq(caCompanyActivity.classificationSystem, query.classificationSystem),
		);
	}
	if (query.primaryOnly === true) {
		conditions.push(eq(caCompanyActivity.isPrimary, true));
	}
	if (query.knownAt !== undefined) {
