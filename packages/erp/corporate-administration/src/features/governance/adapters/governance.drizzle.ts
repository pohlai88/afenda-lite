...
class DrizzleCorporateAdministrationGovernanceStore implements GovernanceStore {
	...
	async listGovernanceBodiesAsOf(
		...
		return this.#read(async () => {
			const conditions = [
				...
				drizzleSql`${caGovernanceBody.effectiveFrom} <= ${input.asOf} AND ${input.asOf} < COALESCE(${caGovernanceBody.effectiveTo}, '9999-12-31'::date)`,
			];
			if (input.bodyType !== undefined) {
				conditions.push(eq(caGovernanceBody.bodyType, input.bodyType));
			}
			if (input.includeRetired !== true) {
				conditions.push(eq(caGovernanceBody.status, "active"));
			}
			if (cursor.data !== null) {
				conditions.push(
					drizzleSql`(
						${caGovernanceBody.bodyType} > ${cursor.data[0]}
						OR (${caGovernanceBody.bodyType} = ${cursor.data[0]} AND ${caGovernanceBody.normalizedBodyCode} > ${cursor.data[1]})
						OR (${caGovernanceBody.bodyType} = ${cursor.data[0]} AND ${caGovernanceBody.normalizedBodyCode} = ${cursor.data[1]} AND ${caGovernanceBody.id} > ${cursor.data[2]})
					)`,
				);
			}
			...
			const rows = await this.#database
				...
				.limit(pageSize + 1);
			const pageRows = rows.slice(0, pageSize).map(mapGovernanceBody);
			const last = pageRows.at(-1);
			return {
				items: pageRows,
				nextCursor:
					rows.length > pageSize && last !== undefined
						? encodeGovernanceBodyCursor(cursorScope, [
								last.bodyType,
								last.normalizedBodyCode,
								last.id,
							])
						: null,
			};
	...
	async listGovernanceMembershipsAsOf(
		...
		return this.#read(async () => {
			const conditions = [
				eq(caGovernanceMembership.organizationId, input.organizationId),
				eq(caGovernanceMembership.governanceBodyId, input.governanceBodyId),
				eq(caGovernanceMembership.status, "active"),
				drizzleSql`${caGovernanceMembership.termFrom} <= ${input.asOf} AND ${input.asOf} < COALESCE(${caGovernanceMembership.termTo}, '9999-12-31'::date)`,
			];
			if (input.memberPartyId !== undefined) {
				conditions.push(
					eq(caGovernanceMembership.memberPartyId, input.memberPartyId),
				);
			}
			const rows = await this.#database
				.select()
				...
				.where(and(...conditions))
				.orderBy(
					desc(caGovernanceMembership.isChair),
					asc(caGovernanceMembership.seatLabel),
	...
	async listGovernanceMembershipPageAsOf(
		input: Parameters<GovernanceStore["listGovernanceMembershipPageAsOf"]>[0],
	) {
		const cursorScope = governanceMembershipCursorScope(input);
		const cursor = decodeGovernanceMembershipCursor(input.cursor, cursorScope);
		if (!cursor.ok) {
			return cursor;
		}
		return this.#read(async () => {
			...
			if (cursor.data !== null) {
				const chairRank = cursor.data[0] ? 1 : 0;
				conditions.push(
					drizzleSql`(
						CASE WHEN ${caGovernanceMembership.isChair} THEN 1 ELSE 0 END < ${chairRank}
						OR (${caGovernanceMembership.isChair} = ${cursor.data[0]} AND ${caGovernanceMembership.seatLabel} > ${cursor.data[1]})
						OR (${caGovernanceMembership.isChair} = ${cursor.data[0]} AND ${caGovernanceMembership.seatLabel} = ${cursor.data[1]} AND ${caGovernanceMembership.id} > ${cursor.data[2]})
					)`,
				);
			}
			const pageSize = input.pageSize ?? 50;
			const rows = await this.#database
				...
				.limit(pageSize + 1);
			const pageRows = rows.slice(0, pageSize).map(mapGovernanceMembership);
			const last = pageRows.at(-1);
			return {
				items: pageRows,
				nextCursor:
					rows.length > pageSize && last !== undefined
						? encodeGovernanceMembershipCursor(cursorScope, [
								last.isChair,
								last.seatLabel,
								last.id,
							])
