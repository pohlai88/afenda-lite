...
class DrizzleCorporateAdministrationMeetingStore implements MeetingStore {
	...
	async listGovernanceMeetings(
		...
		return this.#read(async () => {
			...
			if (input.governanceBodyId !== undefined) {
				conditions.push(
					eq(caGovernanceMeeting.governanceBodyId, input.governanceBodyId),
				);
			}
			if (input.status !== undefined) {
				conditions.push(eq(caGovernanceMeeting.status, input.status));
			}
			if (cursor.data !== null) {
				conditions.push(
					drizzleSql`(
						${caGovernanceMeeting.scheduledStartAt} > ${cursor.data[0]}
						OR (${caGovernanceMeeting.scheduledStartAt} = ${cursor.data[0]} AND ${caGovernanceMeeting.id} > ${cursor.data[1]})
					)`,
				);
			}
			...
			const rows = await this.#database
				...
				.orderBy(
					asc(caGovernanceMeeting.scheduledStartAt),
					asc(caGovernanceMeeting.id),
				)
				.limit(pageSize + 1);
			const pageRows = rows.slice(0, pageSize).map(mapMeeting);
			const last = pageRows.at(-1);
			return {
				items: pageRows,
				nextCursor:
					rows.length > pageSize && last !== undefined
						? encodeGovernanceMeetingCursor(cursorScope, [
								canonicalInstantSchema.parse(
									last.scheduledStartAt.toISOString(),
								),
								last.id,
							])
						: null,
