import { type CanonicalDate, canonicalDateSchema } from "../../kernel/dates";
...
export function declarationExpiresWithin(input: {
	...
	return (
		input.declaration.expiresOn >= input.asOf &&
		input.declaration.expiresOn <= windowEnd
	);
}

export function declarationExpiryWindowEnd(input: {
	asOf: CanonicalDate;
	windowDays: number;
}): CanonicalDate {
	const end = new Date(`${input.asOf}T00:00:00.000Z`);
	end.setUTCDate(end.getUTCDate() + input.windowDays);
	return canonicalDateSchema.parse(end.toISOString().slice(0, 10));
}
