...
| CA-CL-03 Atomicity | DONE | Neon outbox-failure rollback proven for company, establishment, governance (`createGovernanceBody`), officer (`defineStatutoryOffice`), meeting (`scheduleGovernanceMeeting`), and resolution (`recordWrittenResolution`) exposed commands. Evidence (2026-08-05) on `ca-0-4-demo`, `AFENDA_DATABASE_TEST_TARGET=preview`, `REQUIRE_DATABASE_TESTS=1`: failure-injection specs under `__tests__/failure-injection/*-atomicity.test.ts` — each cohort 1 passed (body/office/meeting/resolution rows roll back; completed receipt=0; released receipt=1; outbox unchanged). Command input instants for schedule/written-resolution fingerprinted as `canonicalInstantSchema` (Date objects rejected by canonical JSON). Follow-on boundary-9 evidence (same day): `__tests__/database/ca-app-01-idempotency-replay.neon.test.ts` (5 passed — replay + fingerprint conflict for all five create commands; establishment stale version) and `__tests__/concurrency/ca-app-01-natural-key.concurrency.test.ts` (4 passed — establishment/governance/officer/resolution natural-key races). Schedule input now rejects hybrid/virtual without remote access summary so receipts remain replayable. |
