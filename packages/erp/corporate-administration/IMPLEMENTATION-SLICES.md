...
| CA-CL-03 Atomicity | PARTIAL | Company cohort Neon outbox-failure rollback already covered. Establishment cohort (2026-08-05): `registerLegalEstablishment` rolls back establishment row, completed receipt, and outbox when append fails (`__tests__/failure-injection/legal-establishment-atomicity.test.ts` on `ca-0-4-demo`, preview target, 1 passed). Governance cohort (2026-08-05): `createGovernanceBody` rolls back governance body row, completed receipt, and outbox when append fails (`__tests__/failure-injection/governance-body-atomicity.test.ts` on `ca-0-4-demo`, preview target, 1 passed). Still missing officer, meeting, and resolution exposed-command failure/concurrency evidence. |
