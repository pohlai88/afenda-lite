...
export function normalizeTaxRegistrationNumber(raw: string): Result<{
	...
	if (
		...
		return errorResult.fail("BAD_REQUEST", {
			publicMessage: "Invalid tax registration number length",
		});
	}
	...
	if (normalizedRegistrationNumber.length === 0) {
		return errorResult.fail("BAD_REQUEST", {
			publicMessage: "Tax registration number is empty after normalize",
		});
	}
	if (
		...
		return errorResult.fail("BAD_REQUEST", {
			publicMessage: "Normalized tax registration number too long",
		});
	}
	return {
		ok: true,
