import type {
	SalesOperationDeclaration,
	SalesOperationDeclarationRegistry,
} from "./types";
 ...
 */
export function defineSalesOperationRegistry<
	const TRegistry extends SalesOperationDeclarationRegistry,
>(registry: TRegistry): TRegistry {
	...
	for (const [publicName, definition] of Object.entries(registry)) {
		if (definition.publicName !== publicName) {
			throw new Error(
				`Sales operation registry key ${publicName} does not match public name ${definition.publicName}`,
			);
		}
		if (operationIds.has(definition.id)) {
			throw new Error(`Duplicate Sales operation id: ${definition.id}`);
		}
		if (publicNames.has(definition.publicName)) {
			throw new Error(
				`Duplicate Sales public operation name: ${definition.publicName}`,
			);
		}
...
export function composeSalesOperationRegistries(
	...registries: readonly SalesOperationDeclarationRegistry[]
): readonly SalesOperationDeclaration[] {
	...
	for (const definition of definitions) {
		if (operationIds.has(definition.id)) {
			throw new Error(
				`Duplicate composed Sales operation id: ${definition.id}`,
			);
		}
		if (publicNames.has(definition.publicName)) {
			throw new Error(
				`Duplicate composed Sales public operation name: ${definition.publicName}`,
			);
		}
		operationIds.add(definition.id);
		publicNames.add(definition.publicName);
...
export function projectSalesOperationIds<
	TDefinition extends SalesOperationDeclaration,
>(definitions: readonly TDefinition[]): readonly TDefinition["id"][] {
	return Object.freeze(definitions.map(({ id }) => id));
}

export function projectSalesAuthorization<
	TDefinition extends SalesOperationDeclaration,
>(
