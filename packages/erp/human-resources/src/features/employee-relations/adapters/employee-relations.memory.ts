...
import type { EmployeeCaseStatus } from "../status";
import type {
	EmployeeCase,
	EmployeeCaseAction,
	EmployeeCaseAppeal,
...
export interface EmployeeRelationsMemoryState {
...
export type EmployeeRelationsMemoryHost = Pick<
	HumanResourcesStore,
	"getEmployeeById" | "getEmploymentById"
>;

export type MemoryEmployeeRelationsMethods = Pick<
	HumanResourcesStore,
	| "openEmployeeCase"
...
function cloneEvent(record: EmployeeCaseEvent): EmployeeCaseEvent {
	return {
		...record,
		payloadJson: record.payloadJson === null ? null : { ...record.payloadJson },
	};
}
...
function promoteToInvestigatingIfOpen(
...
export function resetEmployeeRelationsMemoryState(
	state: EmployeeRelationsMemoryState,
): void {
	state.cases.clear();
	state.events.clear();
...
export function createMemoryEmployeeRelationsMethods(
	erState: EmployeeRelationsMemoryState,
): MemoryEmployeeRelationsMethods &
	ThisType<EmployeeRelationsMemoryHost & MemoryEmployeeRelationsMethods> {
	return {
		...
		async recommendEmployeeCaseAction(record, ports, meta) {
			...
			const replay = resolveActionReplay(
				existing.data,
				record.createRequestFingerprint,
			);
			if (!replay.ok) {
		...
		async approveEmployeeCaseAction(input, ports, meta) {
			...
			const loadedAction = getActionInCase(state, input);
			if (!loadedAction.ok) {
				return loadedAction;
			}
			const action = loadedAction.data;
