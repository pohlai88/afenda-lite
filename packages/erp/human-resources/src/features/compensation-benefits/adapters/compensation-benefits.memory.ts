...
import { buildCreateAuditFact } from "../../../kernel/emissions/audit-facts";
import type { HumanResourcesMutationMeta } from "../../../kernel/emissions/mutation-meta";
import { appendRegistryGatedOutbox } from "../../../kernel/emissions/sql-side-effects";
...
import { runRollbacks } from "../../../kernel/execution/rollback";
import {
...
import type { RecruitmentMemoryState } from "../../recruitment/adapters/recruitment.memory";
...
import { compareMoneyOrder, rangesOverlap } from "../money";
import {
	assertCompensationProposalAmendable,
	assertCompensationProposalStatusTransition,
} from "../proposal-guards";
import { compensationReviewAuditSnapshot } from "../review-audit";
import { assertReviewCycleOpenForMutation } from "../review-guards";
import {
...
import type { IdempotentCompensationReviewCycleRecord } from "../store-contract";
import {
...
} from "./compensation-review-cycle.memory";
import {
...
} from "./employee-compensation-lifecycle.memory";
...
export function createMemoryCompensationBenefitsMethods(
	...
	return {
		...
		async getApprovedCompensationHandoff(input: {
			...
			const effectiveCompensations = Array.from(
				state.employeeCompensations.values(),
			).filter(
				(compensation) =>
					...
					compensation.effectiveFrom <= input.effectiveDate &&
					(compensation.effectiveTo === null ||
						compensation.effectiveTo >= input.effectiveDate),
			);
			if (effectiveCompensations.length > 1) {
				return errorResult.fail("CONFLICT", {
					publicMessage: "The request conflicts with current state",
				});
			}
			const activeCompensation = effectiveCompensations[0] ?? null;
			...
			const activeBenefitEnrollments = Array.from(
				...
				(e) =>
					e.organizationId === input.organizationId &&
					e.employeeId === input.employeeId &&
					e.employmentId === input.employmentId &&
					e.status !== "cancelled" &&
					e.effectiveFrom <= input.effectiveDate &&
					(e.effectiveTo === null || e.effectiveTo >= input.effectiveDate),
			);
