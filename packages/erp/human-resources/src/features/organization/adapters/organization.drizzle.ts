...
import { humanResourcesEntityEventPayloadJson } from "../../../kernel/emissions/audit-facts";
	...
	async createPosition(
		...
		try {
			const [rows] = await afendaDatabase.transaction((sqlValue6) => [
				sqlValue6`
						WITH department AS (
							SELECT department.id, department.organization_id
							FROM hr_department AS department
							WHERE department.id = ${record.departmentId}
								AND department.organization_id = ${record.organizationId}
								AND department.status = 'active'
						),
						job AS (
							SELECT job.id
							FROM hr_job AS job
							WHERE job.id = ${record.jobId}
								AND job.organization_id = ${record.organizationId}
