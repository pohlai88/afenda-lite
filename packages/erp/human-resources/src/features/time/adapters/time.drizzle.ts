...
import { isResultFailure } from "../../../kernel/execution/result-guards";
import {
	runSequential,
	sequentialContinue,
	sequentialReturn,
...
} from "../attendance/exception-detection";
import {
...
} from "../attendance/session-resolution";
import {
...
import { aggregatePayrollMinutes } from "../payroll-minute-totals";
import type {
...
} from "./time-transactions.drizzle";
...
type TimeEmploymentStore = Pick<
	...
	"findOpenEmploymentByEmployee" | "getEmploymentById"
>;

type TimeEmployment =
	Awaited<ReturnType<TimeEmploymentStore["getEmploymentById"]>> extends Result<
		infer T
	>
		? T
		: never;
...
export const drizzleTimeMethods: HumanResourcesTimeStore &
	ThisType<HumanResourcesTimeStore & TimeEmploymentStore> = {
	...
	async generateTimesheetEntries(input, _ports, deps: TimesheetGenerationDeps) {
		try {
			...
			let employment: TimeEmployment = null;
			if (currentTimesheet.employmentId === null) {
				const found = await this.findOpenEmploymentByEmployee({
					organizationId: input.organizationId,
					employeeId: currentTimesheet.employeeId,
				});
				if (!found.ok) {
				...
				const employmentResult = await this.getEmploymentById({
					organizationId: input.organizationId,
