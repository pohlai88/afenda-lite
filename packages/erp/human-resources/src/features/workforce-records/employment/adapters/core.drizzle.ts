...
} from "../assignment-guards";
import { mapAssignmentLineageFields } from "../assignment-lineage-map";
import { compareEmploymentContractsByLineage } from "../employment-contract-guards";
...
export const drizzleCoreMethods: DrizzleCoreMethods &
	...
	async listEmployees(input: {
		...
		try {
			...
			if (input.employmentStatus) {
				const employments = await afendaDatabase.client
	...
	> {
		...
		const audit = preparedAudit.data;
		const preparedSuccessorAudit = afendaAudit.transaction.prepare({
			organizationId: input.organizationId,
			actorUserId: input.actorUserId,
			correlationId: meta.correlationId,
			module: "human-resources",
				...
				sqlValue2`
						...
						position AS (
							...
							WHERE position_root.id = ${record.positionId}
								...
								AND position_root.status = 'active'
						),
						legal_entity AS (
							...
							WHERE legal_entity_dimension.id = ${record.organizationDimensions.legal_entity.id}
								AND legal_entity_dimension.organization_id = ${record.organizationId}
								AND legal_entity_dimension.kind = 'legal_entity'
								AND legal_entity_dimension.effective_from <= ${record.startsOn}
								AND (legal_entity_dimension.effective_to IS NULL OR legal_entity_dimension.effective_to >= ${record.startsOn})
						),
						business_unit AS (
							...
							WHERE business_unit_dimension.id = ${record.organizationDimensions.business_unit.id}
								AND business_unit_dimension.organization_id = ${record.organizationId}
								AND business_unit_dimension.kind = 'business_unit'
								AND business_unit_dimension.effective_from <= ${record.startsOn}
								AND (business_unit_dimension.effective_to IS NULL OR business_unit_dimension.effective_to >= ${record.startsOn})
						),
						location AS (
							...
							WHERE location_dimension.id = ${record.organizationDimensions.location.id}
								AND location_dimension.organization_id = ${record.organizationId}
								AND location_dimension.kind = 'location'
								AND location_dimension.effective_from <= ${record.startsOn}
								AND (location_dimension.effective_to IS NULL OR location_dimension.effective_to >= ${record.startsOn})
						),
						cost_centre AS (
							...
							WHERE cost_centre_dimension.id = ${record.organizationDimensions.cost_centre.id}
								AND cost_centre_dimension.organization_id = ${record.organizationId}
								AND cost_centre_dimension.kind = 'cost_centre'
								AND cost_centre_dimension.effective_from <= ${record.startsOn}
								AND (cost_centre_dimension.effective_to IS NULL OR cost_centre_dimension.effective_to >= ${record.startsOn})
						),
						project AS (
							...
							WHERE project_dimension.id = ${record.organizationDimensions.project.id}
								AND project_dimension.organization_id = ${record.organizationId}
								AND project_dimension.kind = 'project'
								AND project_dimension.effective_from <= ${record.startsOn}
								AND (project_dimension.effective_to IS NULL OR project_dimension.effective_to >= ${record.startsOn})
						),
						mutated AS (
