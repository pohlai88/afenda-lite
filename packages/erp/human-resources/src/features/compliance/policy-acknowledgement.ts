...
import { fingerprintPolicyAcknowledgementIssue } from "../../kernel/identity/fingerprint";
import {
	...
	return runComplianceCapabilityCommand(input, options, {
		...
		execute: async (data, { store, ports }) => {
			...
			if (existingByKey.data !== null) {
				...
				return errorResult.ok(existingByKey.data.acknowledgement);
			}
			...
			return store.issuePolicyAcknowledgementRequirement(
				...
				buildMutationMeta({
					...
					operationId: HUMAN_RESOURCES_COMMAND_POLICY_ACKNOWLEDGEMENT_ISSUE,
				}),
			);
		},
	});
	...
	return runComplianceCapabilityCommand(input, options, {
		...
		execute: (data, { store, ports }) =>
			store.revokePolicyAcknowledgement(
				...
				buildMutationMeta({
					correlationId: data.correlationId,
					operationId: HUMAN_RESOURCES_COMMAND_POLICY_ACKNOWLEDGEMENT_REVOKE,
				}),
			),
	});
}
	...
	return runComplianceCapabilityCommand(input, options, {
		...
		execute: (data, { store, ports }) =>
			store.supersedePolicyAcknowledgementRequirement(
				...
				buildMutationMeta({
					correlationId: data.correlationId,
					operationId: HUMAN_RESOURCES_COMMAND_POLICY_ACKNOWLEDGEMENT_SUPERSEDE,
				}),
			),
	});
}
