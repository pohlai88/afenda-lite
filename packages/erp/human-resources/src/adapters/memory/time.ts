...
export function createMemoryTimeMethods(
	...
	return {
		...
		async recordAttendanceEvent(input: AttendanceEventRecordInput, ports) {
			...
			const correlationId =
				input.correlationId ??
				`hr-time-hr_attendance_event-${event.id}`;
			const recorded = await audit(ports, {
		...
		async correctAttendanceEvent(input, ports) {
			...
			if (!recorded.ok) {
				...
				state.attendanceAdjustments.pop();
				return recorded;
			}
			const outbox = await emitOutbox(ports, {
				organizationId: event.organizationId,
			...
			for (const session of sessions) {
				const already = Array.from(state.timesheetEntries.values()).some(
					(entry) =>
						...
						entry.sourceReference === session.id,
				);
				if (already) continue;
		...
		async approveTimesheet(input, ports) {
			...
			return transitionTimesheet(state, ports, input, "approved", {
				approvedAt: new Date(),
		...
		async approveOvertimeRequest(input, ports) {
			...
			const correlationId =
				input.correlationId ??
				`hr-time-hr_overtime_request-${request.id}`;
			const recorded = await audit(ports, {
				organizationId: request.organizationId,
				actorUserId: input.actorUserId,
				correlationId,
...
async function transitionAssignment(
	...
	const correlationId =
		input.correlationId ??
		`hr-time-hr_shift_assignment-${assignment.id}`;
	const recorded = await audit(ports, {
		organizationId: assignment.organizationId,
		actorUserId: input.actorUserId,
		correlationId,
