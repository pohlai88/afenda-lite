...
Domain-sliced Zod input schemas for `@afenda/human-resources`. The composed entry is `src/kernel/validation/index.ts`; compliance schemas live in `src/features/compliance/schema.ts`.
