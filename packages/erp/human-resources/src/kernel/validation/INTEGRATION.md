...
```text
PRODUCTION (SSOT)
  src/kernel/validation/index.ts        composed schema barrel
  src/features/compliance/schema.ts   compliance schemas
  src/schemas/**              domain Zod modules

  src/composition/store/index.ts          composed store contract
