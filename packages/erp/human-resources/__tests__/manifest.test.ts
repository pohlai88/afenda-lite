...
describe("humanResourcesModuleManifest", () => {
	...
	it("keeps auth maps aligned with shipped command and query ids", () => {
		...
		).toBe(HUMAN_RESOURCES_PERMISSION_LEAVE_HANDOFF_READ);
		expect(
			humanResourcesModuleManifest.authorization.queries[
				HUMAN_RESOURCES_QUERY_APPROVED_TIME_HANDOFF_GET
			],
		).toBe(HUMAN_RESOURCES_PERMISSION_TIME_HANDOFF_READ);
		expect(HUMAN_RESOURCES_PERMISSION_CODES).toContain(
			HUMAN_RESOURCES_PERMISSION_TIME_TIMESHEET_APPROVE,
		);
		expect(HUMAN_RESOURCES_PERMISSION_CODES).toContain(
	...
	it("keeps the kernel error catalog complete", () => {
		...
		).toBe("human_resources.assignment_outside_employment_range");
