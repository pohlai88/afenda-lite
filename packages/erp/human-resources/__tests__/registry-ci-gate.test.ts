 ...
 * Strict 290/290 — zero HR-AUD-06 exemptions.
 */
...
describe("Slice 3.8 — HR mutation-emission registry CI gate", () => {
	...
	it("locks classified count at 290 and inventory alignment with zero unclassified", () => {
		expect(Object.keys(HUMAN_RESOURCES_MUTATION_EMISSION_REGISTRY_RECORD)).toHaveLength(
			290,
		);
		expect(HUMAN_RESOURCES_MUTATION_EMISSION_REGISTRY).toHaveLength(290);
		...
		expect(classifiedIds.size).toBe(290);
	});
});
