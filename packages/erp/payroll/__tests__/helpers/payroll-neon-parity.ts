...
function assertNotProductionDatabaseTarget(): void {
	...
	if (declaredTarget !== "test" && declaredTarget !== "preview") {
		throw new Error(
			"Payroll Neon parity requires AFENDA_DATABASE_TEST_TARGET=test or preview.",
		);
	}
	...
	for (const [name, value] of [
		...
		if (value === "production") {
			throw new Error(
				`Payroll Neon parity must not target production (${name}=production).`,
			);
		}
	}
}
...
export const PAYROLL_NEON_PARITY_SKIP_REASON =
...
export const RUN_PAYROLL_NEON_PARITY =
