...
| D0 | Statutory-fact capture — regional minimum-wage zone ownership | **CLOSED (Stage 1)** | The VN regional minimum-wage zone (I–IV) is an **HR-owned employment fact derived from work location**, not payroll setup: it changes when the employee moves, it is captured with the rest of the statutory profile, and payroll consumes it through the handoff rather than deriving or configuring it. Stage 1 ships the HR capture surface `@afenda/human-resources` `src/features/statutory-profile/` — effective-dated `hr_statutory_profile` (tax residency, nationality + expatriate flag, dependant count + closed relief-declaration array pinned to `hr.statutory-relief.v1`, statutory identifiers, VN zone) with open-segment supersession, plus `hr_prior_employer_ytd` for mid-year joiners (money-as-string + currency). Operations `upsertStatutoryProfile` / `getStatutoryProfile` (as-of) / `listStatutoryProfiles` / `recordPriorEmployerYtd` / `listPriorEmployerYtd` reuse the existing `human-resources.sensitive-identifiers.manage` and `.read` permission codes — no new platform-catalog permission. Reads are fail-closed on the privacy capability and exclude restriction-active subjects (D7); both tables are registered in the DSAR subject collector as `highly_sensitive` / `pay_and_benefits`. Schema `0056_hr_statutory_profile` (ops-gated migrate). **Still open:** Stage 2 handoff widening (`packages/data-plane/events/src/schemas/hr-payroll-handoff.ts` + HR handoff assembly) and Stage 3 payroll YTD port — until those land the D4 leave-balance and prior-employer inputs stay caller-asserted. | 2026-08-06 | Human Resources |
