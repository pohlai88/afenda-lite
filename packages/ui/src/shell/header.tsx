"use client";

import { ActivityIcon, BellIcon } from "lucide-react";
import type { ReactNode } from "react";
import ActivityDialog from "../components/composite/activity-dialog";
import NotificationDropdown from "../components/composite/notification-dropdown";
import ProfileDropdown from "../components/composite/profile-dropdown";
import { Button } from "../components/ui/button";
import { Separator } from "../components/ui/separator";
import { SidebarTrigger, useSidebar } from "../components/ui/sidebar";
import { useSettings } from "../hooks/use-settings";
import { cn } from "../lib/utils";
import CommandMenu from "./command-menu";
import ModeToggle from "./mode-toggle";
import ThemeCustomizer from "./theme-customizer";

const Header = ({
	profileSlot,
	notificationSlot,
	activitySlot,
}: {
	/** When set, replaces the default ProfileDropdown (playground lab fixtures). */
	profileSlot?: ReactNode;
	/** When set, replaces the default NotificationDropdown (playground lab fixtures). */
	notificationSlot?: ReactNode;
	/** When set, replaces the default ActivityDialog (playground lab fixtures). */
	activitySlot?: ReactNode;
}) => {
	const { settings, updateSettings } = useSettings();
	const { open } = useSidebar();

	return (
		<header className="sticky top-0 z-50 px-4 before:absolute before:inset-0 before:rounded-t-xl before:mask-[linear-gradient(var(--card),var(--card)_18%,transparent_100%)] before:backdrop-blur-md sm:px-6">
			<div
				className={cn(
					"bg-card relative z-51 mx-auto mt-3 flex w-full items-center justify-between rounded-xl border px-6 py-2",
					settings.layout === "compact" ? "max-w-348" : "w-full",
				)}
			>
				<div className="flex items-center gap-1.5 sm:gap-4">
					<SidebarTrigger
						className="[&_svg]:size-5!"
						onClick={() => updateSettings({ sidebarOpen: !open })}
					/>
					<Separator
						orientation="vertical"
						className="hidden h-4! self-center! sm:block"
					/>
					<CommandMenu />
				</div>
				<div className="flex items-center gap-1.5">
					{activitySlot ?? (
						<ActivityDialog
							trigger={
								<Button variant="ghost" size="icon">
									<ActivityIcon />
								</Button>
							}
							triggerClassName="max-md:hidden"
						/>
					)}
					{notificationSlot ?? (
						<NotificationDropdown
							trigger={
								<Button variant="ghost" size="icon" className="relative">
									<BellIcon />
									<span className="bg-destructive absolute top-2 right-2.5 size-2 rounded-full" />
								</Button>
							}
						/>
					)}
					<ModeToggle />
					<ThemeCustomizer />
					{profileSlot ?? <ProfileDropdown />}
				</div>
			</div>
		</header>
	);
};

export default Header;
