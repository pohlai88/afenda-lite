...
type UserViewProfileData = Pick<
	AppUser,
	"twoFactorEnabled" | "recentDevices" | "notificationSettings"
>;

function createUserViewProfile(
	userIndex: number,
	country: string,
): UserViewProfileData {
	const browsers = [
	...
	return {
		...
		recentDevices: [
			{
				...
				browser: requireAt(browsers, offset % browsers.length, "browser"),
				device: requireAt(devices, offset % devices.length, "device"),
				location: country,
			...
			},
			{
				...
				browser: requireAt(browsers, (offset + 1) % browsers.length, "browser"),
				device: requireAt(devices, (offset + 2) % devices.length, "device"),
				location: country,
				lastActive: `${1 + (offset % 3)} days ago`,
			},
			{
				id: `device-${String(userIndex).padStart(3, "0")}-3`,
				browser: requireAt(browsers, (offset + 2) % browsers.length, "browser"),
				device: requireAt(devices, (offset + 3) % devices.length, "device"),
				location: country,
		...
		notificationSettings: [
			...
			},
			{
				...
				description:
					"Email when your account has new activity or security alerts.",
				email: true,
				browser: true,
				app: offset % 3 !== 0,
			},
