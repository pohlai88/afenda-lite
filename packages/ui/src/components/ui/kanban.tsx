  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (onMove && !isColumn(active.id)) {
        ...
        if (activeContainer && overContainer) {
          const activeColumnItems = columns[activeContainer] ?? []
          const overColumnItems = columns[overContainer] ?? []
          const activeIndex = activeColumnItems.findIndex((item: T) => getItemValue(item) === active.id)

          const overIndex = isColumn(over.id)
            ? overColumnItems.length
            : overColumnItems.findIndex((item: T) => getItemValue(item) === over.id)
