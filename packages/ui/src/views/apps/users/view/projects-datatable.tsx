...
const getProjectLogoColor = (id: string): string => {
  ...
  return PROJECT_LOGO_COLORS[hash % PROJECT_LOGO_COLORS.length] ?? PROJECT_LOGO_COLORS[0]!
}

const getProjectProgressColor = (id: string): string => {
  const hash = id.split('').reduce((sum, char) => sum + char.charCodeAt(0), 0)

  return PROJECT_PROGRESS_COLORS[hash % PROJECT_PROGRESS_COLORS.length] ?? PROJECT_PROGRESS_COLORS[0]!
}
