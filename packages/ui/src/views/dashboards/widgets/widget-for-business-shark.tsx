    ...
    <Card className={className}>
      ...
      <CardContent className='space-y-2'>
        ...
        {plans.map((plan, index) => (
          <Label
            key={index}
            className='has-data-checked:border-primary dark:has-data-checked:border-primary flex cursor-pointer items-center gap-3 rounded-md border px-4 py-2'
          >
            <Checkbox defaultChecked={plan.checked} />
            <div className='flex w-full items-center justify-between gap-2'>
              <p className='text-sm leading-none font-medium'>{plan.name}</p>
              <Badge className='bg-primary/10 text-primary h-6 rounded-sm px-3 py-1'>${plan.price}</Badge>
            </div>
          </Label>
        ))}
      </CardContent>
