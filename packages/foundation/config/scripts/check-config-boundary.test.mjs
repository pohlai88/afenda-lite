...
function write(root, relativePath, contents) {
	...
	writeFileSync(
		file,
		typeof contents === "string" ? contents : `${JSON.stringify(contents, null, "\t")}\n`,
	);
}
...
function fixture() {
	...
	write(root, `${CONFIG_DIR}/package.json`, { name: "@afenda/config", exports: EXPORTS });
...
test("INV-4 rejects a runtime import", () => {
	const root = fixture();
	write(root, "apps/web/source.ts", 'import config from "@afenda/config";\nvoid config;\n');
...
test("INV-9 honours the named exceptions", () => {
	const root = fixture();
	write(root, "packages/foundation/errors/package.json", { name: "@afenda/errors" });
...
test("INV-10 rejects an invariant documented but not asserted", () => {
	const root = fixture();
	write(root, `${CONFIG_DIR}/CONTRACT.md`, `${CONTRACT}| **INV-11** | invented |\n`);
