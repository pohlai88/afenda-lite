...
semantic-consumer, hostile-boundary, deletion, and documentation lanes. The
execution evidence is recorded in this contract and living repository gates.
...
- Lane 7 is complete. Package documentation, generated OpenAPI, repository
  semantic gates, and focused consumer suites match the final cutover. The
  saturated combined repository test run produced one resource-only Master Data
  timeout; that complete package passes standalone.
...
Current verified semantic root exports (`package.json.exports` is root `"."` only):
...
Public subpaths `./result`, `./http`, `./common`, and `./adapters/postgres` are
...
Consumer and maintainer entry: `packages/foundation/errors/README.md` (purpose,
root-only quick start, decision table, capabilities, security, commands).

Product / governance PRD: `packages/foundation/errors/docs/PRD.md` (kernel
package PRD). Semantic SSOT is `packages/foundation/errors/docs/CONTRACT.md`
plus sealed disk.

Do not restore package-root copies of `PRD.md`, `CONTRACT.md`, or `ADMISSION.md`.
Do not recreate retired Scratch trees as documentation authority. Repo-root
Living `docs/architecture/**` bodies remain absent until an explicit Docs-lane
reopen.
...
Documentation closure includes governed references to deleted subpaths,
`AppError`, factories, maps, superseded ActionResult shapes, and old wire shapes
in `AGENTS.md`, farm companions, and the official generated package page.
...
| 7. Governance seal | **Implemented; sealed** | Permanent checks, docs regeneration | See living gates + this contract |
...
contract, and focused consumers pass. The one saturated combined-workload
timeout and its green standalone rerun are recorded in this contract.
...
Focused package and repository errors gates are authoritative. Broader monorepo
aggregates are CI-parity only when justified.
