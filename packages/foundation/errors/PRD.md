...
**Placement note.** Kit index paths under `docs/kernel/package-specs/**` are blocked by the Living-`docs/` write hook until an explicit Docs-lane reopen. This file is the operative individual PRD beside the sealed package. `docs/kernel/` kit bodies remain stale until that reopen syncs them to point here.
...
Package cutover seal (`.protected.sha256`) is not a KERNEL-GOVERNANCE digest-bound seal and is not Module Enterprise Readiness.
...
Living scan evidence (cutover): 834 canonical consumers; 0 partial consumers; 0 prohibited interpretation findings (`PR.md`).
...
| 09 | `errorWire.serialize` emits `{ schema: "afenda.failure/v1", error }`; deserialize accepts V1 + retained legacy flat shape. |
...
- Compatibility dimensions: source, runtime, wire (`afenda.failure/v1` + retained legacy flat), generated OpenAPI consumer docs.
...
| Package protection digest | `protect:check` / `.protected.sha256` |
...
Historical Lanes 1–7 in `CONTRACT.md` / `PR.md` are **complete cutover evidence**. Remaining slices close KERNEL formalization; they do not rewrite the sealed facade.

| Slice | Coherent outcome | Gate to advance |
| --- | --- | --- |
| ERRORS-PRD-1 | This individual PRD exists beside the package; kit dual-body pending Docs-lane sync | Doc parity vs CONTRACT + living exports |
...
- Permanent boundary/semantic gates green; protection digest current.
...
- Treating package protection digest as KERNEL seal or Module Enterprise Readiness.
...
Compatibility: internal for representation; sealed cutover already deleted superseded surfaces
Before digest: 2f0fbf870253c1a39b1355152a0a4f583b17b15b8800fe79a4930a18e0f173f7
After digest: re-run inspector (embedding digests inside protected docs races the hash)
Contract changes: PRD + packages/KERNEL-GOVERNANCE.md authority normalization; no public API change
Auxiliary entrypoints: none
...
Deleted surfaces: none this turn (historical cutover already removed legacy APIs)
Consumer blast radius: 0 (documentation/governance only)
Consumers checked: n/a for docs-only
Evidence: inspector snapshot; PRD authored against CONTRACT + disk
Remaining conditions: G1–G10 KERNEL VERIFIED gaps; Docs-lane reopen to sync docs/kernel kit
```
